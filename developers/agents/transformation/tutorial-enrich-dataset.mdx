...
In this tutorial, we will build a simple Weaviate **[`TransformationAgent`](./index.md)**. We will build an agent that has access to a collection containing a bunch of research papers, their abstracts, and titles. We will then use the agent to create additional properties for each of our objects in the collection.
...
```
Size of the ArxivPapers dataset: 448
```
...
Finally, let's ask for a `BOOL` property, which indicates whether the paper is a survey or not. I.e., we'll ask the LLM to determine if the paper presents novel techniques or if it's a survey of existing ones.
...
When initializing the agent, we have to decide which `collection` it may have access to. In this case, we want it to have access to the "ArxivPapers" collection we previously created.
