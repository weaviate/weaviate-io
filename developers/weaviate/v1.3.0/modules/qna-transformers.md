...
# In short
* The Question Answering (Q&A) module is a Weaviate module for answer extraction from data.
* The module depends on a text vectorization module that should be running with Weaviate.
* The module adds an `ask {}` filter to the GraphQL `Get {}` queries
...
services:
  weaviate:
    ...
    - --scheme
    - http
    image: semitechnologies/weaviate:1.3.0
    ports:
    - 8080:8080
...
This module adds a search filter to GraphQL `Get{...}` queries: `ask{}`. This new search filter takes the following arguments: 
...
| `properties` 	| list of strings 	| no 	| `["summary"]` 	| The properties of the of the queries Class which contains text. If no properties are set, all are considered	|
...
* You cannot use the `'ask'` filter along with a `'near'` filter!

### Example query

{% include code/1.x/qna-transformers.ask.html %}

{% include molecule-gql-demo.html encoded_query='%7B%0D%0A++Get+%7B%0D%0A++++Article%28%0D%0A++++++ask%3A+%7B%0D%0A++++++++question%3A+%22Who+is+the+king+of+the+Netherlands%3F%22%2C%0D%0A++++++++certainty%3A+0.7%2C%0D%0A++++++++properties%3A+%5B%22summary%22%5D%0D%0A++++++%7D%2C+%0D%0A++++++limit%3A1%0D%0A++++%29+%7B%0D%0A++++++title%0D%0A++++++_additional+%7B%0D%0A++++++++answer+%7B%0D%0A++++++++++hasAnswer%0D%0A++++++++++certainty%0D%0A++++++++++property%0D%0A++++++++++result%0D%0A++++++++++startPosition%0D%0A++++++++++endPosition%0D%0A++++++++%7D%0D%0A++++++%7D%0D%0A++++%7D%0D%0A++%7D%0D%0A%7D' %}
