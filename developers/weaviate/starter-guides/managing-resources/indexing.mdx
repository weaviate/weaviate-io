...
A dynamic index requires its flat and HNSW index settings at creation time. The dynamic index uses the flat index settings initially, then automatically switch to the HNSW index with provided settings when the threshold is reached.
:::
