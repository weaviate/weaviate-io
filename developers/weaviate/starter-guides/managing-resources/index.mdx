...
Weaviate's flexible resource management features allow you to balance search speeds, accuracy/recall and costs.

This guide will help you make informed decisions about resource allocation by providing an overview of the following:
...
A tenant status management strategy can help you to balance cost and performance. *Hot* tenants can be deactivated to *warm* storage to reduce memory usage, and any tenant can be offloaded to *cold* storage to reduce memory and disk usage. Conversely, any tenant can be reactivated when needed.
