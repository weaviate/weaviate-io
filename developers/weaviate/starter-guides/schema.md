...
Weaviate uses two types of indexes: [vector indexes](../concepts/indexing/vector-indexes) and [inverted indexes](../concepts/indexing.md#inverted-indexes). Vector indexes are used to store and organize vectors for fast vector similarity-based searches. Inverted indexes are used to store data for fast filtering and keyword searches.
