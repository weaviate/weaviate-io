...
Weaviate is a vector database. [Vector databases](/blog/what-is-a-vector-database) store data objects and vectors that represent those objects. The vector embedding is also called an "embedding."
...
This guide uses a Weaviate Cloud instance to host the collection. To connect to a Weaviate Cloud instance, you need the following information:
...
Weaviate stores [data](../concepts/data.md) in collections. Each data object in a collection has a set of properties and a vector embedding. Before you import data, you should create a [collection definition](/developers/weaviate/config-refs/schema) to define the data properties for the collection.
...
This [`nearVector`](/developers/weaviate/search/similarity#search-with-a-vector) query supplies a query vector. The query vector is an embedding of the query string, "biology".
