---
title: Batch import
sidebar_position: 15
image: og/docs/howto.jpg
# tags: ['how-to', 'import']
---

import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import FilteredTextBlock from '@site/src/components/Documentation/FilteredTextBlock';
import PyCode from '!!raw-loader!/_includes/code/howto/manage-data.import.py';
import PyCodeV3 from '!!raw-loader!/_includes/code/howto/manage-data.import-v3.py';
import TSCode from '!!raw-loader!/_includes/code/howto/manage-data.import.ts';
import TSCodeLegacy from '!!raw-loader!/_includes/code/howto/manage-data.import-v2.ts';
import JavaCode from '!!raw-loader!/_includes/code/howto/java/src/test/java/io/weaviate/docs/manage-data.import.java';
import GoCode from '!!raw-loader!/_includes/code/howto/go/docs/manage-data.import_test.go';

[Batch imports](../tutorials/import.md#to-batch-or-not-to-batch) are an efficient way to add multiple data objects and cross-references.

<details>
  <summary>Additional information</summary>

To create a bulk import job, follow these steps:

1. Initialize a batch object.
1. Add items to the batch object.
1. Ensure that the last batch is sent (flushed).

</details>

## Basic import

The following example adds objects to the `YourName` collection.

<Tabs groupId="languages">
  <TabItem value="py" label="Python (v4)">
    <FilteredTextBlock
      text={PyCode}
      startMarker="# BasicBatchImportExample"
      endMarker="# END BasicBatchImportExample"
      language="py"
    />
  </TabItem>

  <TabItem value="py3" label="Python (v3)">
    <FilteredTextBlock
      text={PyCodeV3}
      startMarker="# BasicBatchImportExample"
      endMarker="# END BasicBatchImportExample"
      language="py"
    />
  </TabItem>

  <TabItem value="js" label="JS/TS v3">
    <FilteredTextBlock
      text={TSCode}
      startMarker="// BasicBatchImportExample"
      endMarker="// END BasicBatchImportExample"
      language="ts"
    />
  </TabItem>

  <TabItem value="js2" label="JS/TS v2">
    <FilteredTextBlock
      text={TSCodeLegacy}
      startMarker="// BasicBatchImportExample"
      endMarker="// END BasicBatchImportExample"
      language="ts"
    />
  </TabItem>

  <TabItem value="java" label="Java">
    <FilteredTextBlock
      text={JavaCode}
      startMarker="// BasicBatchImportExample"
      endMarker="// END BasicBatchImportExample"
      language="java"
    />
  </TabItem>

  <TabItem value="go" label="Go">
    <FilteredTextBlock
      text={GoCode}
      startMarker="// BasicBatchImportExample"
      endMarker="// END BasicBatchImportExample"
      language="go"
    />
  </TabItem>
</Tabs>

## Specify an ID value

Weaviate generates an UUID for each object. Object IDs must be unique. If you set object IDs, use one of these [deterministic UUID](#generate-a-deterministic-id) methods to prevent duplicate IDs:

- [`generate_uuid5`](https://weaviate-python-client.readthedocs.io/en/stable/weaviate.util.html#weaviate.util.generate_uuid5) (Python)
- `generateUuid5` (TypeScript)

<Tabs groupId="languages">
  <TabItem value="py" label="Python (v4)">
    <FilteredTextBlock
      text={PyCode}
      startMarker="# BatchImportWithIDExample"
      endMarker="# END BatchImportWithIDExample"
      language="py"
    />
  </TabItem>

  <TabItem value="py3" label="Python (v3)">
    <FilteredTextBlock
      text={PyCodeV3}
      startMarker="# BatchImportWithIDExample"
      endMarker="# END BatchImportWithIDExample"
      language="py"
    />
  </TabItem>

  <TabItem value="js" label="JS/TS v3">
    <FilteredTextBlock
      text={TSCode}
      startMarker="// BatchImportWithIDExample"
      endMarker="// END BatchImportWithIDExample"
      language="ts"
    />
  </TabItem>

  <TabItem value="js2" label="JS/TS v2">
    <FilteredTextBlock
      text={TSCodeLegacy}
      startMarker="// BatchImportWithIDExample"
      endMarker="// END BatchImportWithIDExample"
      language="ts"
    />
  </TabItem>

  <TabItem value="java" label="Java">
    <FilteredTextBlock
      text={JavaCode}
      startMarker="// BatchImportWithIDExample"
      endMarker="// END BatchImportWithIDExample"
      language="java"
    />
  </TabItem>

  <TabItem value="go" label="Go">
    <FilteredTextBlock
      text={GoCode}
      startMarker="// BatchImportWithIDExample"
      endMarker="// END BatchImportWithIDExample"
      language="go"
    />
  </TabItem>
</Tabs>

## Specify a vector

Use the `vector` property to specify a vector for each object.

<Tabs groupId="languages">
  <TabItem value="py" label="Python (v4)">
    <FilteredTextBlock
      text={PyCode}
      startMarker="# BatchImportWithVectorExample"
      endMarker="# END BatchImportWithVectorExample"
      language="py"
    />
  </TabItem>

  <TabItem value="py3" label="Python (v3)">
    <FilteredTextBlock
      text={PyCodeV3}
      startMarker="# BatchImportWithVectorExample"
      endMarker="# END BatchImportWithVectorExample"
      language="py"
    />
  </TabItem>

  <TabItem value="js" label="JS/TS v3">
    <FilteredTextBlock
      text={TSCode}
      startMarker="// BatchImportWithVectorExample"
      endMarker="// END BatchImportWithVectorExample"
      language="ts"
    />
  </TabItem>

  <TabItem value="js2" label="JS/TS v2">
    <FilteredTextBlock
      text={TSCodeLegacy}
      startMarker="// BatchImportWithVectorExample"
      endMarker="// END BatchImportWithVectorExample"
      language="ts"
    />
  </TabItem>

  <TabItem value="java" label="Java">
    <FilteredTextBlock
      text={JavaCode}
      startMarker="// BatchImportWithVectorExample"
      endMarker="// END BatchImportWithVectorExample"
      language="java"
    />
  </TabItem>

  <TabItem value="go" label="Go">
    <FilteredTextBlock
      text={GoCode}
      startMarker="// BatchImportWithVectorExample"
      endMarker="// END BatchImportWithVectorExample"
      language="go"
    />
  </TabItem>
</Tabs>

## Specify named vectors

:::info Added in `v1.24`
:::

When you create an object, you can specify named vectors (if [configured in your collection](./collections.mdx#define-multiple-named-vectors)).

<Tabs groupId="languages">
  <TabItem value="py" label="Python (v4)">
    <FilteredTextBlock
      text={PyCode}
      startMarker="# BatchImportWithNamedVectors"
      endMarker="# END BatchImportWithNamedVectors"
      language="py"
    />
  </TabItem>

  <TabItem value="py3" label="Python (v3)">
    <FilteredTextBlock
      text={PyCodeV3}
      startMarker="# BatchImportWithNamedVectors"
      endMarker="# END BatchImportWithNamedVectors"
      language="py"
    />
  </TabItem>

  <TabItem value="js" label="JS/TS v3">
    <FilteredTextBlock
      text={TSCode}
      startMarker="// BatchImportWithNamedVectors"
      endMarker="// END BatchImportWithNamedVectors"
      language="ts"
    />
  </TabItem>

  <TabItem value="js2" label="JS/TS v2">
    <FilteredTextBlock
      text={TSCodeLegacy}
      startMarker="// BatchImportWithNamedVectors"
      endMarker="// END BatchImportWithNamedVectors"
      language="ts"
    />
  </TabItem>

</Tabs>

## Import with references

You can batch create links from an object to another other object through cross-references.

<Tabs groupId="languages">
  <TabItem value="py" label="Python (v4)">
    <FilteredTextBlock
      text={PyCode}
      startMarker="# BatchImportWithRefExample"
      endMarker="# END BatchImportWithRefExample"
      language="py"
    />
  </TabItem>
  <TabItem value="py3" label="Python (v3)">
    <FilteredTextBlock
      text={PyCodeV3}
      startMarker="# BatchImportWithRefExample"
      endMarker="# END BatchImportWithRefExample"
      language="py"
    />
  </TabItem>
  <TabItem value="js2" label="JS/TS v2">
    <FilteredTextBlock
      text={TSCodeLegacy}
      startMarker="// BatchImportWithRefExample"
      endMarker="// END BatchImportWithRefExample"
      language="ts"
    />
  </TabItem>
</Tabs>

## Python-specific considerations

The Python clients have built-in batching methods to help you optimize import speed. For details, see the client documentation:

- [Python client `v4`](../client-libraries/python/index.md#batching)
- [Python client `v3`](../client-libraries/python/python_v3.md#batching)

## Stream data from large files

If your dataset is large, consider streaming the import to avoid out-of-memory issues.

<!-- TODOv4: update Python - JSON and CSV with v4 code -->

<Tabs groupId="languages">
  <TabItem value="py" label="Python (v4) - JSON">

  <FilteredTextBlock
    text={PyCode}
    startMarker="# START JSON streaming"
    endMarker="# END JSON streaming"
    language="py"
  />

  </TabItem>
  <TabItem value="pycsv" label="Python (v4) - CSV">

  <FilteredTextBlock
    text={PyCode}
    startMarker="# START CSV streaming"
    endMarker="# END CSV streaming"
    language="py"
  />

  </TabItem>
  <TabItem value="py3" label="Python (v3) - JSON">

  <FilteredTextBlock
    text={PyCodeV3}
    startMarker="# START JSON streaming"
    endMarker="# END JSON streaming"
    language="py"
  />

  </TabItem>
  <TabItem value="py3csv" label="Python (v3) - CSV">

  <FilteredTextBlock
    text={PyCodeV3}
    startMarker="# START CSV streaming"
    endMarker="# END CSV streaming"
    language="py"
  />

  </TabItem>
  <TabItem value="js" label="TypeScript - JSON">

  <FilteredTextBlock
    text={TSCode}
    startMarker="// START JSON streaming"
    endMarker="// END JSON streaming"
    language="ts"
  />

  </TabItem>
  <TabItem value="jscsv" label="TypeScript - CSV">

  <FilteredTextBlock
    text={TSCode}
    startMarker="// START CSV streaming"
    endMarker="// END CSV streaming"
    language="ts"
  />

  </TabItem>
</Tabs>

## Batch vectorization

:::info Added in `v1.25`.
:::

import BatchVectorizationOverview from '/_includes/code/client-libraries/batch-import.mdx';

<Tabs groupId="languages">
  <TabItem value="py" label="Python (v4)">
    <FilteredTextBlock
      text={PyCode}
      startMarker="# START BatchVectClient"
      endMarker="# END BatchVectClient"
      language="py"
    />
  </TabItem>
</Tabs>

## Model provider configurations

You can configure the batch vectorization settings for each model provider, such as the requests per minute or tokens per minute. Note that each provider exposes different configuration options. The following examples sets rate limits for Cohere and OpenAI integrations, and provides API keys for both.

 <Tabs groupId="languages">
  <TabItem value="py" label="Python (v4)">
    <FilteredTextBlock
      text={PyCode}
      startMarker="# START BatchVectorizationClientModify"
      endMarker="# END BatchVectorizationClientModify"
      language="py"
    />
  </TabItem>
</Tabs>

### Rate limiting headers

To manually set rate limits for the batch vectorizer, send the following request headers:

| Purpose | Header |
| :- | :- |
| Limit tokens | x-ratelimit-limit-tokens |
| Remaining tokens | x-ratelimit-remaining-tokens |
| Reset tokens | x-ratelimit-reset-tokens |
| Limit requests | x-ratelimit-limit-requests |
| Remaining requests | x-ratelimit-remaining-requests |
| Reset requests | x-ratelimit-reset-requests |

## Additional considerations

Data imports can be resource intensive. Consider the following when you import large amounts of data.

### Asynchronous imports

:::info Added in `v1.22`.
:::

To maximize import speed, enable [asynchronous indexing](/developers/weaviate/config-refs/schema/vector-index#asynchronous-indexing).

Asynchronous indexing is an experimental feature in `1.22`, and may not be suitable for production use.

To enable asynchronous indexing, set the `ASYNC_INDEXING` environment variable to `true` in your Weaviate configuration file.

```yaml
weaviate:
  image: cr.weaviate.io/semitechnologies/weaviate:||site.weaviate_version||
  ...
  environment:
    ASYNC_INDEXING: 'true'
  ...
```
### gRPC connection

:::info Added in `v1.23`.
:::

The gRPC API is faster than the REST API. Use the gRPC API to improve import speeds.

The [Python client `v4`](../client-libraries/python/index.md) and the [TypeScript client v3](/developers/weaviate/client-libraries/typescript/typescript-v3) use gRPC. If you cannot use one of the new clients, you can also access the [gRPC API directly](../api/grpc.md).

## Related pages

- [Connect to Weaviate](/developers/weaviate/starter-guides/connect.mdx)
- [How-to: Create objects](./create.mdx)
- [References: REST - /v1/batch](/developers/weaviate/api/rest#tag/batch)
- [Configuration: Indexes](/developers/weaviate/config-refs/schema/vector-index#asynchronous-indexing)

import DocsMoreResources from '/_includes/more-resources-docs.md';

<DocsMoreResources />
