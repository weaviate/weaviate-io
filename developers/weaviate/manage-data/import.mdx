...
To maximise import speed, enable asynchronous indexing in Weaviate and use gRPC batch imports.

Both features are available in Weaviate `1.22` and higher. (Note that asynchronous indexing is still experimental as of `1.22`, and we do not recommend using it in production.)

#### How to enable asynchronous indexing

To enable asynchronous indexing, set the `ASYNC_INDEXING` environment variable to `true` in the Weaviate configuration (e.g. in the `docker-compose.yml` file) ([read more](../configuration/indexes.md#asynchronous-indexing)).
...
:::info Where is the class definition?
...
You can optionally specify a [UUID](https://en.wikipedia.org/wiki/Universally_unique_identifier) as the ID, leave it blank to have Weaviate generate one. You can also use a deterministic UUID (UUIDv5) as shown here to [prevent duplicates](#managing-potential-duplication).
