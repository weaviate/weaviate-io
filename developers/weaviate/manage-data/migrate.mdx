...
Cross-references are properties. As such, these cursor-based exports will include them. Even in cases where the target object does not exist yet, the cross-reference should be included in the export, as validation for cross-references are disabled.
<br/>

As long as the migration is complete, the target object should be created, and the cross-reference should be valid.
