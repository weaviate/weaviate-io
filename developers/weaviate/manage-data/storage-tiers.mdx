...
In all collections, vector indexes can be stored in different storage tiers, depending on the [vector index type](#vector-index-temperature).

In multi-tenant collections, you can additionally change tenant states (`Active`, `Inactive`, `Offloaded`) which will move data between storage tiers to trade off resource availability and readiness. <!--This way you can manage performance and cost.-->
