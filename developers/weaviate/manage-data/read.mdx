...
To efficiently check if an object with a given [id](../api/graphql/additional-properties.md#id) exists without retrieving it, make a `HEAD` request to the [`/v1/objects/` REST endpoint](/developers/weaviate/api/rest_openapi#tag/objects/head/objects/{className}/{id}), or use the following client code:
