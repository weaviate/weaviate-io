...
Configure individual properties in a collection. Each property can have it's own configuration. These are common options:
...
<details>
  <summary>
    Limitations when you add a property after importing data
  </summary>

Adding a new property after you import data limits searches that use the new property.<br/><br/>

Property indexes are built at import time. If you add a new property after importing some data, objects in the existing index aren't automatically updated to add the new property. Queries may return unexpected results because only new objects have the new property; the older objects are not re-indexed to add it.<br/><br/>

To index all objects, do one of the following:
- Add the property before importing objects.
- Delete the collection; re-create it with the new property; and re-import the data.
