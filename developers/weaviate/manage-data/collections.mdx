...
:::info Available from `v1.26.8` and `v1.27.1`
The `reranker` and `generative` configurations are mutable from `v1.26.8` and `v1.27.1`.
:::
...
Update a [`reranker`](../concepts/search/index.md#rerank) model integration for [reranking retrieved results](../search/rerank.md).
