...
Adding a new property after you import data limitation searches that use the new property.

<br/>

Property indexes are built at import time. If you add a new property after importing some data, the existing index isn't updated to add the new property. This means that the new property will not be indexed for existing objects. This can lead to unexpected behavior when querying.
