...
You can list (i.e. retrieve) every object as shown in the below example, looping through the data with the `after` operator.
...
Putting the pieces together, the below example will retrieve the `WineReview` class definition and objects from `https://some-endpoint.weaviate.network` and use them to populate `https://anon-endpoint.weaviate.network`:
