...
The `after` parameter is based on the order of IDs. No other ordering of data, such as sorting or searching is not possible.
:::
...
The below example will loop through all objects (and their `title` property) in the `WineReview` class from `https://some-endpoint.weaviate.network` using the ID of the last object retrieved as the cursor:
...
<FilteredTextBlock
  ...
  endMarker="# Finished retrieving data"
  language="py"
/>
</TabItem>
</Tabs>
...
And then restore to a new instance, by:
- Creating a new class using the fetched schema, and
- Batch-importing objects.

<Tabs groupId="languages">
<TabItem value="py" label="Python">
<FilteredTextBlock
  text={PythonCodeExample}
  startMarker="# Restore to a new instance"
  endMarker="# Finished restoring to a new instance"
  language="py"
/>
</TabItem>
</Tabs>
...
Putting the pieces together, the below example will retrieve all objects and the schema from the `WineReview` class from `https://some-endpoint.weaviate.network` and populate `http://localhost:8080` with the same:
