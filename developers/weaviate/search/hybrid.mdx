...
This page shows you how to perform keyword searches using Weaviate using the `hybrid` parameter. The `hybrid` parameter uses a combination of the `bm25` (i.e. sparse vector) and vector (i.e. dense vector) search algorithms.
...
It ranks them using the default parameters, and returns the top 3.
...
You can use the `alpha` argument to weight the keyword (`bm25`) or vector search results. An `alpha` of `1` is a pure vector search and `0` is a pure keyword search. The default is `0.75`.
