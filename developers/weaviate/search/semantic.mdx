...
If you want to search for an object that is close in vector space to a concept (or set of concepts), use the [`nearText` search parameter](../../api/graphql/vector-search-parameters#neartext). Weaviate will vectorize the concept(s) passed to `nearText` and will return results sorted ascending by distance. Since Weaviate does the vectorization, it will need the vectorizer API key (in this case OpenAI).
...
To require a maximum distance between the query and the result objects, use the `distance` variable with any `near...` parameter. The interpretation of the value of the distance field depends on the [distance metric](../../config-refs/distances) used. If the distance metric is `cosine`, then the [`certainty`](../../config-refs/distances#distance-vs-certainty) variable can be used instead of `distance`, for backwards compatibility.
