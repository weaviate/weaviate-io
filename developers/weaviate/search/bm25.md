...
The keywords in the BM25 query are logically `OR`ed, unless the [tokenization](../config-refs/schema.md#property-tokenization) setting of the selected property/properties is set to `field`. In other words, Weaviate will return any object that matched at least one of the keywords, unless the selected property/properties were tokenized as `field`.
