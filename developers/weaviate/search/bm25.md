...
To retrieve the sub-properties with one of the legacy clients, use the `_additional` property to specify `score`. The new Python client returns the information as metadata.
...
You can combine `autocut` can be combined with `limit: N` to limit the size of the autocut groups to the first `N` objects.
...
For example, consider a `bm25` that returns these distances `[2.676, 2.021, 2.022, 1.854, 1.856, 1.713]`.

- `autocut: 1` returns the first object.
- `autocut: 2` returns the first three objects.
- `autocut: 3` returns all objects.
