...
1. **Per-class configuration**
    ...
    * The `vectorIndexType` allows the choosing the vector index (defaults to [HNSW](/developers/weaviate/concepts/vector-index-plugins.md#hnsw))
    * The `vectorIndexConfig` is an arbitrary object passed to the index for config (defaults can be found [here](/developers/weaviate/configuration/vector-index-type.md#how-to-configure-hnsw) )
