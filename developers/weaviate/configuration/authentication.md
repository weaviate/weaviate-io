...
1. For very simple usecases such as just needing a single user, you can use the OIDC token issuer of the [weaviate cloud service](https://auth.wcs.api.weaviate.io). Sign up and use `https://auth.wcs.api.weaviate.io/auth/realms/SeMI` as the issuer and your sing-up email as user.
1. If you need a more advanced setup you can use commercial OIDC providers like [Okta](https://www.okta.com/).
1. For full control, you can run your own OIDC token issuer server which is the most complex solution. Popular open-source solutions include Java-based [Keycloak](https://www.keycloak.org/) and Golang-based [dex](https://github.com/dexidp/dex).
...
1. Use `resource owner password flow` for trusted applications such as the [weaviate cloud service](https://auth.wcs.api.weaviate.io). 
