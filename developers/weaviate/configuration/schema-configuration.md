...
:::note
Check out the [schema section of the quickstart tutorial](/developers/weaviate/quickstart/schema.md) to get started with an example.
:::
...
The vectorIndexType defaults to [`hnsw`](/developers/weaviate/concepts/vector-index-plugins.md#hnsw) since this is the only available vector indexing algorithm implemented at the moment.

#### vectorIndexConfig

Check the [`hnsw` page](/developers/weaviate/configuration/vector-index-type.md#how-to-configure-hnsw) for `hnsw` parameters that you can configure. This includes setting the distance metric to be used with Weaviate.
