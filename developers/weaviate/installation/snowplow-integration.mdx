...
Load the logs from the jupyter endpoint to get a Jupyter key.

```sql
CALL SYSTEM$GET_SERVICE_LOGS('WEAVIATE_PRODUCT_REVIEWS.PUBLIC.JUPYTER', '0', 'jupyter');
```

Get the endpoint that you use to access Jupyter Notebooks.
...
Load your data into Weaviate. To use a Jupyter Notebook, use the `weaviate_user` credentials to log into the Jupyter endpoint.
