...
The current JavaScript client version is `v||site.typescript_client_version||`.
:::
...
Once the client has been installed, you can use the client in your TypeScript scripts, such as in the following example.

:::tip Note type specifications
Note below that objects are explicitly typed - `client` is a `WeaviateClient` and `err` is an `Error`.
:::
...
The type definitions can be found under the `types` subdirectory of the package in the `*.d.ts` files, for example as shown on the [npm page](https://www.npmjs.com/package/weaviate-ts-client?activeTab=code).
...
Our [RESTful endpoints](../api/rest/index.md) and [GraphQL functions](../api/graphql/index.md) covered by the TypeScript client will soon have JavaScript examples in the code blocks.
