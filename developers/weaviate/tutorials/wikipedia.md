...
- Your preferred Weaviate [client library](../client-libraries) installed.
...
We can create a Weaviate instance locally using Docker (fastest import and search), or in the cloud using the Weaviate Cloud Services (easiest setup, but importing may be slower due to the network speed). Both options are explained in the [Installation page of the Quickstart tutorial](../quickstart/installation.md).
...
We're now ready to import the articles. For maximum performance, we'll load the articles into Weaviate via [batch import](../api/rest/batch).
