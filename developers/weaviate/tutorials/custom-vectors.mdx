...
In this tutorial, we'll guide you through the process of using Weaviate with your own vectors.

If you choose to specify your own vector embeddings, performing a vector search with Weaviate looks like this:
...
Weaviate is open source, and you can easily [run the binary](../installation/embedded.md) or [run a local instance from a Docker image](../installation/docker-compose.md), or even easier - you can create a free sandbox instance with the [Weaviate Cloud Services](../../wcs/quickstart.mdx) (WCS).
...
Weaviate lets you define vectorizers at the class level, with further control (e.g. specific model, or vectorization behavior) at the property level. Since you're bringing your own vectors, a vectorizer is not needed, so we will set `vectorizer: "none"` in the class here.
...
In this example, we've used the OpenAI API to generate it, with the same [`text-embedding-ada-002` model](https://platform.openai.com/docs/guides/embeddings/what-are-embeddings). Then, in the following query, we pass that vector to the [`nearVector` parameter](../api/graphql/vector-search-parameters.md#nearvector):
