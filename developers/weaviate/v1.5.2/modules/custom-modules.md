...
The Go API interfaces are documented in the code as comments ([example](https://github.com/semi-technologies/weaviate/blob/master/entities/modulecapabilities/config.go)). All interfaces that make up a module can be found in [this package](https://github.com/semi-technologies/weaviate/tree/master/entities/modulecapabilities). A (new) [module](https://github.com/semi-technologies/weaviate/tree/master/modules) does not need to implement all these interfaces. For each interface, you can make the decision whether you want to provide that capability in your module or not. For each capability Weaviate only considers the modules that do implement it.

A good starting point would be to look at the [`text2vec-transformers` module](https://github.com/semi-technologies/weaviate/tree/master/modules/text2vec-transformers) (which is much simpler than the `text2vec-contextionary`). If you want to create a module really similar to this module, but you, for example, don't want the module to provide any GraphQL `_additional` properties, you can skip this interface entirely.
