...
First of all, memory correlates with your dataset size, CPU correlates with your import speed and throughput requirements.

Secondly, as a rule of thumb, you need two times the size of your vectors. This overhead accounts for the HNSW index structure, page caches and overhead during importing, etc. Example: 10M objects with contextionary vectors (300d of float32): 1e7 * 300 * 4 = 12GB of vectors -> 24 GB of total memory. CPUs control both the import speed and the Queries per second. For faster imports or higher throughput at query time, you need to add more. A rule is to use 1 CPU per 4 GB of memory. For example, for the above machine with 24GB memory, a starting point would be 6 CPUs. Deviate up or down based on import speed and throughput requirements.
...
### Q: Schema best practices: if I was looking to perform a semantic search over a the content of a Book would I look to have Chapter and Paragraph represented in the schema etc, would this be preferred over including the entire content of the novel in a single property?
