...
You can use the same approach as for `text2vec-transformers`, see `text2vec-transformers`, i.e. either pick one of the pre-built containers or build your own container from your own model using the `semitechnologies/qna-transformers:custom` base image. Make sure that your model is compatible with Huggingfaces' `transformers.AutoModelForQuestionAnswering`.
