---
title: Generative Search - Ollama
sidebar_position: 30
image: og/docs/modules/generative-ollama.jpg
# tags: ['generative', 'rag', 'ollama']
---
...
`generative-ollama` performs retrieval augmented generation, or RAG, based on the data stored in your Weaviate instance.

The module works in two steps:
1. (Weaviate) Run a search query in Weaviate to find relevant objects.
2. (Ollama) Use a Large Language Model to generate a response based on the results (from the previous step) and the provided prompt or task.
...
To use `generative-ollama`, you must enable it in your Docker Compose file (`docker-compose.yml`). You can do so manually, or create one using the [Weaviate configuration tool](/developers/weaviate/installation/docker-compose.md#configurator).

#### Parameters

- `ENABLE_MODULES` (Required): The modules to enable. Include `generative-ollama` to enable the module.
...
services:
  weaviate:
    ...
    environment:
      ...
      # highlight-start
      ENABLE_MODULES: 'text2vec-ollama,generative-ollama'  # Can be any `text2vec` module.
      # highlight-end
...
The following example configures the `Document` collection to use the `generative-ollama` module, and specifies the Ollama endpoint using `host.docker.internal`
...
import LocalSingleResult from '/_includes/code/generative.local.singleresult.mdx';
...
import LocalGroupedResult from '/_includes/code/generative.local.groupedresult.mdx';
