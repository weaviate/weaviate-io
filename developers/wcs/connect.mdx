...
- [Connect a browser to WCD through the WCD Console.](/developers/wcs/connect#connect-with-a-browser)
...
- [Connect a client application to WCD through APIs.](/developers/wcs/connect#connect-with-an-api)

   - For applications, use the WCD API and one of the [client libraries](/developers/weaviate/client-libraries) to connect to WCD.
...
WCD restarts the cluster when you add a new API key. If you have a stand-alone cluster, you will have a short downtime while the cluster restarts. There is no downtime if you have a high availability cluster.
