...
The new Python and TypeScript clients use the gRPC protocol to connect to WCS. The protocol is very sensitive to network speeds. If you run into timeout errors, increase the connection timeout value or set `skip_init_checks=False` in your connection code.
