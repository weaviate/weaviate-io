...
Browser authentication relies on a username and password. Client authentication uses API keys. Enable two-factor authentication to increase the security of browser logins.  

## Use a browser to connect to WCS

The WCS console is a multi-purpose tool. Login to manage your clusters, users, and billing. To work with your data, use the [GraphQL console](/developers/wcs/guides/console). The console is great for ad hoc queries. If you develop applications, or want additional flexibility, use the Weaviate API and one of the [client libraries](/developers/weaviate/client-libraries) to connect instead.

Follow these steps to connect with a browser.
...
To enable multi-factor authentication (MFA), follow these steps.
...
If you want to disable MFA, [contact us](mailto:support@weaviate.io).
