...
Weaviate stores objects in collections. Collection [schemas](/developers/weaviate/config-refs/schema) are highly configurable, but some properties cannot be changed after you create the collection. For best results, define a collection schema before you import your data. Alternatively, let the [auto-schema](/developers/weaviate/config-refs/schema#auto-schema) feature infer a schema based on your data. 

To create a schema for the `Question` collection, run this code:
