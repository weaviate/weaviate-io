...
Weaviate stores objects in collections. Every collection has a [schema](/developers/weaviate/config-refs/schema). Collection schemas are highly configurable, but some properties cannot be changed after you create the collection.

For best results, define a collection schema before you import your data. Alternatively, let the [auto-schema](/developers/weaviate/config-refs/schema#auto-schema) feature infer a collection schema based on your data. 
...
This example code downloads a sample data set and uploads it to your WCS instance.
...
- If you want to try Weaviate, continue with a sandbox cluster.
