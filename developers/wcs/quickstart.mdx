...
1. Create environment variables, or edit the sample code, to pass the URL and API key to your client.
...
Weaviate stores objects in collections. Every collection has a [schema](/developers/weaviate/config-refs/schema). Collection schema are highly configurable, but some properties cannot be changed after you create the collection.

For best results, define a collection schema before you import your data. Alternatively, let the [auto-schema](/developers/weaviate/config-refs/schema#auto-schema) feature infer a schema based on your data. 

To create a schema for the `Question` collection, follow these steps:
...
Weaviate uses [batch](/developers/weaviate/manage-data/import) processing to import data efficiently.

This example code downloads a small, sample data set and uploads it to your WCS instance.
