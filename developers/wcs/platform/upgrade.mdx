...
The WCS console shows an "Update!" button on the details panel when a newer version is available. 
...
## Upgrade the server version
 ...
 WCS restarts your cluster during an upgrade. To avoid downtime, consider using high availability (HA) clusters for production workloads. A standard cluster is unavailable during upgrade. An HA cluster upgrades one node at a time so the cluster is available during the upgrade.
