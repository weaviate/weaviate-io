...
2. `operator` is set to define what you want to do with the value inside the path (e.g., `Equal` or `GreaterThan`, etc. See the list [here](/developers/docs/api/graphql/filters.md#filter-structure)).
...
Last but not least, all the standard filters are documented in the [filters section](/developers/docs/api/graphql/filters.md) of the GraphQL references documentation or in the documentation of the individual modules.
...
You can find detailed documentation on the `Aggregate{}` function [here](/developers/docs/api/graphql/aggregate.md).
