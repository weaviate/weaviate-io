...
*Note: The following steps are only required if you want to manually add the module to an existing setup. If you are starting from scratch, a much more convenient option is to use our [configuration and customization tool](/developers/docs/installation/docker-compose.md#configurator).*
...
Choose [any of our pre-built CLIP models](/developers/docs/installation/docker-compose.md#configurator) (for building your own model container, see below) and spin it up with your setup. Use a CUDA-enabled machine for optimal performance.
...
depends on the [distance metric used](/developers/docs/configuration/distances.md).
