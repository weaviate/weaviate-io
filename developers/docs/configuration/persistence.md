...
For Kubernetes setup, the only thing to bear in mind is that Weaviate needs a `PersistentVolumes` through `PersistentVolumeClaims` ([more info](/developers/docs/quickstart/installation.md#requirements)) but the Helm chart is already configured to store the data on an external volume.
