...
:::note
Check out the [schema section of the quickstart tutorial](/developers/docs/quickstart/schema.md) to get started with an example.
:::
...
The vectorizer (`"vectorizer": "..."`) can be specified per class in the schema object. Check the [modules page](/developers/docs/modules/index.md) for available vectorizer modules.
...
With the [`text2vec-contextionary`](/developers/docs/modules/retriever-vectorizer-modules/text2vec-contextionary.md) vectorizer module you can specify whether class names, property names or entire properties are included in the calculation of the data object's vector. Read [here](/developers/docs/configuration/schema-configuration.md#regulate-semantic-indexing) how this works.

#### vectorIndexType

The vectorIndexType defaults to [`hnsw`](/developers/docs/concepts/vector-index-plugins.md#hnsw) since this is the only available vector indexing algorithm implemented at the moment.

#### vectorIndexConfig

Check the [`hnsw` page](/developers/docs/configuration/vector-index-type.md#how-to-configure-hnsw) for `hnsw` parameters that you can configure. This includes setting the distance metric to be used with Weaviate.

#### shardingConfig (introduced in v1.8.0)

The `"shardingConfig"` controls how a class should be [sharded and distributed across multiple nodes](/developers/docs/concepts/cluster.md). All values are optional and default to the following settings:
