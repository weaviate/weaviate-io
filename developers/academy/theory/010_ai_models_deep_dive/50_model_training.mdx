...
We also learned that the `weights` and `biases` are collectively called `parameters` of a deep learning model. As the name indicates, `parameters` define the behavior of a deep learning model (in contrast to an architecture of a deep learning model).
...
Two deep learning models, even with the same architecture, can have different parameters to perform a different job to each other.
...
In a majority of cases, users will be selecting models off-the-shelf, although fine-tuning a model can be a good solution for the right use case, as long as it is done with the right expertise and goal in mind. We will discuss this more in our later section on model evaluation and selection.
