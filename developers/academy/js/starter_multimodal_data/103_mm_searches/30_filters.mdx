---
title: Filters
description: Filters for Multimodal Searches
---
...
This query is identical to [that shown earlier](./10_multimodal.mdx) for search, but with the addition of a filter. The `filters` parameter makes use of the `filter` namespace to set the filter conditions. The current query filters the results to only include those with a release year after 2010.
