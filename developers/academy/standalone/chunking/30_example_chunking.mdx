...
In the preceding sections, you've seen [what chunking is, and why it's important](./10_what_and_why.mdx). You've also seen some [common chunking methods, and how they work](./20_how.mdx).

In this section, we'll look at a practical example of chunking in action. We'll ingest multiple sections of a book using different chunking methods, and then compare the results.

We will do this by inspecting individual chunks, as well as by comparing results of various searches.
...
This process yields 14 text objects, each containing a chapter title, filename, and the text of the section.
...
Now we can try out various chunking methods on each section. We'll use the following chunking methods:
...
This way, each section will be chunked into smaller chunks. We'll re-construct text objects around chunks. Each chunk will now also contain additional metadata in the chunk number.
...
We can now import the chunks into Weaviate. To allow for easy comparisons, we'll import each set of chunks into the same class, while adding a `chunking_method` property to each chunk for easy filtering.
