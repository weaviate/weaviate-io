...
The search is carried out as follows, looping through each chunking strategy by filtering our dataset. We'll obtain just a couple of top results for each search term.
...
In this example, the query is a broad and general one on the `history of git`.

As such, the longer chunks seem to suit the purpose better, with the paragraph chunks containing the most amount of context.

And while the 25-word chunks may be semantically similar to the history of git, it does not contain enough contextual information to enhance the readers' understanding of the topic.
...
To be clear and fair, the other result sets also contain this information. But it may be worth considering how the result may be used and displayed - the longer the result, the more cognitive effort it may take the user to identify the relevant information.
