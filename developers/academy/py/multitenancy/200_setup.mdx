...
You may have seen Docker configurations elsewhere ([e.g. Docs](../../../weaviate/installation/docker-compose.md), [Academy](../starter_text_data/101_setup_weaviate/20_create_instance/20_create_docker.mdx)). But the highlighted configurations may be new to you:
...
- `ENABLE_MODULES`: We include `offload-s3` to enable tenant offloading to an S3 bucket. This will help us manage inactive users' data efficiently.
...
### <i class="fa-solid fa-code"></i> Create the collection
...
Next, we will learn how to manage tenants and tenant data.
