...
The distance metric used in HNSW determines how the distance between vectors is calculated. In turn, it impacts where each vector is placed in the graph.
...
### <i class="fa-solid fa-code"></i> Specify HNSW in Weaviate
...
To set the dynamic list size for index construction, provide the `constructionEf` parameter when creating the collection.
