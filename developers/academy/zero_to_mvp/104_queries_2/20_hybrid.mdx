...
A hybrid search works by combining the results of a `bm25` search with the results of a vector search. More specifically, it uses a combination of each results's BM25F search ranking and its vector search ranking among the set of results.
