...
Your external IP may not always be `127.0.0.1`, and by default, the Weaviate pod listens for TCP traffics on the `localhost` or `127.0.0.1`. To make the pod reachable, expose the Weaviate services using port-forwarding.
...
Execute each command on a new terminal. Note that if the terminal is closed or the process is stopped, the forwarded ports
will be closed, hence making the pod inaccessible.
