...
<ReactPlayer url='https://youtu.be/gN8hfwqJNM0' controls='true'/>
<br/>
...
When you query for data, you can use the `offset` parameter to skip a number of results. This is useful for pagination, where you want to show a certain number of results per page.

The `offset` parameter works in conjunction with the existing `limit` parameter to shows results from the `offset+1` to `offset+1+limit`.
...
This query retrieves the 2 results (`limit`: 2) after the first 2 results (`offset`: 2).

We can confirm this by comparing the results to the below query, which retrieves the top 4 results. The last two results from the below query are the same as the two results from the above query.
...
The `offset` parameter is available with all vector search functions including `Get` and `Aggregate`.
...
- The `offset` parameter can be used in conjunction with `limit` to skip results and build pagination.
...
const offsetExample = [{
  questionText: 'How can you combine the offset and limit parameters to display the second page of results with 10 results per page?',
  answerOptions: [
    {
