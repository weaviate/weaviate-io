...
For example, let's say we are still interested in a similar input text as before, but instead of individual queries, we would like to know how many questions might be related to this query.
...
The current schema includes a property (`answer`) of the `text` data type, and a property (`value`) of the `int` data type.
...
And as you might imagine, these properties that can be queried will vary according to the data type that is being aggregated.
