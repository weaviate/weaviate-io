...
This query aggregates the objects in the `JeopardyQuestion` class to obtain the total count. Since there are no restrictions, it returns the total number of retrievable objects which is 10,000. 
...
{
  Aggregate {
    JeopardyQuestion (groupBy: ["value"]) {
      groupedBy {
        value
      }
      meta {
...
This query supplies an additional `groupedBy` argument, as a result of which the counts are of each `value`. The query also requests `groupedBy` a property (of `value`) so that each count is identifiable by the `value`.
...
For example, let's say we are still interested in a similar concept, but instead of individual queries, we would like to know how many questions might be related to this query.
...
This query introduces the `distance` argument. This argument is a threshold that restricts the returned results to those that are relevant to the input. Without it, the search results would potentially include the entire class.
...
The sub-properties under `answer` in the query requests additional aggregations here, resulting in the top occurring answers being returned as well as their counts.
