...
Do note that, however, names in GraphQL [are case-sensitive](https://spec.graphql.org/October2021/#sec-Names), so `Get` and `JeopardyQuestion` are different from `get` and `jeopardyquestion`.
:::
