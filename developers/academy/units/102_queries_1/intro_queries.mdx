...
We will primarily be using the **Weaviate Python client** in this module to perform queries. But we will discuss a little bit of GraphQL too, as it forms the basis for those Python queries.

### <i class="fa-solid fa-chalkboard"></i>&nbsp;&nbsp;Uses in Weaviate

Weaviate provides both REST and GraphQL API interfaces. They work together to provide a rich API for its users.
...
So keep these terms in mind when they appear in explanations.

### <i class="fa-solid fa-chalkboard"></i>&nbsp;&nbsp;GraphQL & Python

We will be using the Python client in this unit to perform queries. So, you will not need to directly enter GraphQL. 

So when we show a query such as this:
...
To help learn the underlying query structures, we show raw GraphQL queries in this unit, as well as the Python code.
