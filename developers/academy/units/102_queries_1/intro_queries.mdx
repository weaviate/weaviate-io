...
Weaviate provides REST and GraphQL API interfaces for communication. This section will explain the differences between the two and how they are used in Weaviate.
...
You might have seen this function earlier. The `Get` function is used in Weaviate to retrieve data objects. 

For many use cases, `Get` will be the most common type of query function used in Weaviate.
...
For example, the following query will produce the maximum, minimum and mean of the `value` property in the `Question` class.
...
Arguments are used in Weaviate to provide additional search parameters and filters. It can be used to:
