...
Follow the "Get Started with WCS" [section on this page](../hello_weaviate/running_weaviate#get-started-with-wcs) and come back.
...
```python
import wv_datasets
dataset = wv_datasets.JeopardyQuestions10k()
dataset.upload_dataset(client, batch_size=200)
```

This will upload 10,000 objects from the quiz show *Jeopardy!* to your Weaviate instance. This should take about ~5 minutes, depending on your set-up, so we recommend grabbing a coffee or tea. 

This dataset **includes vectors**, so you will not incur API costs.

:::tip Too slow? Try the smaller dataset.
If you want something faster, you can run:
```
dataset = wv_datasets.JeopardyQuestions1k()
```
:::
