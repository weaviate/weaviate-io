...
<ReactPlayer url='https://youtu.be/BDmCotY2b5U' controls='true'/>
<br/>
...
Your query should return something like the below:
...
## <i class="fa-solid fa-square-chevron-right"></i>&nbsp;&nbsp;`Aggregate` with a vector search parameter

As we did with `Get` queries, we can also use vector search parameters such as `nearText` in an `Aggregate` query. Take a look:
...
- Vector search parameters, like `nearText`, can be used in `Aggregate` queries.
