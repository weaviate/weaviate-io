...
import PythonCodeExample from '!!raw-loader!./snippets/20_schema.py';
...
In this section, you will learn how to define a schema, as well as some key considerations while doing so.
...
To define a class, you can use the below syntax.
...
If you ran the first class creation command shown, and this command, Weaviate will throw an error as the class `Article` already exists. For the purposes of this section, delete the class by running the below command.<p><br/></p>
...
For text objects, you must select one of the `text2vec` modules such as `text2vec-cohere`, `text2vec-huggingface`, `text2vec-openai` which is enabled at the Weaviate instance.

You can see the list of available modules by running the below command.
...
WCS instances come pre-configured with a number of modules. For example, the below response shows that the `text2vec-openai` module is available, so we can use it in our schema.
...
You can also set the `moduleConfig` parameter at the property level to set module behavior for each property. For example, you could set whether to vectorizer the property name (`vectorizePropertyName`), or whether to skip the property from vectorization altogether (`skip`).

The below example includes a `skip` parameter to `True` for the `url` property, so that the URL text will be skipped when producing a vector embedding for the object.
