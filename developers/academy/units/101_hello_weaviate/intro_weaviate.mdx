...
You may be familiar with traditional databases such as relational databases SQL. A database can catalog, store and retrieve information. A **vector** database can carry out these tasks also, with the key difference being that they can perform these tasks based on similarity.
...
Which would return books that contained any of these strings (`major`, `important`, `prominent`, ... etc) in the `wiki_summary` column.
...
What's more, this doesn't solve the problem of how to rank the list of resulting books.
