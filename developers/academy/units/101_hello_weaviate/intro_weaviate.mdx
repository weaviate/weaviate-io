...
Weaviate is an open-source vector search engine. But what does that mean? Let's unpack it here.

### <i class="fa-solid fa-chalkboard"></i>&nbsp;&nbsp;Vector search engine
...
A vector search engine is a type of search engine that retrieves information based on similarity. This is different to traditional search methods, for example using SQL.
...
A vector search engine makes this job simpler by enabling searches based on similarity. 
...
For now, keep in mind that Weaviate is a vector search engine at its core which can also leverage AI tools to do more with the retrieved information.
...
Vector searches can be very computationally intensive. To overcome this problem, Weaviate uses a combination of indexes that allows it to perform extremely fast vector searches, as well as to filter data using Boolean criteria on data types such as strings and numbers.

Again - this is a complex topic, but for now, it's enough to know that Weaviate leverages multiple types of data indexes.
