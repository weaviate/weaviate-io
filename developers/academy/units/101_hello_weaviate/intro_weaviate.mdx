...
A vector search engine is a type of search engine that retrieves information based on similarity. 

Imagine that you are looking in a library, looking for books related to birds. Maybe you are a bird-watcher, or perhaps you're an ornithologist (bird scientist). If the library was catalogued using a traditional search system, you might construct a query like this: 
...
Which would return books that contained any of these strings (`bird`, `avian`, `ornithology`, ... etc) in the `title`. Note how we have to identify any and all terms that *might* have been used in relation to the topic. Otherwise, the search would miss any entities that didn't include any of these terms.
...
For example, you can extract answers to questions using Weaviate. Given a list of Wikipedia entries, you could ask Weaviate: 
