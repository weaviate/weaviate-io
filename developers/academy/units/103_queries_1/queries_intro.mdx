...
TODO - refactor the below (cut from weaviate hands on) to this page
...
The query format begins with a `Function`, such as `Get` (to retrieve objects) and `Aggregate` (to retrieve metadata), as well as a `Class` denoting the data object collection. Notably, it then goes on to specify a set of `properties` which determine the fields to be returned. 
...
### Retrieve objects with GraphQL

![](../../tmp_images/a_robot_arm_picks_up_a_word_from_a_pile_of_words_1.png)
