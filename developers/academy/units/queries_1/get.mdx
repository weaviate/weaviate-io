...
:::info `... on Category` syntax
The `... on Category` pattern syntax is called a "fragment" in GraphQL terms. This fragment specifies that the `hasCategory` property is of type `Category`, and that the results should be returned as such.
:::
