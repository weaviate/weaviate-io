...
2. `operator` is set to define what you want to do with the value inside the path (e.g., `Equal` or `GreaterThan`, etc. See the list [here](/docs/weaviate/api/graphql/filters.md#filter-structure)).
...
Last but not least, all the standard filters are documented in the [filters section](/docs/weaviate/api/graphql/filters.md) of the GraphQL references documentation or in the documentation of the individual modules.
...
You can find detailed documentation on the `Aggregate{}` function [here](/docs/weaviate/api/graphql/aggregate.md).
