...
export default function BlogPostItems({items}) {
  ...
  return (
    <div className="tw-grid tw-grid-cols-1  sm:tw-grid-cols-2 lg:tw-grid-cols-3">
      {items.map(({content}) => {
        const {metadata, assets} = content;
        return (
          <article className="tw-min-w-0 tw-border-[0.1px]  tw-border-solid tw-border-[#6E6B91] tw-bg-[#1A1A1A] tw-p-[26px]" key={metadata.permalink}>
            ...
            <div className="tw-mb-[14px] tw-flex tw-min-h-6 tw-flex-wrap tw-gap-1.5">
              {metadata.tags.slice(0, 3).map((tag) => (
                <Link className="tw-rounded-lg tw-border-[0.2px] tw-border-solid tw-border-[#6E6B91] tw-bg-transparent tw-px-[7px] tw-pt-[0.4rem] tw-pb-1 tw-text-[0.62rem] tw-capitalize tw-leading-none tw-text-[#d0d8ea] hover:tw-border-[#63e689] hover:tw-bg-[#63e689]/[0.06] hover:tw-text-[#edf4ff] hover:tw-no-underline" key={tag.permalink} to={tag.permalink}>{tag.label}</Link>
              ))}
            </div>
