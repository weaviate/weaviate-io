...
Semantic searches (or question answering) are essentially searches by similarity, such as by the meaning of text, or by what objects are contained in images. For example, consider a search for “science fiction”. A “good” attempt at meaning-based information retrieval should find documents related to the concept of “future” as being closer to the query than those relating to “data science”. 

Because we have an understanding of language and meaning, we know this to be true even though documents titled “Data science” contain the word “science” from the query, while those titled “The future of space travel” do not. But how can computers mimic our understanding of language, and similarities of words or paragraphs? To tackle this problem, semantic search uses at its core a data structure called **vector embedding** (or simply, **vector** or **embedding**) - an array of numbers. A [vector search engine](/blog/2022/12/vector-library-vs-vector-database.html) like Weaviate will compute an embedding for each data object as it is inserted or updated into the database. The embeddings are placed into an index, so that the search engine can [quickly](/blog/2022/09/Why-is-Vector-Search-so-fast.html) find the [closest](/blog/2022/09/Distance-Metrics-in-Vector-Search.html) vectors to a given vector that represents the query.

## What exactly are vector embeddings?
Vectors are numeric representations of data that capture certain features of the data. For example, in the case of text data, “cat” and “kitty” have similar meaning, even though the _words_ “cat” and “kitty” are very different if compared letter by letter. In other words, it would be very difficult to make enough typographical errors to go from one to the other. But for semantic search to work effectively, representations of “cat” and “kitty” must sufficiently capture their semantic similarity. This is where vector representations are used, and why their derivation is so important.
...
Vectorization of text objects begins with tokenization: the text is split into tokens such as words, lowercased, stop words (“the”, “a” etc.) (and often punctuation) are removed, and rare words are replaced with placeholders. The resulting tokens are then encoded using one of the methods mentioned above. Finally, the encodings are combined into a sentence-level encoding, e.g. by averaging the values for all words across each dimension.
...
* word vectors can be manipulated to derive analogies (e.g. “king is to man as is queen is to …” as you saw earlier), and
...
* It doesn’t address words with multiple meanings (polysemantic): `run`, `set`, `go`, `take` each have [over 300 meanings](https://www.insider.com/words-with-the-most-definitions-2019-1) (!)
