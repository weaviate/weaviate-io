...
> Note: The vectorization is done by [Weaviate “core”](https://github.com/semi-technologies/weaviate) and not at the client level. So even though we use Python examples, the principles are universally applicable.
...
As each object only contains the two properties, `question` and `answer`, let’s try concatenating our text and compare it to Weaviate’s. And instead of the `text2vec-cohere` module, we will go straight to the Cohere API. 
...
Unless specified otherwise, Weaviate follows the following logic in order to produce a string to be vectorized:

- Only vectorizes properties that use `string` or `text` data types 
...
> You can read more about each variable in the [schema configuration documentation](/developers/weaviate/current/schema/schema-configuration.html).
...
The schema is defined such that at least some of the options, such as `moduleConfig`/`text2vec-cohere` /`vectorizeClassName` and `properties`/`moduleConfig`/`text2vec-cohere`/`vectorizePropertyName` differ to their defaults. 
