...
The models that can both compute **dense vector embeddings for data** and for **search queries** are so-called *Bi-Encoder models*. *Bi-Encoder models* compute vectors for data entries "independently", meaning that each data item (or search queries) will be *vectorized*. Data vectors and query vectors can be compared by computing the similarity (for example cosine similarity) between the vectors. This means that data objects can be compared to each other, or data objects and search queries can be compared. All data items in the database can be represented by a vector embedding using a Bi-Encoder model before query time. During query time, you can use an [ANN algorithm](https://weaviate.io/developers/weaviate/current/vector-index-plugins/index.html){:target="_blank"} to retrieve similar vectors with their data objects, and you don't have to compare each data object with a query or every data object in the dataset. This makes a Bi-Encoder highly efficient.
