...
### Learn more
For more info, check out [the documentation](/developers/weaviate/references/distances).
...
### Background
One of Weaviate's features is full CRUD support. CRUD operations enable the mutability of data objects and their vectors, which is a key difference between a vector database and an ANN library. In Weaviate, every data object has an ID (UUID). This ID is stored with the data object in a key-value store. IDs don't have to be globally unique, because in Weaviate [classes](/developers/weaviate/references/schema-configuration#class-object) act as namespaces. While each class has a different [HNSW index](/developers/weaviate/vectorization/hnsw), including the store around it, which is isolated on disk.

There was however one point in the API where reusing IDs between classes was causing serious issues. Most noticeable this was for the [v1/objects/{id}](/developers/weaviate/references/rest/objects) REST endpoints.
