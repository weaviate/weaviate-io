...
The `async vector indexing` feature must be enabled in the Weaviate configuration file, and the gRPC API is currently only supported through the new `v4` Python client (which is in beta).
...
The upcoming `v4` Python client supports gRPC, which will allow you to take advantage of these performance improvements.
...
This client marks the first beta release of the `v4` client, with not only gRPC support but also an improved API. The new API is more Pythonic, with a focus on improving the user/developer experience through custom classes and methods, which will trigger IDE auto-completion in place of untyped dictionaries.
...
It can be as small as a typo, or as big as a missing feature, or anything in between. It can also be a positive comment, of course! We really look forward to hearing from you.
