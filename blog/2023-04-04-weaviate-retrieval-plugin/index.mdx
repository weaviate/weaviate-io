---
title: The ChatGPT Retrieval Plugin - Giving Generative AI Long-term Memory using Weaviate
...
So what are ChatGPT plugins? Some are explaining plugins as the “eyes and ears” of LLMs to help products like ChatGPT interact more naturally with users by knowing of and integrating with other apps such as Zoom, Uber Eats, Duolingo, and many more. I would argue that plugins are much more than just the “eyes and ears” of ChatGPT.  Hear me out.
...
To demonstrate this in action consider the example below. This is what would happen if you used ChatGPT without the Weaviate Retrieval Plugin.

![pic1](./img/pic1.png)

As expected the model does not have any specific information with which to answer the question! Now lets say we enable the Weaviate Retrieval Plugin as shown below:

![pic2](./img/pic2.png)

The example below demonstrate the plugin in action. Notice how the prompt: `What is my cat’s name?` is used to create the query: `cat’s name` which is then sent to the vector database to conduct a search over the documents.
...
- You can persist conversations with ChatGPT in the attached vector database to carry on conversations between turning on and shutting off ChatGPT

The repercussions are that we can now schedule small and regular updates to content stored in connected vector databases to give the model awareness of new information and reading the need to retrain the LLM model and update frequently - this ensures the LLM provides robust and up to date custom answers even in-between time consuming retraining weight updates.
...
Another great application that the Weaviate Retrieval Plugin enables is personalization of ChatGPT around your personal details.
...
Another way you could personalize ChatGPT to you is by giving it an understanding of who you are. For example below I tell ChatGPT that languages I understand and customize it's responses around it's knowledge of me.
...
For the last example I'll share what I think is one of the most exciting and scary applications of the Weaviate Retrieval Plugin: to use it to persist the memory of previous converstions you have with ChatGPT into Weaviate so that it can refer to them in a later converstion. Currently it is not possible to tell ChatGPT to persist something in the attached vector database through the chat UI, so I have replicated this by manually adding the chat content to Weaviate in between conversations.
...
Voilà! ChatGPT now remembers it's previous conversations with you - persisting them in the attached instance of Weaviate!
