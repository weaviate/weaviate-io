...
:::caution Outdated integration mechanism
Published 2023-09-26. The Weaviate and Confluent partnership described here still ships, but the mechanism shown below does not. This post streams data through a Spark cluster loaded with the `confluent-connector` JAR. That approach has been superseded by the certified Weaviate Sink Kafka Connector, which runs on Kafka Connect with no Spark cluster and no custom JAR. The Python snippets also use the removed `v3` client API. For a current setup, read [Introducing the New Weaviate Confluent Apache Kafka Connector](/blog/weaviate-apache-kafka-connector).
:::
