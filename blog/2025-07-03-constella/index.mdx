...
:::note Editor's note
This post is Constella's own account, and the code is theirs. We have corrected two things since publication. The tenant name goes in the object body as `"tenant"`, not in an `X-Weaviate-Tenant` request header, which Weaviate does not read. And the `string[]` data type in the schema below is now `text[]`, because `string` was removed from Weaviate. The Python examples use the v3 Python client that Constella was running at the time; for new code, see the [v4 client documentation](https://docs.weaviate.io/weaviate/client-libraries/python).
:::
...
curl -X POST \
  ...
  -d '{"class": "Note", "tenant": "user-123", "properties": {"title": "Note 1", "content": "AI + Mindmaps"}}' \
...
results = (
    client.query.get("Note", ["title", "fileLink"])
