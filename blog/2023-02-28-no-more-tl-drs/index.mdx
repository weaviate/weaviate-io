...
Our summarizer module (`sum-transformers`) can be added to a Weaviate instance to do exactly that, and we would like to show you how to use both in this post. 
...
### Summarization - with `generative-openai`
...
So, this module can also be used to generate summaries by prompting it directly to do so. The below is one such example:
...
All transformer models have a maximum input length size. For example, `bart-large-cnn` has a maximum limit of 1024 tokens, where each token is a part of a word (i.e. a few characters). 
