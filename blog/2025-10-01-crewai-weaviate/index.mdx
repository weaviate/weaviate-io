...
weaviate_collection = client.collections.use("WeaviateBlogChunk")
```
...
We're also currently working on adding the [`WeaviateStorage` as an `ExternalMemory`](https://github.com/crewAIInc/crewAI/pull/3124) to CrewAI. It uses Weaviate's multi-tenancy to store the memory per agent for data isolation and scale. This will help Agents continuously learn across various tasks and interactions.
