...
I'm here to tell you that it doesn't have to be that way. In this article, I'll show you how to make testing easier with [Embedded Weaviate](https://weaviate.io/developers/weaviate/installation/embedded), and other tips for better automated testing.
...
[Embedded Weaviate](https://weaviate.io/developers/weaviate/installation/embedded) makes one part of this puzzle much easier, since Weaviate can be instantiated directly from the client. The following is all you need to do to start a Weaviate server:
...
* **Whether to test search quality**: This depends primarily on the model used for vectorization, such as by a [Weaviate vectorizer module](https://weaviate.io/developers/weaviate/model-providers). We suggest evaluating models separately, but not tested as a part of the application.
...
* **Other common issues to test** include:
  ...
  * Specifying the vector correctly when [bringing your own vectors](https://weaviate.io/developers/weaviate/starter-guides/custom-vectors)
...
[Embedded Weaviate](https://weaviate.io/developers/weaviate/installation/embedded) lets us spawn a Weaviate server instance from the client, and automatically tear it down when the client terminates. The data is [persisted](https://weaviate.io/developers/weaviate/installation/embedded#lifecycle) between sessions, so we recommend deleting your data before each test.
...
We'll create a class for question & answer objects from the game show *Jeopardy!*, by specifying its name and the vectorizer ([`text2vec-openai`](https://weaviate.io/developers/weaviate/model-providers/openai/embeddings)).
...
In this section, we'll import a small subset (100 objects) of the [original Jeopardy dataset](https://www.kaggle.com/datasets/tunguz/200000-jeopardy-questions). As always, we'll use [batching](https://weaviate.io/developers/weaviate/manage-data/import) for optimal speed.

:::tip For very large files
While we load the JSON into memory here, you can use other methods such as [streaming](https://weaviate.io/developers/weaviate/manage-data/import#tip-stream-data-from-large-files) for very large JSON or CSV files.
:::
...
In this post, we've seen how to write an integration test for an application using Weaviate &mdash; [Embedded Weaviate](https://weaviate.io/developers/weaviate/installation/embedded) in particular.
