...
<!-- TODO: Needs a new hero image - have you asked Svitlana? -->
...
But writing tests and running them can be a chore - for many reasons. For example, if the application is built with Weaviate, the test suite may need to set up and tear down a separate Weaviate server, which can be time-consuming and error-prone.

I'm here to tell you, however, that it doesn't seen to be that way. In this article, I'll show you how to make testing easier with [Embedded Weaviate](/developers/weaviate/installation/embedded), and other tips for better automated testing.

## Testing and Weaviate

In this article, we will focus on [integration tests](https://en.wikipedia.org/wiki/Integration_testing). Integrated testing is an important part of the development process, especially so for complex applications. Weaviate-based apps usually fall in this category.

For one, Weaviate must interact with the application in a variety of ways. In addition, Weaviate will interact with and additional, external services such as vectorizers or LLMs.

Such complexity makes it important to test the application as a whole, and not just its individual components. And this complexity also means that arranging the test suite can be cumbersome, with a variety of moving parts that need to be set up and torn down.
...
<!-- NOTE: The reader probably knows what integrated testing is, if they are here - or they can look it up - have added a link just in case. -->

[Embedded Weaviate](/developers/weaviate/installation/embedded) makes one part of this puzzle much easier, since Weaviate can be instantiated directly from the client. The following is all you need to do to start an Weaviate server:
...
This is not only useful for new contributors, but also for experienced developers. Starting anew as a new contributor, or working from a different machine, can be a hassle. With Embedded Weaviate, you can just run the test suite and be done with it.
...
<!-- Such an integration test could more easily cover the interaction between the application code, the Weaviate server, and the inference provider (e.g. OpenAI, Cohere, Google PaLM) used via a [vectorizer](/developers/weaviate/modules/retriever-vectorizer-modules) or [generator](/developers/weaviate/modules/reader-generator-modules) module. -->


## Scoping tests

<!-- Note: Rewrote this section to provide context and highlight key messages from each point -->

While you may be familiar with tests and integration tests in general, we provide suggestions for Weaviate-powered applications:
* **Whether to test search quality**: This depends primarily on the model of the [vectorization module](/developers/weaviate/modules/retriever-vectorizer-modules). We suggest evaluating models separately, but not tested as a part of the application.
...
[Embedded Weaviate](/developers/weaviate/installation/embedded) lets us spawn an Weaviate server instance from the client, and automatically tear it down when the client terminates. The data is [persisted](/developers/weaviate/installation/embedded#lifecycle) between sessions, so we recommend deleting your data before each test.
...
We'll create question & answer objects from the game show *Jeopardy!*, by specifying its name and the vectorizer ([`text2vec-openai`](/developers/weaviate/modules/retriever-vectorizer-modules/text2vec-openai)).
...
:::tip <i class="fa-solid fa-lightbulb"></i> When might I use tests like these?
...
And all of it was done with very little code with Embedded Weaviate. Next, let's see how we might test imports.
...
And did you notice that we didn't have to spin up an instance of Weaviate through GitHub Actions? That's because we're using Embedded Weaviate, which means we can run our tests without having to separately spin up an instance of Weaviate.
