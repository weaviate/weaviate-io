...
:::info Highlights
- Weaviate has two hybrid fusion algorithms: `rankedFusion` and `relativeScoreFusion`.
- `relativeScoreFusion` has been the default since `v1.24` and is the right choice for most workloads. `rankedFusion` remains available as an opt-in alternative.
:::
...
Weaviate ships two algorithms — `rankedFusion` and `relativeScoreFusion`. `relativeScoreFusion` has been the default since `v1.24` and is recommended for most workloads.
...
### Why `relativeScoreFusion` is the default

`relativeScoreFusion` has been the default since `v1.24`. `rankedFusion` came first and remains available — it's the more conservative choice when you want results to be driven primarily by relative ranking rather than by absolute score magnitudes.

Refinements such as [over-search](https://docs.weaviate.io/weaviate/api/graphql/search-operators#oversearch-with-relativescorefusion) make `relativeScoreFusion` robust enough for most production workloads, which is why it became the default.
