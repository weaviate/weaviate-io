...
Hybrid searches can be ‘weighted’ to give more weight to the vector or keyword search. This is done using the `alpha` parameter. You can read more [here](https://weaviate.io/developers/weaviate/search/hybrid#weight-keyword-vs-vector-results).
:::.
...
The answer is that `rankedFusion` is the older, reliable choice that has worked quite well. In the meantime, we have been reviewing how `relativeScoreFusion` is received by the community, and making small tweaks like adding [over-search](https://weaviate.io/developers/weaviate/api/graphql/search-operators#oversearch-with-relativescorefusion) to make it more robust.
...
- [How-to: Hybrid search](https://weaviate.io/developers/weaviate/search/hybrid)
- [API References: Hybrid search](https://weaviate.io/developers/weaviate/api/graphql/search-operators#hybrid)
