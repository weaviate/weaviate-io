...
# date: 2022-09-13
# toc: true
# redirect_from: /blog/Why-is-Vector-Search-so-fast.html
---
...
For example, I can ask: "What is the tallest building in Berlin?", and the Vector Search engine (in the case of my demo – [Weaviate](/developers/docs/)) responds with "Fernsehturm Berlin".<br/>
...
*For a great example, check [Weaviate benchmarks](/developers/docs/benchmarks/ann#sift1m-1m-128d-vectors-l2-distance), to see how three parameters – [efConstruction, maxConnections and ef](/developers/docs/benchmarks/ann#what-is-being-measured) - affect recall, latency, throughput and import times.*
...
## HNSW in Weaviate
[Weaviate](/developers/docs/) is a great example of a vector search engine that uses ANN algorithms to offer ultra-fast queries. The first ANN algorithm introduced to Weaviate is a custom implementation of [Hierarchical Navigable Small World graphs (HNSW)](/developers/docs/concepts/vector-index-plugins#hnsw).

![HNSW](./img/ann-hnsw.png){:width="50%"}<br/>
*[Figure 4 - Proximity graph-based ANN search]*

Check out [Weaviate ANN benchmarks](/developers/docs/benchmarks/ann) to see how HNSW performed on realistic large-scale datasets. You can use it to compare the tradeoffs between recall, QPS, latency, and import time.<br/>
