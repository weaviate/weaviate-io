...
So, you can use the `text2vec-palm` module module to build a Weaviate database using PaLM-derived vectors for text objects, and perform lightning-fast searches.

The model used, `textembedding-gecko-001`, takes a maximum of 3,072 input tokens, and outputs 768-dimensional vector embeddings.
...
The [`generative-palm`](/developers/weaviate/modules/reader-generator-modules/generative-palm.md) module is another exciting development for Weaviate and Google Cloud / Vertex AI users.
...
The base model is `text-bison-001`, which has the following properties:

- Max input token: 8,192
- Max output tokens: 1,024
- Training data: Up to Feb 2023

## <i class="fa-solid fa-code"></i> Usage examples

To use either modules (or the Vertex PaLM API in general), you will need a Google Cloud "access token", which is also referred in Weaviate as the "PaLM API Key". If you have the Google Cloud CLI tool installed, you can find it via:

```shell
gcloud auth print-access-token
```

### <i class="fa-solid fa-input-numeric"></i> How to use `text2vec-palm`

...

### <i class="fa-solid fa-typewriter"></i> How to use `generative-palm`
