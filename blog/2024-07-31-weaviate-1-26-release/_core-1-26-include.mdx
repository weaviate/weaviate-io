...
The rangeable index can be enabled alone or with the [filterable index](/developers/weaviate/concepts/prefiltering#indexrangefilters). Efficient range filters can thus be combined with other filters (e.g. `Equal`) to quickly narrow your searches to the most relevant information.
