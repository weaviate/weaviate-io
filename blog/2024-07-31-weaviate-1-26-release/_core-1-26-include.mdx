...
The rangeable index can be enabled alone or with the [filterable index](/developers/weaviate/config-refs/schema/range-index#considerations). Efficient range filters can thus be combined with other filters (e.g. `Equal`) to quickly narrow your searches to the most relevant information.
...
Async replication addresses this risk and minimizes it. The async replication mechanism uses an efficient algorithm to monitor and compare nodes within a cluster. If the algorithm identifies an inconsistency, it re-syncs the data on the inconsistent node.
...
This way, do not have to enable the individual API-based modules in the `ENABLE_MODULES` environment variable.
...
The `ENABLE_API_BASED_MODULES` variable disables the [`Explore`](/developers/weaviate/api/graphql/explore.md) feature when `ENABLE_API_BASED_MODULES` is `true`.

## Multi target vector search

Weaviate 1.24 introduces objects with multiple vectors, where each vector may come from a different vectorizer, or be derived using different source properties. This release introduces multi target vector search. Starting in 1.26, you can make a single query that combines results from across multiple target vectors.
...
Tenant [activity status](/developers/weaviate/concepts/data.md#tenant-status) is used to manage resource usage.
...
For details, see [tenant activity statuses](/developers/weaviate/manage-data/multi-tenancy.md#update-tenant-activity-status) and the [`s3-offload` module](/developers/weaviate/configuration/modules.md#offload-s3-module).
...
For code examples, see [async usage examples](/developers/weaviate/client-libraries/python/async#async-usage-examples). See the [application example](/developers/weaviate/client-libraries/python/async.md#application-level-example) for sample code that uses the async client with [FastAPI](https://fastapi.tiangolo.com/), a popular asynchronous web framework for creating modular web API microservices.
