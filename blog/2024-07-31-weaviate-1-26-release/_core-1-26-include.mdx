...
When you anticipate using numerical comparisons over ranges of values, enable the new rangeable index in your [collection schema](https://weaviate.io/developers/weaviate/config-refs/schema). The rangeable index is available for `int`, `number`, and `date` data types.

The rangeable index can be enabled alone or with the [filterable index](https://weaviate.io/developers/weaviate/concepts/filtering#indexrangefilters). Efficient range filters can be combined with other filters, such as the `Equal` filter, to quickly narrow your searches to the most relevant information.
...
For details, see [async replication](https://weaviate.io/developers/weaviate/concepts/replication-architecture/consistency#async-replication).
...
To enable the Anthropic integration on a local deployment, add `generative-anthropic` to the `ENABLE_MODULES` [environment variable](https://weaviate.io/developers/weaviate/config-refs/env-vars), or set the `ENABLE_API_BASED_MODULES` environment variable to `true`.

For details, see the [Anthropic integration page](https://weaviate.io/developers/weaviate/model-providers/anthropic).

## Bulk enable modules

**Experimental** This feature is experimental and may change in future releases.

To enable all API-based [model provider integrations](https://weaviate.io/developers/weaviate/model-providers), set the `ENABLE_API_BASED_MODULES` [environment variable](https://weaviate.io/developers/weaviate/config-refs/env-vars) to `true`.
...
When `ENABLE_API_BASED_MODULES` is `true`, the GraphQL [`Explore`](https://weaviate.io/developers/weaviate/api/graphql/explore) feature is unavailable.
...
For details, see [multi target vector search](https://weaviate.io/developers/weaviate/search/multi-vector).

## Tenant states changed

Tenant [activity status](https://weaviate.io/developers/weaviate/concepts/data#tenant-status) is used to manage resource usage.
...
This feature is only available for self-hosted instances and Weaviate Enterprise Cloud. It is not available for [Weaviate Cloud](https://weaviate.io/developers/wcs) serverless instances.

For details, see [tenant activity statuses](https://weaviate.io/developers/weaviate/manage-data/multi-tenancy#activate-tenant) and the [`s3-offload` module](https://weaviate.io/developers/weaviate/configuration/modules#offload-s3-module).
...
Weaviate can gather metrics that are compatible with the open-source [Prometheus](https://prometheus.io/docs/introduction/overview/) monitoring and alerting system. The v1.26 Weaviate release adds [new metrics](https://weaviate.io/developers/weaviate/configuration/monitoring) that monitor aysnc indexing queues.

Use these metrics with Prometheus and [Grafana](https://grafana.com/docs/) to monitor your Weaviate instance.

If you don't see a metric that you need, you can extend Weaviate to add new metrics. To suggest a new metric, or to get started writing your own, see the [contributor guide](https://weaviate.io/developers/contributor-guide).
...
To enable the Korean tokenizer, set the `ENABLE_TOKENIZER_KAGOME_KR` [environment variable](https://weaviate.io/developers/weaviate/config-refs/env-vars) to `true`, and set the [tokenization method](https://weaviate.io/developers/weaviate/config-refs/schema/#kagome_kr-tokenization-method) for the relevant properties to `kagome_kr`.
