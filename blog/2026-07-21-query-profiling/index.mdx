...
Here `filters_build_allow_list_took` is the largest slice, and `filters_ids_matched` is over a million. The filter is broad, so building the allow-list is the cost. This is a cardinality problem, not a disk one: rethink the filter so it selects less, or apply the wide condition differently rather than as a pre-filter over the whole set.
