...
These add up to big speed and reliability advantages for the [gRPC API in Weaviate](https://weaviate.io/developers/weaviate/api/grpc) in comparison to the current GraphQL + REST solution.
...
The different settings relate to a [vector index (HNSW) parameter](https://weaviate.io/developers/weaviate/concepts/vector-index#hierarchical-navigable-small-world-hnsw-index) `ef`.
...
By the way - if you’ve missed it, the new `v4` client library is a [major update](https://weaviate.io/blog/py-client-v4-release). Not just for its gRPC use, but also the hugely improved Python API. So if you have an existing codebase, the speed will be another incentive to [migrate it to the latest client](https://weaviate.io/developers/weaviate/client-libraries/python/v3_v4_migration), along with the improved client API and support for the shiny new features.
:::
