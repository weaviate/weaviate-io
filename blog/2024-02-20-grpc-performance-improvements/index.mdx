...
Firstly, gRPC uses Protocol Buffers (protobufs) to [define data structures](https://github.com/weaviate/weaviate/tree/master/grpc/proto/v1s) like the one shown below to produce binary payloads instead of JSON. Because these definitions are used to build the payload, and the payload is in binary, each payload is smaller, and faster to build.
