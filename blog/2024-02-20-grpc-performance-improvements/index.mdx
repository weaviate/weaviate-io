...
For most of you, the changes mean just one thing. The latest Weaviate solutions are simply much faster than our pre-gRPC solutions, which were using REST + GraphQL. Take a look at these results:

:::warning TODO
ADD UPDATED FIG
:::

The chart really speaks for itself (although - we’ll expand on that below). And since these changes are under the hood, you won’t even really see which calls use gRPC, vs those that use REST or REST+GraphQL. It will simply be a function of using a client and server pair that relies on gRPC.
...
gRPC was initially developed by Google for working with microservices, but it is now open-source and maintained by Cloud Native Computing Foundation. Thus, gRPC is yet another open-source project that works as a key part of the network infrastructure ecosystem.

<details>
  <summary>What is an RPC?</summary>

An RPC, or “remote procedure call”, can be used to remotely execute a function or procedure on a device from another device. (Read more)[LINK_TO_SOME_RPC_DEF]
...
Firstly, gRPC uses Protocol Buffers (protobufs) to define data structures as shown below to produce binary payloads instead of JSON. Because these definitions are used to build the payload, and the payload is in binary, each payload is smaller, and faster to build.
...
These add up to big speed and reliability advantages for the gRPC API in Weaviate in comparison to the current GraphQL + REST solution.
...
Moreover, parsing complex and deeply nested JSON responses, or crafting intricate GraphQL queries, can degrade the developer experience and elevate the risk of errors. gRPC addresses these issues head-on by facilitating the customization of data structures for both requests and responses. Its reliance on strictly defined data types streamlines parsing processes and boosts reliability.
...
We’ve said that the gRPC solution improves speed. But how much faster is it, exactly? Let’s take a look at two key scenarios - data ingestion and query throughput. The lighter bars show gRPC results, whereas the darker bars show REST / GraphQL query results.

<details>
  <summary>Test set-up</summary>

The tests were carried out using two pre-vectorized (sift1m and DBPedia) datasets, with Weaviate version `1.23.9` and Weaviate Python Client version `4.4.4` on an M1 MacBook Pro. The `sift1m` dataset includes 128-dimensional vectors and the `DBPedia` dataset includes 1536-dimensional vectors.
<br/>

You can find the [test code here](https://github.com/weaviate/weaviate-python-client/blob/e2398593c8346b0e32c4077897583398e2673b73/profiling/test_import_and_query.py) including links to the datasets, and instructions to run the tests yourself.

</details>

### Data ingestion

Let’s begin with improvements to the data ingestion time. The following chart shows results from total time taken to insert ~1M objects.

:::warning TODO
ADD UPDATED FIG
:::

The gRPC-based data imports show significant reductions in time taken over the REST-based alternatives for both datasets. The time taken to ingest 1M DBPedia objects was nearly halved from over 34 minutes to under 19 minutes. The larger improvement likely comes from the larger payload size as the dataset includes 1536-dimensional vectors in comparison.

This improvement will scale as the dataset size grows, too. At this rate, inserting a hundred million objects - a relatively common number - would be faster by over a full day!

### Queries

gRPC speed improvements will extend to database queries as well. The following chart shows how many queries can be served in given scenarios.

:::warning TODO
ADD UPDATED FIG
:::

Here you can see that each query takes 40-80% less time to return results when using gRPC in comparison to results with REST + GraphQL. This results in a significant, up to 3.3x, improvement in how many queries can be served.
...
The different settings relate to a [vector index (HNSW) parameter](https://weaviate.io/developers/weaviate/concepts/vector-index#what-is-hierarchical-navigable-small-world-hnsw) `ef`, which dictates how long the operation within Weaviate takes. But you can see in all cases that the improvements are significant.
...
Next, you need a version of Weaviate that supports gRPC, and a matching client version. The reason is that gRPC requires matching server and client protobuf files for calls, messages and so on. As a result, any updates must be coupled server-side and client-side.
...
By the way - if you’ve missed it, the new `v4` client library is a [major update](https://weaviate.io/blog/py-client-v4-release). Not just for its gRPC use, but also the hugely improved Python API. So if you have an existing codebase, the speed will be another incentive to migrate it to the latest client (e.g. [to the `v4` client API in Python](/developers/weaviate/client-libraries/python/v3_v4_migration)), along with the improved client API and support for the shiny new features.
:::
