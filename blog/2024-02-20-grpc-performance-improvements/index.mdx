...
![gRPC vs REST+GraphQL: DBPedia queries](./img/query-improvements-dbpedia.png)
...
Query throughput is central to databases' operation. And the improvements here are certainly very meaningful. The following charts show how many queries can be served in the given scenarios using gRPC and REST+GraphQL.

![gRPC vs REST+GraphQL: SIFT queries](./img/query-improvements-sift.png)
![gRPC vs REST+GraphQL: DBPedia queries](./img/query-improvements-dbpedia.png)

The charts show that each query takes 40-70% less time to return results when using gRPC in comparison to results with REST + GraphQL. This results in a significant, over 2.6x, improvement in how many queries can be served.
