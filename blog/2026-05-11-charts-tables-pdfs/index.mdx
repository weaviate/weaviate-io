...
Weaviate ships a hosted version of such a model, called `multi2vec-weaviate`. It runs on Weaviate Cloud, and you don't have to host or manage it. From your perspective it's just another vectorizer.
...
* Vectorizing it with `multi2vec-weaviate`, producing many vectors per page.

A few things worth noting:

* **One row equals one page.** The unit of retrieval is the page, which is also the unit a human navigates a document by.
...
The [Weaviate Query Agent](https://docs.weaviate.io/agents/query) is a managed agent that wraps vector retrieval with multi-step reasoning, source citations, and inline page images. It is available out of the box on any Weaviate Cloud cluster. Point it at the collection you just created and ask.
...
Then, render each PDF page to a 2000-pixel PNG and store it as a BLOB in a collection vectorized with `multi2vec-weaviate`:
...
* **The vectorizer is `multi2vec_weaviate(image_field="page_image")`.** That's the same model the drag-and-drop UI uses. You can mix and match: ingest from a notebook today, query from the Console tomorrow.
...
* **In code, it's the same primitive.** `multi2vec_weaviate(image_field="page_image")` is the entire vectorizer config. The rest is rasterising pages with PyMuPDF and a small `batch.add_object` loop.
