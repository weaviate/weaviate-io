...
Weaviate offers such a model called `multi2vec-weaviate`; it is natively available on Weaviate Cloud and you don't have to host or manage a model in order to generate multi-vectors from your PDFs.
...
For further optimization, you might consider using a hybrid approach, where you identify pages with charts to implement as as multi-vectors and the remaining corpus as text, and then use an RRF-style approach to merge results at query time.
...
* **Turn the same primitive into deployable code.** `multi2vec_weaviate(image_field="page_image")` is the entire vectorizer config. The rest is rasterising pages with PyMuPDF and a small `batch.add_object` loop.
