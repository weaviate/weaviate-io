...
### Use Cases
This enables a variety of [use cases](/developers/docs/concepts/replication-architecture/motivation.html). For example, if a Weaviate node goes down, another node can shoulder the load without losing availability or data. Data in Weaviate will thus have a **higher availability** for its users.
...
It is often tolerable if data is temporarily out of sync, as long as consistency is reached eventually. Keeping the [CAP Theorem](/developers/docs/concepts/replication-architecture/#cap-theorem) trade-offs in mind, Weaviate's replication architecture **prefers availability over consistency**.

Nevertheless, both read and write consistency will become [fully tunable](/developers/docs/concepts/replication-architecture/consistency.html) from `v1.18` (read consistency is partly tunable from `v1.17`).

Weaviate has a [leaderless replication design](/developers/docs/concepts/replication-architecture/cluster-architecture.html), so there is no distinction between primary and secondary nodes, thereby removing all single points of failures.
...
### Learn More
To learn more, visit the documentation pages for [Replication Architecture](/developers/docs/concepts/replication-architecture) and/or [Usage](/developers/docs/configuration/replication.html), where all concepts are explained in detail, supported by illustrations! 
...
### Learn More
The hybrid operator is available through GraphQL, REST and all Weaviate client languages. Check out the [documentation](/developers/docs/api/graphql/vector-search-parameters.html#hybrid) for more information and learn how to write the query in the other client programming languages.
...
### Learn More
The `bm25` operator is available through GraphQL, REST and all Weaviate client languages. Check out the [documentation](/developers/docs/api/graphql/vector-search-parameters.html#bm25) for more information.
