...
<Figure>
  <img
    ...
    width="800"
  />
    <figcaption><small>Single vector to multi-vector comparison<br /><br />
  </small></figcaption>
</Figure>

![single-multi-comp.png](./img/single-multi-comp.png)
...
![multi-vector-embeddings1.png](./img/multi-vector-embeddings1.png)
...
If we embed a million documents of ~100 tokens per document, a single-vector embedding model (of 768 dimensional single-precision, 32-bit, floating point numbera) may require `768 * 1M * 4 bytes = ~3.1GB` of memory a multi-vector embedding model (of 96 dimensions) may require a whopping `96 * 100 * 1M * 4 bytes = ~40GB`!

![single-multi-memory.png](./img/single-multi-memory.png)
...
- dot-product of the encoded embeddings $x_{single}$ (aka single-vector embedding), and
- `maxSim` of the multi-vectors embeddings $x_{multi}$
...
![muvera_high_level.png](./img/muvera_high_level.png)
...
![muvera_steps_1.png](./img/muvera_steps_1.png)

![muvera_steps_2.png](./img/muvera_steps_2.png)
...
**Which function to use? (NOTE: This should be an insert)**
...
From now on, in the description of MUVERA algorithm we will be considering as partitioning function the SimHash.
...
![muvera_steps_3.png](./img/muvera_steps_3.png)
...
![Heap allocation without using MUVERA + `sq` vs. MUVERA +  `sq` ](./img/test_results_heap_usage.png)

Heap allocation without using MUVERA + `sq` vs. MUVERA +  `sq`

![Import time without using MUVERA + `sq` vs. MUVERA +  `sq` ](./img/test_results_time.png)

Import time without using MUVERA + `sq` vs. MUVERA +  `sq`

![QPS without using MUVERA + `sq` vs. MUVERA +  `sq` ](./img/test_results_qps.png)

QPS without using MUVERA + `sq` vs. MUVERA +  `sq`

![Recall without using MUVERA + `sq` vs. MUVERA +  `sq` ](./img/test_results_recall.png)
...
However, some of this can be mitigated through the HNSW search settings. As the graphs show, setting a higher `ef` value in the query settings (e.g. >512) can increase recall to 80%+, and over 90% at 1024.
...
![muvera_compared.png](./img/muvera_compared.png)
