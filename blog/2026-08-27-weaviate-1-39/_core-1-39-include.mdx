...
:::note A balance of 0.5 is not a safe middle
On hybrid search the two sides of the trade are not measured on the same scale: relevance is a merged score normalized into `[0, 1]`, and diversity is a raw distance in your collection's metric. Start at `1.0` and check that you get your current ordering back, then lower `balance` until the page stops repeating itself.
:::
