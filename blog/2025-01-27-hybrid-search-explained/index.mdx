...
$$
\mathrm{score}(D, Q) = \sum_{i=1}^{n} \mathrm{IDF}(q_i)\,
...
<center>Source: Benham and Culpepper, 2018</center>
...
The above table shows the ranked order of the BM25 and Dense search. To fuse the two lists together, we need to take the sum of the reciprocal ranks. Based on the results, the top document is `Document B` with a ranking of 1.5, then `Document A` at 1.3, and `Document C` at 0.83. (We set _k_ to 0 to keep the arithmetic readable. Weaviate uses 60.)

One thing to be clear about: the walkthrough above describes `rankedFusion`, which is *not* the default. Weaviate supports two fusion algorithms, and `relativeScoreFusion` has been the default since `v1.24`. The difference is what each one throws away. `rankedFusion` keeps only the position of a result in each list and discards the scores. `relativeScoreFusion` keeps the scores: it min-max normalizes each list so the top score becomes 1 and the bottom becomes 0, with everything in between keeping its relative spacing, then adds the weighted normalized scores. That preserves the information that result 1 beat result 2 by a mile rather than by a hair. You can switch to `rankedFusion` via the `fusionType` parameter.
...
- `fusionType` (optional, default is `relativeScoreFusion`): choose the fusion algorithm
