...
Weaviate supports two fusion algorithms, `rankedFusion` and `relativeScoreFusion`. `relativeScoreFusion` is the default fusion algorithm since `v1.24`. You can switch to `rankedFusion` via the `fusionType` parameter.
...
- `query`: search query
- `alpha` (optional, default is `0.5`): weighting for each search algorithm
