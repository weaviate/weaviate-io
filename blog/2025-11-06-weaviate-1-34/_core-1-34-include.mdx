...
## Enhanced stability during version updates

Weaviate v1.34 introduces a few improvements when it comes to **rolling updates**, with the aim of strengthening zero-downtime deployments. We are dedicated in ensuring version upgrades or scaling events cause no performance degradation, no increased error rates, and no observable side effects—regardless of the number of tenants, collection sizes, or query volume. This makes Weaviate suitable for mission-critical applications that require continuous availability.
