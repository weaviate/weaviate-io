Weaviate `v1.34` is now available open-source and on [Weaviate Cloud](https://console.weaviate.cloud). This release brings **ACORN filter strategy by default** for faster filtered searches, **flat index support with RQ quantization**, **server-side batching**, **30+ new observability metrics**, and a sneak peek into upcoming **C# and Java v6 client libraries**.
...
Weaviate `v1.34` introduces **flat index support with rotational quantization (RQ)**, combining flat indexing with efficiency of RQ compression.
...
- [How-to: Configure RQ compression](https://docs.weaviate.iop/weaviate/configuration/compression/rq-compression)
...
Server-side batching in `v1.34` is a **preview** feature. This means it's still under development and may change in future releases. We don't recommend using it in production environments at this time.

:::

Building on the server-side batch imports introduced in `v1.33`, This release improves backpressure mechanisms and performance optimizations for data ingestion.
...
- [C# client documentation](https://docs.weaviate.io/weaviate/client-libraries/csharp)
...
The default `Contextual AI` models are the `v2` GLMs for generation and `ctxl-rerank-v2-instruct-multilingual` for reranking.
