...
_How Engram fits alongside `MEMORY.md`_

Engram structures memory around _topics_: semantic categories that let recall filter to exactly what's relevant rather than searching a flat pile of everything. I went through my typical work and identified four categories that would actually be meaningful for my workflow:
...
With those categories in place defining _what_ to store, the next question emerged naturally: _when_? I then landed on these interaction patterns:
...
_The session lifecycle: when Engram saves and recalls_
