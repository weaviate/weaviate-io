...
```shell
{
  ...
  "weaviate.connection.url": "$WEAVIATE\_REST\_URL",

  "weaviate.grpc.url": "$WEAVIATE\_GRPC\_URL",

  "weaviate.grpc.secured": true,

  "weaviate.auth.scheme": "API\_KEY",

  "weaviate.api.key": "$WEAVIATE\_API\_KEY",

  "collection.mapping": "Weaviate\_test",
...
```shell
$\> curl \-i \-X PUT https://\<KAFKA\_CONNECT\_URL\>/connectors/weaviate/config \-H 'Content-Type:application/json' \--data @connector.json
```
