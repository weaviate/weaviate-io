...
This feature will improve import speed for large datasets by allowing the vector indexing to be performed asynchronously to object creation.

In the past, when importing a large dataset, creation of the HNSW would be performed synchronously to object creation. This meant that the import process would be blocked until the indexing process was complete. While indexing does occur relatively quickly, it was still a bottleneck for the process.

With `1.22`, the vector indexing process is now performed asynchronously to object creation. This means that the import process is no longer blocked by the indexing process, and can continue to create objects while the indexing process is ongoing.
...
You may already have seen [a blog post](../2023-09-26-collections-python-client/index.mdx) and [a documentation page](/developers/weaviate/client-libraries/experimental.md) about this new client. If you have yet to try it out, we encourage you to do so!
