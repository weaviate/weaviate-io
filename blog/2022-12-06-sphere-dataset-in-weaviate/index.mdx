...
:::caution Outdated code in the v3 tab
Published 2022-12-06. The v3 client tab on this page uses syntax that no longer works against the current Python client — use the v4 tab. Sphere is still an active Weaviate benchmark; see the [ANN benchmarks page](https://docs.weaviate.io/weaviate/benchmarks/ann).
:::
...
client.schema.create_class({
    ...
    "moduleConfig": {
        ...
        "options": {
            "waitForModel": True,
            "useGPU": False,  # Set to True if your account allows GPU usage
            "useCache": True
        }
    },
    "properties": []  # Rely on auto-schema - make sure it is enabled on your Weaviate instance
})

# Import the data#
# The data schema will be inferred by Weaviate and created automatically
start = time.time()
c = 0
