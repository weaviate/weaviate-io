...
### Use Cases
This enables a variety of [use cases](https://weaviate.io/developers/weaviate/concepts/replication-architecture/motivation). For example, if a Weaviate node goes down, another node can shoulder the load without losing availability or data. Data in Weaviate will thus have a **higher availability** for its users.
...
It is often tolerable if data is temporarily out of sync, as long as consistency is reached eventually. Keeping the [CAP Theorem](https://weaviate.io/developers/weaviate/concepts/replication-architecture/#cap-theorem) trade-offs in mind, Weaviate's replication architecture **prefers availability over consistency**.

Nevertheless, both read and write consistency will become [fully tunable](https://weaviate.io/developers/weaviate/concepts/replication-architecture/consistency) from `v1.18` (read consistency is partly tunable from `v1.17`).

Weaviate has a [leaderless replication design](https://weaviate.io/developers/weaviate/concepts/replication-architecture/cluster-architecture), so there is no distinction between primary and secondary nodes, thereby removing all single points of failures.
...
### Learn More
To learn more, visit the documentation pages for [Replication Architecture](https://weaviate.io/developers/weaviate/concepts/replication-architecture) and/or [Usage](https://weaviate.io/developers/weaviate/configuration/replication), where all concepts are explained in detail, supported by illustrations!
...
Work on hybrid search is not over! We will continue to make improvements and introduce new features in the future. For example, there is already something in the [backlog](https://github.com/weaviate/weaviate/issues/2393) to add [`where` filters](https://weaviate.io/developers/weaviate/api/graphql/filters#where-filter) to hybrid search (give it a thumbs up while you're there). 🙂

### Learn More
The hybrid operator is available through GraphQL, REST and all Weaviate client languages. Check out the [documentation](https://weaviate.io/developers/weaviate/api/graphql/search-operators#hybrid) for more information and learn how to write the query in the other client programming languages.
...
### BM25 Operator
In Weaviate v1.17, we introduce a search operator called [`bm25`](https://weaviate.io/developers/weaviate/api/graphql/search-operators#bm25), which performs a case-insensitive keyword search, then ranks the results using the BM25F ranking function - **B**est **M**atch **25** with Extension to Multiple Weighted **F**ields, a variant of BM25 that can take into account different weights for different properties (fields) of the document, in order to produce more accurate results.
...
### Learn More
The `bm25` operator is available through GraphQL, REST and all Weaviate client languages. Check out the [documentation](https://weaviate.io/developers/weaviate/api/graphql/search-operators#bm25) for more information.
