...
We do not include ColBERT in this comparison as for this qualitative experiment there is only a single document, therefore ColBERT in a retrieval setup would only have one document it can return. However, in our linked notebook we have included a ColBERT comparison using it as a reranker.
:::
...
| **Late Interaction (no pooling)**              | 8,000                                  | 100,000             | 800 million          |
| **Naive Approach (chunking before inference)** | 16 ( 8,000 / 512 )                     | 100,000             | 1.6 million         |
| **Late Chunking (chunking after inference)**   | 16 ( 8,000 / 512 )                     | 100,000             | 1.6 million         |
