...
For example, let's compare the storage requirements for a long context model that outputs **8000 token-level embeddings**:
...
| Approach                               | Number of embeddings | Number of Documents | Total Vectors Stored |
| -------------------------------------- | -------------------- | ------------------- | -------------------- |
| **Late Interaction (no pooling)**      | 8,000 (tokens)       | 100,000             | 800 million          |
| **Naive Approach (pooling + chunked)** | 8,000 / 512 (tokens) | 100,000             | ~1.6 million         |
