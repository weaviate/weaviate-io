...
<table>
  ...
  <tr>
    <td>
      ...{ommitted for brevity} When building a Retrieval Augmented Generation (RAG) application, using account or
...
To answer this query correctly we need to return **both** of the above chunks. However, with the naive approach we end up with two separate chunks that are not neighbouring one another.
