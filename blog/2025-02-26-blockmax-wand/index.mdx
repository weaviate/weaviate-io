...
BlockMax WAND is in **preview** as of version 1.29. [Instructions on how to enable it are available here.](/developers/weaviate/concepts/indexing#blockmax-wand-algorithm)
...
Keyword search requires us to first define what words we want to search. A big part of this process is [tokenization](/developers/academy/py/tokenization/basics). It splits the input documents into [tokens](https://en.wikipedia.org/wiki/Lexical_analysis#Token) (i.e. terms, numbers or anything else one would consider important to be searched individually).  
...
| Doc Id | Document Title | Tokenizated Title |
| :---- | :---- | :---- |
...
Thus, if you want to search for `“Hybrid search or Vector search”`, we get the terms `[“hybrid”, “search”, “or”, “vector”]`.
Inspecting the posting lists for each term, we can see that documents 1, 2 and 3 have at least one of the query terms.
...
The exhaustive way of computing the [BM25](/blog/hybrid-search-explained#bm25) scores we use at Weaviate would be to check all the documents that have at least one of the query terms and score them. 
...
| WAND | BlockMax WAND |
| :----: | :----: |
| ![WAND](./img/wand_block.jpg) | ![BlockMax WAND](./img/blockmax_block.jpg) |
...
**Table**:  Average % of doc query terms scored Weaviate 1.29.0 on standard [beir datasets](https://github.com/beir-cellar/beir). 
...
<iframe src="https://andremourao.com/wand/" width="100%" height="800px" />
...
Term frequencies are usually in the 1-100 range, with no hard ceiling. Let’s assume they may grow all the way to 2^32. Storing them as-is would take 32 bits \* number of documents for each term.
...
**Table**: Average p50 query time in ms for Weaviate 1.29.0 on standard [beir datasets](https://github.com/beir-cellar/beir) using the test queries. Experiments performed on a Apple M3 Max 36 GB. Similar gains observed on x86-64 cloud machines
