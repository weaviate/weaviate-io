...
As the text corpora size becomes larger and larger, keyword searches can take long times to execute. An [inverted index](https://en.wikipedia.org/wiki/Inverted_index) is a critical data structure for speeding up keyword searches. 
...
In keyword search context, these `terms` are called `tokens`. 

Keyword search requires us to first define what tokens we want to search. A big part of this process is [tokenization](/developers/academy/py/tokenization/basics). It splits the input documents into [tokens](https://en.wikipedia.org/wiki/Lexical_analysis#Token) (i.e. terms, numbers or anything else one would consider important to be searched individually).  
...
Now we have turned documents into a [bag-of-words](https://en.wikipedia.org/wiki/Bag-of-words_model) model, where we can find for individual query tokens in the sentences. But having to go through all documents to find the ones that have query tokens isn’t efficient.   

This is where the *inverted* part of the [inverted index](https://en.wikipedia.org/wiki/Inverted_index) comes from: instead of going document \-\> token, create an index that does from token \-\> documents. It works like the indexes at the end of books, but instead of mapping tokens to book pages, we map tokens to documents using posting lists.

A posting list is a list of "postings", which contain the information needed to score documents: doc id (to identify which documents have the term), [term frequency (tf)](https://en.wikipedia.org/wiki/Tf%E2%80%93idf), which represents the number of times the term is part of the property. For example, `tf(“vector”)` for doc 3 is 2, as it shows up twice.
...
**Table**: Posting lists for tokens `hybrid`, `search`, and `vector` in the example dataset.
...
Keyword search is based on the intuition that some words, or `tokens`, are more important than others.
*vector*, *hybrid*, *search*, *database* are more descriptive of the contents of the documents than words like *a*, *to*, *of*, *power*, and thus, should be scored higher.
...
But this is quite resource intensive; most searches are  for the top 10-100 results, and even with pagination, at most, only \-100 documents end up being shown to the user for each search. 
This means that if 100 000 documents have at least one of the query tokens (normal for queries with common words in databases with 1 million documents), this is 0.1% of the documents, many of which are completely irrelevant to the query, wasting a lot of CPU and I/O resources.
...
* Approximate evaluation over query token postings in parallel to identify candidate docs with max impact heuristics (based on idf);  
...
Max impact is the maximum score a token can contribute to the score.  
...
* As we move forward, we can then start skipping documents where the sum of the max impacts of its tokens, is lower than the lower bound.
...
**Table**:  Average % of doc query tokens scored Weaviate 1.29.0 on standard [beir datasets](https://github.com/beir-cellar/beir) without stopword removal. 
Exhaustive search always needs to examine 100% of the document query tokens with at least one query token.

The experimental results show that BlockMax WAND is able to further halve the number of documents inspected from the already remarkable 15-30% number of tokens scored to 5-15%.

But how does BlockMax WAND work in practice?

### BlockMax WAND Demo

Words are abstract and it can be hard to understand exactly what is happening at each step of the process.
So, we've created a **demo** to show you exactly how BlockMax WAND works in practice!
- **Input your documents, queries, and search parameters** and see exactly how Exhaustive search, WAND, and BlockMax WAND work in practice.
...
Without further ado, let's see BlockMax WAND in action!
...
Terms and tokens are used interchangeably in the context of this section.
This is a lot of information to store and decode for each query:

- For 1 million documents with 100 tokens each, we would need to store and decode 100 million term frequencies and doc ids.
- Term frequencies are usually in the 1-100 range, with no hard ceiling. Let’s assume they may grow all the way to 2^32. Storing them as-is would take 32 bits \* number of documents for each token.
- Doc ids are 64 bit unsigned integers, and we need to store them for each document that has the token.
...
**Table**: Comparing the size of `searchable` folders of Weaviate 1.29.0 on standard [beir datasets](https://github.com/beir-cellar/beir). 
...
**Table**: Average p50 query time in ms for Weaviate 1.29.0 on standard [beir datasets](https://github.com/beir-cellar/beir) using the test queries without stopword removal. Experiments performed on a Apple M3 Max 36 GB. Similar gains observed on x86-64 cloud machines
