...
| A block is a mini posting list (list of doc ids and TFs) with extra metadata: Max doc id: highest doc id that shows up in the block Max impact: max TF of a document within the block (norm TF equals TF/prop length) | ![BlockMax WAND Block example](./img/block_example.png) |
| :---- | :---- |
...
Average p50 query time for Weaviate 1.29.0 on standard [beir datasets](https://github.com/beir-cellar/beir) using the test queries. Experiments performed on a Apple M3 Max 36 GB. Similar gains observed on x86-64 cloud machines
