...
image: ./img/hero.png
tags: ['release']
description: "Weaviate 1.20 - multi-tenancy, PQ, search re-ranking, autocut, and more!"

---
![Weaviate 1.20](./img/hero.png)

<!-- truncate -->
Weaviate `v1.20` is here! As always, a brand-new release means a set of brand-new features. Here are the ⭐️*highlights*⭐️:

![Feature highlights](./img/highlights.png)
...
<!-- TODO - check if downgrade from 1.20 to 1.19 is possible
...
We restructured arrays by collapsing two-dimensional slices into a single dimension.
<p><br/></p>
...
Below we provide a small taste of these experiments comparing PQ `v1.18` vs the new and improved PQ of `v1.20`. Firstly we show the performance on an experimental dataset followed by a more real-world dataset.
<p><br/></p>
...
Before `v1.20`, limiting the number of search results meant manually specifying a `distance` or the `limit` (max) number of objects. Both of which can be a bit cumbersome to use, especially with no prior knowledge of the dataset.
...
t can be combined with all [vector search operators](/developers/weaviate/api/graphql/get#vector-search-operators): `nearXXX`, `bm25`, and `hybrid`. `autocut` is placed after the operator, at the same level with `limit`/`offset`. It is disabled by default and can be explicitly disabled by setting its value to 0 or a negative number.
...
You can use reranking in Weaviate’s GraphQL as follows:
...
We are calling this the `relative score fusion` ranking algorithm, as it works by normalizing and summing the BM25 score and vector similarity values to produce an output. This is in contrast to the existing `ranked fusion` algorithm which produced a new rank based on a sum of inverses of the BM25 and vector rankings.
...
In Weaviate `1.20` we’re rolling out a way to monitor the fraction of requests that succeed/fail due to user errors, and/or server errors. This enables us to track these ratios over time and see if significant updates affect this. Having a code for each request simplifies the log monitoring process!
...
<details>
  <summary>Examples of requests</summary>

Here is an example of a few requests:
