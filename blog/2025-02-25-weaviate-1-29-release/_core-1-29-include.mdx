...
If you would like to try out multi-vector embeddings in Weaviate, check out the [Multi-vector embeddings tutorial](https://weaviate.io/developers/weaviate/tutorials/multi-vector-embeddings) which will take you end-to-end, for both:

- [Using Jina AI ColBERT model integration](https://weaviate.io/developers/weaviate/tutorials/multi-vector-embeddings#option-1-colbert-model-integration), or
- [Using user-provided multi-vector embeddings](https://weaviate.io/developers/weaviate/tutorials/multi-vector-embeddings#option-2-user-provided-embeddings).

## NVIDIA model support

Weaviate's suite of [model integrations](https://weaviate.io/developers/weaviate/model-providers/) now includes support for NVIDIA's NIM inference service.

![Embedding integration illustration](./img/integration_nvidia_embedding.png)

Weaviate users can now use NVIDIA model integration to [create text embeddings](https://weaviate.io/developers/weaviate/model-providers/nvidia/embeddings), [create multi-modal embeddings](https://weaviate.io/developers/weaviate/model-providers/nvidia/embeddings-multimodal), and use [generative AI models](https://weaviate.io/developers/weaviate/model-providers/nvidia/generative). (Reranker model support coming soon)
...
Refer to the [RBAC documentation](https://weaviate.io/developers/weaviate/configuration/rbac) for more information.
...
To read more about BlockMax WAND, and to try it out, refer to the [Indexing page](https://weaviate.io/developers/weaviate/concepts/indexing#blockmax-wand-algorithm).
...
To activate async replication, set `asyncEnabled` to true in the [`replicationConfig` section of your collection definition](https://weaviate.io/developers/weaviate/manage-data/collections#replication-settings). Visit the [How-to: Replication](https://weaviate.io/developers/weaviate/configuration/replication#async-replication-settings) page to learn more about the available async replication settings, and [Concepts: Replication/Consistency](https://weaviate.io/developers/weaviate/concepts/replication-architecture/consistency) for more information on how async replication works.
