...
If the first hop passes the filter, then the algorithm traverses the graph normally since the second hop will be explored anyways as the search progresses. We only use the two hop expansion if the connecting node does not pass the filter. This means that when is not a uniform distribution of the filtered nodes in the graph, our algorithm will operate as a regular HNSW in areas where a dense amount of nodes pass the filter and as a pure ACORN where a sparse amount of nodes pass the filter.
...
Each figure shows test results for a different selectivity rate, between 50% and 20%. The selectivity rate is the percentage of objects that pass the filter. The higher the selectivity rate, the more objects pass the filter and the easier it is to find the target region in the search space.

The `acorn` line represents the new ACORN implementation, while the `sweeping` line represents the old implementation. The y-axis shows the throughput in queries per second (QPS), while the x-axis shows the search quality (recall). The search quality is controlled by setting the `ef` parameter in the vector index configuration. The higher the `ef`, the better the search quality, but the slower the search.
...
![ACORN vs pre-ACORN, 50% selectivity](./img/beir-cohere-500k-filtered-dot-0.5-dark.png#gh-dark-mode-only)
![ACORN vs pre-ACORN, 50% selectivity](./img/beir-cohere-500k-filtered-dot-0.5-light.png#gh-light-mode-only)

**40% selectivity**

![ACORN vs pre-ACORN, 40% selectivity](./img/beir-cohere-500k-filtered-dot-0.4-dark.png#gh-dark-mode-only)
![ACORN vs pre-ACORN, 40% selectivity](./img/beir-cohere-500k-filtered-dot-0.4-light.png#gh-light-mode-only)

**30% selectivity**

![ACORN vs pre-ACORN, 30% selectivity](./img/beir-cohere-500k-filtered-dot-0.3-dark.png#gh-dark-mode-only)
![ACORN vs pre-ACORN, 30% selectivity](./img/beir-cohere-500k-filtered-dot-0.3-light.png#gh-light-mode-only)

**20% selectivity**

![ACORN vs pre-ACORN, 20% selectivity](./img/beir-cohere-500k-filtered-dot-0.2-dark.png#gh-dark-mode-only)
![ACORN vs pre-ACORN, 20% selectivity](./img/beir-cohere-500k-filtered-dot-0.2-light.png#gh-light-mode-only)
...
Next, let's look at the results when the correlation between filters and queries is low.

### Low correlation between filters and queries

This is where the ACORN implementation shows its value. In this example, we have a dataset shifting queries to different data sources. This means that queries would target different data sources and thus we should expect a low correlation between the filters and queries. The results are shown in the image below.

![ACORN vs pre-ACORN, low correlation](./img/beir-cohere-dot-filtered-negative-light.png#gh-dark-mode-only)
![ACORN vs pre-ACORN, low correlation](./img/beir-cohere-dot-filtered-negative-light.png#gh-light-mode-only)
...
As a result, this could have been a source of a potential bottleneck in the system. But not any more - the new ACORN algorithm performs admirably in this scenario, and the performance does not get bogged down by these "unusual" (but actually quite common) queries.

By preventing occurrences of such bottlenecks, Weaviate with the new ACORN search strategy is able to ensure that a vector/hybrid search system does not get bogged down when these “unusual” queries in fact become quite common as we have seen happen in the wild.
