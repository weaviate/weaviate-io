...
We’ve created an AmazonProducts collection with two properties, title and description, which aligns with the columns in our DataFrame. Additionally, the collection is using the [Databricks module](https://weaviate.io/developers/weaviate/model-providers/databricks) to: 
