...
:::caution These models have been retired
`text-davinci-003` and `command-xlarge-nightly` were the current models when this post was written in May 2023, and every generation shown below came from them. Both have since been retired by their providers, so the code below will fail until you pick a current model from the [OpenAI](https://docs.weaviate.io/weaviate/model-providers/openai/generative) or [Cohere](https://docs.weaviate.io/weaviate/model-providers/cohere/generative) generative docs. The pattern is unchanged; only the model name moves.
:::
...
client = weaviate.connect_to_embedded(
    headers={
        "X-OpenAI-Api-Key": "<YOUR OPENAI KEY HERE>",  # if using Cohere, replace with "X-Cohere-Api-Key"
    },
)
...
ad_collection.config.add_property(
    Property(
        name="target",
...
user_collection = client.collections.use("User")
listing_collection = client.collections.use("Listing")
ad_collection = client.collections.use("Ad")

users = user_collection.query.fetch_objects(limit=5).objects
...
for user in users:
    ...
    for ad in response.objects:
        new_ad_properties = {
            "content": ad.generative.text,
        }
        new_ad_id = uuid4()
        ad_collection.data.insert(
            properties=new_ad_properties,
            uuid=new_ad_id,
        )
        ad_collection.data.reference_add(
            from_uuid=new_ad_id,
            from_property="hasUserTarget",
            to=user.uuid,
        )
```
