...
In this example we will be using the OpenAI `gpt-4o-mini` and the Cohere `command-r-plus` LLMs to generate content, but this is also possible with other providers such as HuggingFace, Anthropic, and more. Set your OpenAI or Cohere key to use with Weaviate's generate module using the following block of code:
...
client.get_meta()  # see which modules are enabled in your Weaviate instance
```
