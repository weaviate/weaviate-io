...
faq:
  ...
  - question: "How do I configure stopwords per language in a vector database?"
    answer: "Define named presets in invertedIndexConfig.stopwordPresets — each preset is a flat list of words. Reference them per property via textAnalyzer.stopwordPreset (only supported on properties with `word` tokenization). For additions/removals on top of a built-in, use the legacy invertedIndexConfig.stopwords field instead — stopwordPresets only accepts flat word lists."
...
Weaviate solves this two ways. First, you can declare **named stopword presets** at the collection level. Each preset is a flat list of words to drop. A preset name that matches a built-in (`en`, `none`) replaces the built-in for this collection. If you want to extend a built-in with `additions`/`removals` rather than replace it, use the legacy [`invertedIndexConfig.stopwords`](https://docs.weaviate.io/weaviate/config-refs/indexing/inverted-index#stopwords) field — `stopwordPresets` only accepts flat word lists.
