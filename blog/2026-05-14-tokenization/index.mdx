...
authors: [andre, ivan]
date: 2026-05-14
tags: ['concepts', 'engineering', 'search', 'tokenization']
image: ./img/hero.png
...
faq:
  - question: 'Does Weaviate support custom tokenizers?'
    answer: 'Yes - built-in methods (word, lowercase, whitespace, field, trigram) plus opt-in language-specific tokenizers (gse, gse_ch, kagome_ja, kagome_kr) enabled via environment variables. Custom user-defined tokenizers are not currently exposed; the focus is a small set of well-tuned options plus a flexible textAnalyzer config layer (accent folding, stopword presets).'
...
Weaviate exposes four general-purpose tokenization methods that cover the vast majority of production use cases (a fifth, `trigram`, is also built-in and is covered separately below for fuzzy matching):
...
[Stopwords](https://docs.weaviate.io/weaviate/concepts/indexing/inverted-index#stop-words) are short, high-frequency words (`the`, `a`, `and`) that BM25 typically drops to make scoring more robust. Many search stacks default to one stopword list per language and apply it to every text field. That works until you have a brand called _"The North Face"_, and dropping `the` quietly destroys recall on the brand.
