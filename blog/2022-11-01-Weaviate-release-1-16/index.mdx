...
For more information on how to use the `IsNull` filter, check out the [documentation](/docs/weaviate/api/graphql/filters#filter-by-null-state).
...
For more information on how to filter by the property length, check out the [documentation](/docs/weaviate/api/graphql/filters#filter-by-property-length).
...
Check the [ref2vec-centroid](/docs/weaviate/modules/retriever-vectorizer-modules/ref2vec-centroid) documentation to learn how to work with Ref2Vec.
...
Check the [/v1/nodes](/docs/weaviate/api/rest/nodes) documentation to learn how to use the `Node Status API`.

## More powerful OIDC Auth
![More powerful OIDC Auth](./img/oidc.png)

Security is very, very important. And let's face it - nobody wants to deal with yet another password. That's why we support OpenID Connect (OIDC) [authentication](/docs/weaviate/configuration/authentication) in Weaviate, which users experience as a convenient and secure single sign-on. 
...
* [Authentication](/docs/weaviate/configuration/authentication)
* [Authorization](/docs/weaviate/configuration/authorization)
* [Environment variables](/docs/weaviate/installation/docker-compose#environment-variables)
