...
For more information on how to use the `IsNull` filter, check out the [documentation](/developers/weaviate/current/graphql-references/filters.html#filter-by-null-state).
...
For more information on how to filter by the property length, check out the [documentation](/developers/weaviate/current/graphql-references/filters.html#filter-by-property-length).
...
<!-- TODO: The video doesn't work find a way to fix it. -->
<!-- Might need to do a static image load trick -->
<!-- MAKE sure to check other <img> and <video> across blogs -->
<!--<video width="100%" autoplay loop controls>
  <source src={require('./img/ref2vec-centroid-demo.mp4')} type="video/mp4" />
...
Check the [ref2vec-centroid](/developers/weaviate/current/retriever-vectorizer-modules/ref2vec-centroid.html) documentation to learn how to work with Ref2Vec.
...
Check the [/v1/nodes](/developers/weaviate/current/restful-api-references/nodes.html) documentation to learn how to use the `Node Status API`.

## More powerful OIDC Auth
![More powerful OIDC Auth](./img/oidc.png)

Security is very, very important. And let’s face it - nobody wants to deal with yet another password. That’s why we support OpenID Connect (OIDC) [authentication](/developers/weaviate/current/configuration/authentication.html) in Weaviate, which users experience as a convenient and secure single sign-on. 
...
* [Authentication](/developers/weaviate/current/configuration/authentication.html)
* [Authorization](/developers/weaviate/current/configuration/authorization.html)
* [Environment variables](/developers/weaviate/current/installation/docker-compose.html#environment-variables)
...
You can find the complete list and the relevant links in the [release notes](https://github.com/semi-technologies/weaviate/releases/tag/v1.16.0).
