...
Published 2024-03-21 as a *beta* of the v3 TypeScript client. Several API names in this post changed before GA (e.g. `weaviate.configure.vectorizer.*` became `weaviate.configure.vectors.*`, and `weaviate.configure.dataType.TEXT` was renamed). For the current API, see the [TypeScript client docs](https://docs.weaviate.io/weaviate/client-libraries/typescript/typescript-v3).
:::
