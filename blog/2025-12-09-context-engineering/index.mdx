...
tags: ['concepts']
image: ./img/hero.png
description: 'Context engineering is the act of selecting, organizing, and managing the information fed into a large language model during inference (i.e. the “context” tokens) to optimize its performance and behavior.'
---
...
![prompt versus context engineering](./img/prompt-vs-context-engineering.png)
...
![common failure modes](./img/common-failure-modes.png)
...
An [**AI agent**](https://weaviate.io/blog/ai-agents) is a system that uses a large language model (LLM) as its “brain” to do decision-making and solve complex tasks. The user supplies a goal, and the agent figures out the steps to achieve it, using available tools in its environment. 
...
![where agents fit in context engineering](./img/agents-in-context-eng.png)
...
![how the query agent works](./img/query-agent.png)
...
![guide to choosing a chunking strategy](./img/chunking-guide.png)
...
:::info
We cover these questions, complete with code examples, in this deep-dive blog post: [Chunking Strategies to Improve Your RAG Performance](https://weaviate.io/blog/chunking-strategies-for-rag#pre-chunking-vs-post-chunking)
:::
...
![prompting techniques overview](./img/prompting-techniques.png)

There's a whole toolkit of prompting techniques you can choose from. But to go from theory to production, you need to know not just *what* these frameworks are, but *how* and *when* to implement them. In our [Context Engineering eBook](https://weaviate.io/ebooks/the-context-engineering-guide?utm_source=blogs&utm_campaign=context-engineering&utm_content=comment), you’ll learn how to implement simple techniques like Chain-of-Thought (CoT) and more advanced ones like the ReAct framework to build systems that can tackle complex, multi-step tasks with reliability.
...
![memory types in agentic systems](./img/memory-types-agentic-systems.png)
...
As you can see, orchestration happens through this powerful feedback loop, often called the Thought-Action-Observation cycle. The agent constantly observes the outcome of its action and uses that new information to fuel its next "thought". This Thought-Action-Observation cycle forms the fundamental reasoning loop in modern agentic frameworks like our own [Elysia](https://weaviate.io/blog/elysia-agentic-rag). 
