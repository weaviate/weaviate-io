...
This article will explain some of their core concepts, such as [vector embeddings](/blog/vector-embeddings-explained) and [vector search](/blog/vector-search-explained). Then, we will dive into the technical details of using distance metrics in vector search and how [vector indexes](https://weaviate.io/developers/weaviate/concepts/vector-index) enable efficient retrieval. Once we have a fundamental understanding, we will discuss use cases and the current tool landscape.
...
Below you can see an example of the [Weaviate vector database architecture](https://weaviate.io/developers/weaviate/concepts#weaviate-architecture).
