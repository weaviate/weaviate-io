...
[![Combining LangChain and Weaviate](./img/hero.png)](/blog/2023-02-21-combining-langchain-and-weaviate/index.mdx)
...
## Tool Use
The last building block we will cover is tool use. [Tool use](https://langchain.readthedocs.io/en/latest/modules/agents/tools.html?highlight=tool%20use) is a way to augment language models to use tools. For example, we can hook up an LLM to search engines, calculators, or even code executors. Of course we will dive into the search engines next, but let’s start with an example of the code executor tool use.
...
We put the chat_history and the latest user input in the curly bracket syntax. Then we take this new query and hit the Weaviate semantic search engine to get context to answer the question. The ChatVectorDB chain we use has a default value of k = 4 search results, if we use longer search results we will need another CombineDocuments chain here as well! With the 4 search results, we answer the question with this final prompt:
...
If this is your first time using Weaviate, please check out the [Quickstart tutorial](/developers/weaviate/quickstart/end-to-end).
