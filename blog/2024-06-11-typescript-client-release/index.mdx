...
image: ./img/hero.png
description: "We're pleased to share that the Weaviate"
---

![The Weaviate TypeScript client stable release](./img/hero.png)


About two months ago we released the [beta version](https://weaviate.io/blog/typescript-client-beta#providing-your-own-generics) of the Weaviate v3 Typescript client. After a successful beta, today we’re happy to share that the v3 Weaviate Typescript client is stable. 
...
As highlighted previously, following the release of a new Python client, we felt it necessary to dedicate time to serve the Javascript community. The v3 client is what came out; with a faster gRPC powered API, first class Typescript support and numerous quality of life improvements! Don't just take our word for it. We've had a wonderful group of early adopter building real applications and solving problems.

:::info Developer Experience First

"I’ve been enjoying the v3 client as it nears release! Lots of care taken with matching the developer mental model, and developer experience if you use TypeScript. We’ve been migrating a production system and all the love and support from the Weaviate developers helps me sleep at night!" 
...
Since the beta release, besides squashing bugs, we’ve got a lot of feedback on what we could do better and have changed the functionality of a few things. Namely
...
As seen above, `sourceProperties` lets you define what properties you want your vector to be created from.
...
Should you want to avoid any confusion with the default v3 import, we recommend renaming the import. This might be useful as you migrate your code base or test out v3 in a v2 project. 
...
const config = await myCollection.config.update({
  vectorizers: [vectorizer.update({
    ...
    vectorIndexConfig: vectorIndex.hnsw({
      quantizer: vectorIndex.quantizer.pq(),
      ef: 4,
    }),
  })],
})
...
### Migrating your client

To migrate your application from a beta or RC version of the client, run the following command.
...
To support you usage, we have the put together the following resources
...
The Weaviate v3 client was built for server side applications. Given this constraints this brings for people wanting a client side package, we found it fitting to keep supporting the v2 client until we have a web version out.

We will support both the v2 and v3 clients and ship new Weaviate features to both. When this commitment to support both clients changes, we've be sure to communicate it in a timely manner. Our  plan is to launch a web version of the client and gracefully sunset the v2 client.
...
We're going to focus our energy on the following in the new few months.

- Improving client error messages 
- Releasing a web version of of the v3 Typescript client 
...
We're really excited to share this with everyone and cannot wait to see what you all build. We've already seen so much interest in the client and want to support builders in anyway we can. 
