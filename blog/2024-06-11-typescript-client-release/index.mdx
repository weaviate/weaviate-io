...
About two months ago we released the [beta version](https://weaviate.io/blog/typescript-client-beta) of the Weaviate v3 Typescript client. After a successful beta, today we’re happy to share that the v3 Weaviate Typescript client is stable. 
...
The Weaviate v3 client was built for server-side applications. Given the constraints this brings for people wanting a client-side package, we found it fitting to keep supporting the v2 client until we have a web version out.

We will support both the v2 and v3 clients and ship new Weaviate features to both. When this commitment to support both clients changes, we've be sure to communicate it promptly. Our plan is to launch a web version of the client and gracefully sunset the v2 client.
