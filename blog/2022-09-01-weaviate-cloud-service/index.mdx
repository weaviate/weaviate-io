...
Weaviate, the leader in open-source vector database technology, announces an [entirely new pricing model](/pricing) for its Weaviate Cloud.
...
In addition to remarkably convenient access to the latest vector database capabilities, Weaviate Cloud customers get dedicated customer support on a private Slack channel. However, consistent with Weaviate's commitment to creating truly open-source software, customers using the free service will always be able to access all of the Weaviate's vector database capabilities.
...
Customers interested in the Weaviate Cloud can purchase service for $0.05 per million dimensions. Or, just one dollar for 20 million dimensions.
