...
1. **Flat vector index + Binary Quantization** - for multi-tenancy use cases.
...
Similarly to with HNSW indexes, the vector cache can be used to to improve query performance by storing vectors of the most recently used data objects. Note that it must be balanced with memory usage considerations.
...
* You can now enable an option to [auto-limit available resources](/developers/weaviate/concepts/resources.md#limit-available-resources) in Weaviate. In applicable systems, you can set the `LIMIT_RESOURCES` [environment variable](/developers/weaviate/config-refs/env-vars.md).
