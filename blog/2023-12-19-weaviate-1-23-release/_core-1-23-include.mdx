...
* Read more about the `flat` index [here](https://weaviate.io/developers/weaviate/concepts/vector-index#flat-index).
...
With the `1.23` release, it is easier to use Weaviate with many open-source large language models (LLMs) such as Llama2-70b, CodeLlama-34b or Mistral-7B-Instruct. This is made possible by the [generative-anyscale](https://weaviate.io/developers/weaviate/model-providers/anyscale/generative) module.
...
* Read more about the `generative-anyscale` module [here](https://weaviate.io/developers/weaviate/model-providers/anyscale/generative).
...
* Read more about the `Python` client `v4` [here](https://weaviate.io/developers/weaviate/client-libraries/python).


## Performance improvements

* [Lazy shard loading](https://weaviate.io/developers/weaviate/concepts/data#lazy-shard-loading) allows you to start working with your data sooner. After a restart, shards load in the background. If the shard you want to query is already loaded, you can get your results right away. If the shard is not loaded yet, Weaviate prioritizes loading that shard and returns a response when it is ready.

* You can now enable an option to [auto-limit available resources](https://weaviate.io/developers/weaviate/concepts/resources#limit-available-resources) in Weaviate. In applicable systems, you can set the `LIMIT_RESOURCES` [environment variable](https://weaviate.io/developers/weaviate/config-refs/env-vars).

## Minor changes

The [`nodes` endpoint](https://weaviate.io/developers/weaviate/config-refs/nodes) can be used to output information about the nodes in your cluster.
