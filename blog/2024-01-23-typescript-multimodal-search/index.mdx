...
In the newly created folder, create a `docker-compose.yml` file and paste the following code in it. 
...
Next, we will create a Weavaite collection and import data into it. In your `./public` folder, create three folders called *image*, *audio*, and *video*. These will store media corresponding to their folder names respectively. You can then add your own data or [use the data I added](https://github.com/malgamves/next-multimodal-search-demo/tree/main/public) to build out the original application. 
...
To import our data, we need to create a folder called `src` were our import scripts will go. 
...
This file conatins a function to create a collection, a function to delete a collection, and a function to check if a collection exists. If there are any details we want to change that would define our collections, we can do so in this file.
...
This file reads out image, video and audio directories for media and then encodes them before sending them to Weaviate to be stored.
...
In `index.ts` we can change our collection name to our liking. When this file is run, we delete any exsting collection that exists with the name we specify, create a new collection with the same name and then import all the media files in our `.public` folder.

> It's important to not that we need our Weaviate instance running with Docker before going to the next step. 


To run our import proces, we need to add the following scripts to our `package.json` file.
...
We can now run `yarn run import` to start the import process. Depending on how much data you're importing, it can take a little time. While that run, lets create our Search interface.
...
Here we define a function `searchDB()`, that makes a call to Weaviate to search for the given search term. We then map the results to the UI. Weaviate lets us run a vector search from our `BindExample` collection with `withNearText()` and pass the user entered search term as a query. We have the ability to define properties we want to search on with `withFields()` and the number of results we want to return with `withLimit()`. We then pass the query response to be displayed on the UI.
