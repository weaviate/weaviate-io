...
export const getWeaviateClient = async () => {
  if (!client) {
    ...
    headers: {
      'X-Goog-Vertex-Api-Key': process.env.GOOGLE_API_KEY || '',  // Replace with your inference API key
    }
  }
)
  };
...
export const createCollection = async (name: string) => {
  ...
  const newCollection = await client.collections.create({
    name: name,
    vectorizers: weaviate.configure.vectors.multi2VecGoogle({
...
Notably, we create a new collection and point the `multi2vec-google` module at our `image` property, so Weaviate embeds the images themselves. The module also accepts `textFields` and `videoFields` if you want to vectorize more than one modality. We also add the following properties to our collection: `name` and `image`. Think of these as properties we will be able to query against when we start to make queries or import data to Weaviate.
...
export const importMediaFiles = async (collectionName: string) => {
    ...
    const vertexCollection = client.collections.use(collectionName); 
...
export async function vectorSearch(searchTerm: string) {
  const client = await weaviate.connectToWeaviateCloud(process.env.WEAVIATE_URL || '',{
    authCredentials: new weaviate.ApiKey(process.env.WEAVIATE_API_KEY || ''),
    headers: {
      'X-Goog-Vertex-Api-Key': process.env.GOOGLE_API_KEY || ''
    }
  },
);
    
const myCollection = client.collections.use('PalmMultimodalSearch');
