...
Before you do, change the `image: semitechnologies/weaviate:1.16.0` line in that `docker-compose.yml` to a `1.36` or newer release. The import step later in this post uses server-side batching, which the pinned version does not support.
...
Server-side batching needs Weaviate `1.36`+ and Python client `4.20`+, but the `nearest-neighbor-dog-search` demo you cloned still pins `semitechnologies/weaviate:1.16.0` in its `docker-compose.yml`.
:::
