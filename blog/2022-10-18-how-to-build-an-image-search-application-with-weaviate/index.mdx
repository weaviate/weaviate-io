...
Published 2022-10-18. Code samples use the pre-v4 Python client and the `img2vec-neural` module. For current guidance on image search in Weaviate, see the [multimodal guide](/blog/multimodal-guide), which covers the `multi2vec-*` model family that supersedes single-modality image embedding.
:::
