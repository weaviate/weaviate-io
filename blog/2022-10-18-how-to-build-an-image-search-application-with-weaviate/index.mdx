...
When uploading your data objects to Weaviate, you want to import your data in batches. The import speed is faster than uploading the objects one by one. In the v4 client we use a dynamic batch context manager on the collection, which automatically tunes the batch size for us.

```python
dog_collection = client.collections.use("Dog")

with dog_collection.batch.dynamic() as batch:
    # add objects here
    ...
```
