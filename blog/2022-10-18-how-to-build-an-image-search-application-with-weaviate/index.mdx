...
Before you do, change the `image: semitechnologies/weaviate:1.16.0` line in that `docker-compose.yml` to a `1.38.8` or newer release. The import step later in this post uses server-side batching, which the pinned version does not support.
...
We will tell Weaviate that breed and filepath values are text, weight is stored as an integer, and images are stored as a `blob` dataType. Putting it all together, the collection definition should look like the following;
...
client.collections.create(
    ...
    properties=[
        Property(name="breed", data_type=DataType.TEXT, description="name of dog breed"),
        Property(name="image", data_type=DataType.BLOB, description="image"),
        Property(name="filepath", data_type=DataType.TEXT, description="filepath of the images"),
    ],
)
```
...
When uploading your data objects to Weaviate, you want to import them in batches rather than one at a time. We recommend [server-side batching](https://docs.weaviate.io/weaviate/manage-objects/import#server-side-batching): the server tells the client how fast to send data based on its own load, so there is no batch size for you to pick and no tuning to redo when the dataset grows.

The entry point is the `batch.stream()` context manager. You add objects to it one at a time and the client sends them at whatever pace the server asks for:

```python
dog_collection = client.collections.use("Dog")

with dog_collection.batch.stream() as batch:
    batch.add_object(properties={"breed": "corgi", "filepath": "corgi.jpg"})
```

Base64-encoded images are large, so we will feed that context manager from a generator. Objects then go to the server as the generator produces them, and the whole dataset never has to sit in memory at once.

:::caution Bump the demo repo's Weaviate version first
Server-side batching needs Weaviate `1.36`+ and Python client `4.20`+, but the `nearest-neighbor-dog-search` demo you cloned still pins `semitechnologies/weaviate:1.16.0` in its `docker-compose.yml`. Bump that line to a `1.38.8` or newer release before you run the import.
:::
...
def weaviate_img_search(img_str):
   ...
   response = dog_collection.query.near_image(
       near_image=img_str,
       limit=2,
