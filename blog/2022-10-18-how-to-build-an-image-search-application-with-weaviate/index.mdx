...
When uploading your data objects to Weaviate, you want to import your data in batches. The import speed is faster than uploading the objects one by one. We use a [server-side batching](https://docs.weaviate.io/weaviate/manage-objects/import#server-side-batching) context manager on the collection — the server tells the client how fast to send data, applying backpressure so you don't have to tune batch size or concurrency by hand.
