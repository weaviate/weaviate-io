...
The original `ChatVectorDBChain` class shown here has been removed from LangChain; the current Weaviate integration lives in the `langchain-weaviate` package. See the [Weaviate-LangChain integration post](/blog/enterprise-workflow-langchain-weaviate) for an up-to-date example.
