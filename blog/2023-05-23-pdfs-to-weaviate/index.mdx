...
client = weaviate.connect_to_embedded(
    version="1.36.0",  # server-side batching needs Weaviate 1.36 or newer
    headers={"X-OpenAI-Api-Key": os.environ["OPENAI_APIKEY"]},
)
```

:::note Requires Weaviate `1.36`+ and Python client `4.20`+
The import step below uses `batch.stream()`, which hands batching to the server. That arrived in Weaviate `1.36` and in the Python client `4.20`. Embedded Weaviate still defaults to an older build, which is why the version is pinned above. On an older server, use `batch.dynamic()` instead.
:::
...
```python
documents = client.collections.use("Document")

with documents.batch.stream() as batch:
...
```
paper02.pdf 0.8450042
paper01.pdf 0.26854637
```
...
response = documents.generate.fetch_objects(single_prompt=prompt)

for obj in response.objects:
    source = obj.properties["source"]
    abstract = obj.generative.text
...
We tested a few queries above, but we can take this one step further by using [LangChain](https://python.langchain.com/docs/integrations/vectorstores/weaviate/). Once the documents are imported into Weaviate, you can build a simple chatbot to chat with your pdfs by using LangChain’s vectorstore.

:::note LangChain has moved on since 2023
`ChatVectorDBChain`, which the original version of this snippet used, has been removed from LangChain, and the Weaviate vectorstore now lives in the separate `langchain-weaviate` package. The snippet below has been updated to match. For a current end-to-end example, see the [Weaviate and LangChain post](/blog/enterprise-workflow-langchain-weaviate).
:::
