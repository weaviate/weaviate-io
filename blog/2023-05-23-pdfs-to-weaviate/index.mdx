...
Published 2023-05-23. Code samples use the pre-v4 Python client and the now-removed LangChain `ChatVectorDBChain`. For current guidance on ingesting PDFs into Weaviate (now via multimodal embeddings — no OCR step required), see the [multimodal guide](/blog/multimodal-guide).
:::
