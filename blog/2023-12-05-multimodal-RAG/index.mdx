---
title: Multimodal Retreival Augmented Generation(RAG)
slug: multimodal-RAG
authors: zain
date: 2023-12-05
image: ./img/hero.png
...
Not only do humans capture and use multimodal representations of information but given a task we can also incorporate any of these modalities as context to help us guide our answers. Imagine being tasked with explaining a recipe for your favorite dish, you could write me a detailed step by step process or you coil Explain normal rag and then multimodal rag
...
One way to train a model that understands multimodal data including images, audio, video, and text is to first train individual models that understand each one of these modalities separately and then unify their representations of data using a training process called contrastive training.
...
For a more detailed breakdown refer to the complete (notebook)[https://github.com/weaviate-tutorials/multimodal-workshop/blob/main/2-multimodal/1-multimedia-search-complete.ipynb].
...
However, the issue with the current RAG workflows is that they only leverage retrieved text source material. This is because most LLMs only understood language and so any information that's retrieved had to be in text format … until now! 

Recently, there have been a group of large generative models, both closed and open source, that understand both text and images and as a result, we can now support multimodal rag where we can retrieve images from our vector database and pass those into the large multimodal model(LMM) to then generate using. This simple two-step process, illustrated below, is the main idea behind Multimodal-RAG.

![mmRAG_Dark](./img/mmRAG_D.png#gh-dark-mode-only)
![mmRAG_Light](./img/mmRAG_D.png#gh-light-mode-only)
*Shows the two step process of MM-RAG, involving retrieval from a multimodal knowledge base and then generation using a large multimodal model by grounding in the retreived context.*

MM-RAG was (presented earlier this year)[https://arxiv.org/abs/2211.12561] from a group at Stanford. They showed a workflow of multimodal models, one that could retrieve and another that could generate both text and images.
...
![puppy](./img/puppy.png)
*Retreived image from vector database*
...
Currently, GPT4-V cannnot produce images so to generate an image from the above description we will use the new DALL-E-3 model instead:
