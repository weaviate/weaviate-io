...
client.collections.create(
    name="Animals",
    vector_config=wvc.config.Configure.Vectors.multi2vec_bind(
        audio_fields=["audio"],
        image_fields=["image"],
        video_fields=["video"],
    )
...
animals = client.collections.use("Animals")
...
![mmRAG_Light](./img/mm_RAG_L.png#gh-light-mode-only)
...
def retrieve_image(query):
    response = animals.query.near_text(
        query=query,
        filters=wvc.query.Filter.by_property("mediaType").equal("image"),
