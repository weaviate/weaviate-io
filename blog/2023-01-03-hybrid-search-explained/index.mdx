...
The [Hybrid search](/developers/weaviate/api/graphql/search-operators#hybrid) feature was introduced in Weaviate 1.17. It uses sparse and dense vectors to represent the semantic meaning and context of search queries and documents.
...
For a deep dive into how Weaviate's hybrid search combines sparse and dense vectors, you can read more on these [fusion algorithms in our blog post](https://weaviate.io/blog/hybrid-search-fusion-algorithms).
...
* `query`: search query
* `alpha` (optional): weighting for each search algorithm (alpha = 0 (sparse, keyword search), alpha = 1 (dense, vector search), alpha = 0.5 (equal weight for sparse and dense vectors))
