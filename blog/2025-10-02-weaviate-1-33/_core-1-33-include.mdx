...
Weaviate `v1.33` is now available open-source and on [Weaviate Cloud](https://console.weaviate.cloud). This release brings **compression by default** for optimal resource utilization, **1-bit rotational quantization (RQ)**, **server-side batch imports**, **OIDC group management**, and **collection aliases** become generally available (GA).
