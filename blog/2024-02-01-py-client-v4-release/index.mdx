...
`insert_many` sends one request for the entire function call. For requests with a large number of objects, [consider using `batch` imports](/developers/weaviate/client-libraries/python#batch-imports).
:::
