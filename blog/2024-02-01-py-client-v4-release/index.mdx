...
The `v4` release of the Weaviate Python client library has landed, with a brand new API that is more powerful and easier to use 🎉🥳. This is a major update, as reflected by the bump in the major version number.
...
The speed improvements during imports will increase along with the request size. As some rough examples, a 50s import of `ANN` benchmark dataset with the `v3` client will take ~30s with the `v4` client,

Queries will also be faster with the `v4` client.

Anecdotally, a query on an HNSW index with an `ef` of 16 that had a QPS (queries per second) of 162 with the `v3` client had a QPS of 1560 with the `v4` client. Another query with an `ef` of 256 had a QPS of 110 with the `v3` client, and a QPS of 193 with the `v4` client. (This is a function of how long each query takes server-side, as that is not affected by the client improvements.)

These are just some rough examples, but we think many of you will see noticeable speed improvements across the board.
