...
image: ./img/hero.png
tags: ["concepts"]
description: "Understand the distance metrics that power similarity search: cosine similarity, dot product, Euclidean, Manhattan, and Hamming. Learn how to choose the right metric for your vector search application."
---
...
_Note_, the meaning of each value in the array, depends on what machine learning model we use to generate them.

To judge how similar two objects are - the core problem in similarity search - we can compare their vector values using various **distance metrics**.
...
In practice, distance metrics are what make semantic search and similarity search possible. When users perform a semantic search—such as asking a question in natural language or searching by intent rather than exact keywords—the system compares the vector embedding of the query against stored embeddings and ranks results based on their vector distance. Although terms like vector search, similarity search, and semantic search are often used interchangeably, the distinction is useful: semantic search describes the user experience, while distance metrics define the mathematical mechanism that determines which results are most similar and therefore most relevant.

> Distance Metrics convey how similar or dissimilar two vector embeddings are. They are the mathematical foundation of vector similarity search.
...
_The above image is a visual representation of a vector space. To perform a search, your search query is converted to a vector - similar to your data vectors. The vector database then computes the similarity between the search query and the collection of data points in the vector space._
