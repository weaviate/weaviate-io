...
![Summarized documents are easier to understand](./img/Weaviate-summarize_light.png#gh-light-mode-only)![Summarized documents are easier to understand](./img/Weaviate-summarize.png#gh-dark-mode-only)
...
Even when looking only at language models that are trained for summarization task, there is still a wide range of choices in terms of sheer numbers, which vary in the target domain (e.g. medical, legal, scientific, etc.) and size (number of parameters, i.e. speed). If you have specific needs, we recommend investigating other models. 
