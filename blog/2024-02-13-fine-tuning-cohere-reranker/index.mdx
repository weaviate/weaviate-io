...
client = weaviate.connect_to_weaviate_cloud(
    cluster_url="WEAVIATE_URL",  # Replace with your cluster url
    auth_credentials=Auth.api_key("WEAVIATE_AUTH_KEY"),  # Replace with your Weaviate instance API key
    headers={
...
`client.collections.delete_all()` or `client.collections.delete("Blogs")`
...
command_nightly = dspy.Cohere(model="command-r-plus", max_tokens=1000, api_key=cohere_key)

blogs_collection = client.collections.use("Blogs")
...
for blog_chunk in blogs:
    ...
    blogs_collection.data.insert(properties=data_properties)
```
...
out of Weaviate and into a JSON file using the v4 iterator API!
'''
import time
start = time.time()
...
total_objects = blogs_collection.aggregate.over_all(total_count=True).total_count

# Iterate through every object in the collection
data = []
for obj in blogs_collection.iterator():
    synthetic_query = obj.properties.get("synthetic_query", "")
    if len(synthetic_query) < 5:
        continue
    data.append({
        "query": synthetic_query,
        "relevant_passages": [obj.properties["content"]],
    })
...
```python
blogs_fine_tuned = client.collections.use("BlogsFineTuned")

with blogs_fine_tuned.batch.stream() as batch:
    for blog in blogs:
        batch.add_object(properties={"content": blog})
```
...
response = blogs_fine_tuned.query.near_text(
    query="Ref2Vec in Weaviate",
    limit=5,
)
...
Weaviate 1.16 introduced the Ref2Vec module. In this article, we give you an overview of what Ref2Vec is and some examples in which it can add value such as recommendations or representing long objects. The name Ref2Vec is short for reference-to-vector, and it offers the ability to vectorize a data object with its cross-references to other objects. The Ref2Vec module currently holds the name ref2vec-centroid because it uses the average, or centroid vector, of the cross-referenced vectors to represent the referencing object.

---

In other words, the User vector is being updated in real-time here to take into account their preferences and actions, which helps to produce more relevant results at speed. Another benefit of Ref2Vec is that this calculation is not compute-heavy, leading to low overhead. With Ref2Vec, you can use Weaviate to provide Recommendation with "user-as-query". This is a very common and powerful way to build Home Feed style features in apps.

---

![Cross-reference](./img/Weaviate-Ref2Vec_1.png)

Ref2Vec gives Weaviate another way to vectorize a class, such as the User class, based on their relationships to other classes. This allows Weaviate to quickly create up-to-date representations of users based on their relationships such as recent interactions.

---

The following image depicts how Ref2Vec aggregates the representations of 3 Product items to represent a User who has purchased a pair of boots, shorts, and Weaviate t-shirt! Such a representation of the User, by an aggregation of their cross-references, allows Weaviate to conveniently and immediately learn from each User's preferences and actions to provide improved and up-to-date characterizations.

---

## More Coming Soon
We are very excited about the potential of Ref2Vec, and how it leverages existing symbolic data to augment vector searches in a new way. Although Weaviate puts Machine Learning data representations first, this doesn't mean we discard symbolic data and many features of previous systems. Rather, we are actively searching for how symbolic data can improve neural functionality and vice versa.
```
...
response = blogs_fine_tuned.query.near_text(
    ...
    rerank=Rerank(
        prop="content",
        query="Represent users based on their product interactions",
    ),
)

for obj in response.objects:
    print(obj.metadata.rerank_score, obj.properties["content"])
```
...
date: 2022-11-23
---
Weaviate 1.16 introduced the Ref2Vec module. The name Ref2Vec is short for reference-to-vector, and it offers the ability to vectorize a data object with its cross-references to other objects.

---

4.7478566e-06
## More Coming Soon
We are very excited about the potential of Ref2Vec, and how it leverages existing symbolic data to augment vector searches in a new way. Although Weaviate puts Machine Learning data representations first, this doesn't mean we discard symbolic data and many features of previous systems.
```
