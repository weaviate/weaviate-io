...
In version `1.20`, Weaviate introduced the ability to run reranking at the second stage. To get started, Weaviate developers simply need to add one line to their collection definition, `reranker-cohere`:
...
client.collections.create(
    ...
    reranker_config=Configure.Reranker.cohere(model="rerank-multilingual-v2.0"),
)
```
...
:::note Newer Cohere rerank models are available
The model names in this post are the ones available in February 2024, and the scores further down were measured with them. Cohere has shipped newer rerank models since. See the [Cohere reranker docs](https://docs.weaviate.io/weaviate/model-providers/cohere/reranker) for the current list.
:::
...
:::caution The DSPy API has changed
`dspy.Cohere` no longer exists in current DSPy releases; language models are configured through `dspy.LM` and `dspy.configure` instead. The snippet below is left as it was written and run in February 2024, because the prompt capture and the generations that follow came from it. Check the [DSPy documentation](https://dspy.ai/) for the current syntax.
:::
...
blogs_fine_tuned.data.ingest({"content": blog} for blog in blogs)
```

:::note Requires Weaviate `1.36`+ and Python client `4.20`+
`data.ingest()` hands the batching to the server, so you do not size batches yourself. It arrived in Weaviate `1.36` and in the Python client `4.20`. On an older server, use `batch.dynamic()` instead.
:::
...
for i, obj in enumerate(response.objects, 1):
    print(f"--- Object {i} (rerank score: {obj.metadata.rerank_score}) ---")
    print(obj.properties["content"])
```

##### Output
````text
--- Object 1 (rerank score: 0.9261703) ---
![Cross-reference](./img/Weaviate-Ref2Vec_1.png)

Ref2Vec gives Weaviate another way to vectorize a class, such as the User class, based on their relationships to other classes. This allows Weaviate to quickly create up-to-date representations of users based on their relationships such as recent interactions. If a user clicks on 3 shoe images on an e-commerce store, it is a safe bet that they want to see more shoes. Ref2Vec captures this intuition by calculating vectors that aggregate each User's interaction with another class. The below animation visualizes a real example of this in e-Commerce images.

--- Object 2 (rerank score: 0.34444344) ---
The following image depicts how Ref2Vec aggregates the representations of 3 Product items to represent a User who has purchased a pair of boots, shorts, and Weaviate t-shirt!
...
--- Object 3 (rerank score: 0.007121429) ---
In other words, the User vector is being updated in real-time here to take into account their preferences and actions, which helps to produce more relevant results at speed. Another benefit of Ref2Vec is that this calculation is not compute-heavy, leading to low overhead. With Ref2Vec, you can use Weaviate to provide Recommendation with "user-as-query". This is a very common and powerful way to build Home Feed style features in apps. This can be done by sending queries like this to Weaviate:
...
--- Object 4 (rerank score: 5.5508026e-06) ---
---
title: What is Ref2Vec and why you need it for your recommendation system
slug: ref2vec-centroid
authors: [connor]
...
--- Object 5 (rerank score: 4.7478566e-06) ---
## More Coming Soon
We are very excited about the potential of Ref2Vec, and how it leverages existing symbolic data to augment vector searches in a new way. One of my favorite articles about Weaviate is Bob van Luijt's ["The AI-First Database Ecosystem"](/blog/semantic-search-with-wikipedia-and-weaviate). In this article, Bob describes emerging **waves** of database technology; from SQL, to NoSQL, and now, AI-first databases that focus "on data that is processed by a machine learning model first, where the AI models help in processing, storing and searching through the data as opposed to traditional ways". Although Weaviate puts Machine Learning data representations first, **this doesn't mean we discard symbolic data and many features of previous systems**. Rather, we are actively searching for how symbolic data can improve neural functionality and vice versa.
````
