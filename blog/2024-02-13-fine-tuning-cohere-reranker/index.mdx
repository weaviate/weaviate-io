...
Then in [December of 2023](https://txt.cohere.com/rerank-fine-tuning/), Cohere announced the ability to fine-tune their rerank endpoint. Fine-tuning boosts the model’s performance, especially in unique domains. Here is a screenshot of the [pricing page](https://cohere.com/pricing).

![Pricing](img/pricing.png)

In this demo, we’re fine-tuning the model on a dataset of Weaviate blogs and synthetic queries. This will improve the performance of the search system by narrowing the focus to this particular information.

## Demo

:::info
You can clone the repository [here](https://github.com/weaviate/recipes/tree/main/reranking/cohere-ranking/finetune-reranker) to follow along!
:::
...
:::caution
The total cost for this demo was $0.03 — this includes the vectorization and rerank inference. 
:::
...
client = weaviate.Client(
    ...
    auth_client_secret=weaviate.AuthApiKey(api_key="WEAVIATE_AUTH_KEY"),  # Replace w/ your Weaviate instance API key
    additional_headers = {
...
for idx, blog_chunk in enumerate(blogs):
    if idx > 400:
        break
    print(idx)
