...
Another challenge is that any write request that comes in while any nodes are down will be temporarily missing on that node for a while. This will be repaired in time through a read-repair that [happens automatically in the background](https://weaviate.io/developers/weaviate/concepts/replication-architecture/consistency#repairs).
...
But we think that the cost of high availability is worth these prices. We take system availability seriously, and architect Weaviate according to this philosophy. This is one of the reasons that we use [leaderless replication](https://weaviate.io/developers/weaviate/concepts/replication-architecture/cluster-architecture#data-replication-leaderless), and why replication in the first place is so important to us - because it enables our users to have robust systems on which they can rely.
