---
title: Achieve Continuous Uptime with Weaviate’s Multi-Node Setup
slug: achieve-continuous-uptime
authors: [etienne,jp]
date: 2023-11-13
# image: ./img/hero.png
...
### Replication on Weaviate Cloud Services (WCS)
...
### Replication on self-deployed Weaviate

Enabling replication in a self-deployment setup such as a Docker or Kubernetes setup involves the following two steps. First, modify the configuration file to enable a multi-node setup (e.g. 3), and add the `replicationConfig` setting to the class definition like so:
...
Edit `values.yaml` by changing the root-level configuration `replicas: 1` to `replicas: 3`, and save it.
...
Just like that, you’ve got a multi-node Weaviate cluster. Remember that when you create a class, you must have replication enabled. You can do this by adding the `replicationConfig` parameter to the class definition, like so:
...
As we mentioned before, all you need to configure to enable replication is this in the class definition:
