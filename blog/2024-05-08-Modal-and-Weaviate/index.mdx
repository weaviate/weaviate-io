...
### Batch Imports
When importing data into Weaviate, we suggest using batch imports. This will send multiple objects in a single request, rather than importing them individually. There are [four methods](https://docs.weaviate.io/weaviate/manage-objects/import#server-side-batching) to choose from:

1. `stream` (**recommended**): [Server-side batching](https://docs.weaviate.io/weaviate/manage-objects/import#server-side-batching). The server tells the client how fast to send data based on its own current workload, applying backpressure so you don't have to tune batch size or concurrency by hand.

```python
with client.batch.stream() as batch:
```

2. `dynamic`: Client-side dynamic batching. The batch size and number of requests are adjusted as you import data, based on observed server load.

```python
with client.batch.dynamic() as batch:
```

3. `fixed_size`: The user specifies the batch size and number of requests.
```python
with client.batch.fixed_size(batch_size=100, concurrent_requests=4) as batch:
```

4. `rate_limit`: If there is a third-party vectorization rate limit you want to avoid, you can specify the number of objects to send per minute.
