...
While async indexing helped with importing, [product quantization](https://weaviate.io/developers/weaviate/concepts/vector-quantization#product-quantization) (PQ) was important for alleviating the memory footprint. PQ is a technique used to compress the size of vector embeddings by representing the vectors as centroid ids. The centroid ids are calculated by clustering each segment. We enabled PQ to reduce the memory requirements without slowing down the ingestion.

You can either [manually](https://weaviate.io/developers/weaviate/configuration/compression/pq-compression#manually-configure-pq) enable PQ or have it [automatically triggered](https://weaviate.io/developers/weaviate/configuration/compression/pq-compression#configure-autopq) based on the number of vectors. You can manually enable PQ by setting it at the collection level. You’ll first need to import at least 10,000 to 100,000 vectors in order to form the centroids. AutoPQ can be used for new collections and the training step to form the centroids is set at 100,000 vectors by default.
...
Since our application didn’t require high recall, we opted in to save costs by enabling PQ. There are a few [PQ parameters](https://weaviate.io/developers/weaviate/config-refs/schema/vector-index#pq-configuration-parameters) that you can configure like `trainingLimit`, `segments`, and `distribution`. Additionally, you can configure the [HNSW parameters](https://weaviate.io/developers/weaviate/config-refs/schema/vector-index#hnsw-index-parameters) like the `distance`, `ef`, `efConstruction`, and more. It is important to note that some parameters cannot be changed once the collection is created.
...
Consider the answers in this [table](https://weaviate.io/developers/weaviate/config-refs/schema/vector-index#hnsw-configuration-tips) to help with your use case!
...
### Batch Imports
When importing data into Weaviate, we suggest using batch imports. This will send multiple objects in a single request, rather than importing them individually. There are [three methods](https://weaviate.io/developers/weaviate/client-libraries/python#batch-sizing) to choose from:
