...
### Batch Imports
When importing data into Weaviate, we suggest using batch imports. This will send multiple objects in a single request, rather than importing them individually.

:::note Update since publication
Weaviate has since added [server-side batching](https://docs.weaviate.io/weaviate/manage-objects/import#server-side-batching), and it is now the recommended starting point. The server tells the client how fast to send data based on its own load, so there is no batch size or concurrency for you to tune. Open the `stream()` context manager, or use the one-shot `collection.data.ingest(objects)` for a straight bulk load. Both need Weaviate `1.36` or newer and Python client `4.20` or newer. The three client-side methods below still work, and they are what we used for this demo in May 2024.
:::
...
There are [three client-side methods](https://docs.weaviate.io/weaviate/client-libraries/python#batch-processing) to choose from:
