---
title: Weaviate and Modal for Performant Data Ingestion
slug: modal-and-weaviate
description: 'Learn how to vectorize 40 million objects and ingest into Weaviate using Modal!'
...
<figure style={{ maxWidth: "70%", margin: "0 auto" }}>
  <img
    ...
    style={{ width: "100%" }}
  />
  <figcaption>An analogy from our system: Kobe Bryant is the Albert Einstein of Basketball.</figcaption>
...
You can either [manually](/developers/weaviate/configuration/pq-compression#manually-configure-pq) enable PQ or have it [automatically triggered](/developers/weaviate/configuration/pq-compression#configure-autopq) based on the number of vectors. You can manually enable PQ by setting it at the collection level. You’ll first need to import at least 10,000 to 100,000 vectors in order to form the centroids. AutoPQ can be used for new collections and the training step to form the centroids is set at 100,000 vectors by default. For open-source users, you can enable it by adding `ASYNC_INDEXING=true` in your environment variable, or you can enable it through Weaviate Cloud Services.
...
For our application, we used dynamic batch import and saw an improvement in the import speed! 
