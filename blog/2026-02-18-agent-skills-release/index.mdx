...
- **Skills (/skills/weaviate):** These are focused, Weaviate-specific scripts. They handle things like schema inspection, data ingestion, and precision search. These are the tools your agent reaches for when it's writing your backend code.
- **Cookbooks (/cookbooks):** These are full end-to-end project blueprints. They provide agents with a comprehensive guideline of a complete application (e.g., FastAPI, Next.js, and Vector Database). These are for the vibe coding phase, where you want to describe an entire system and have the agent architect it at once.
