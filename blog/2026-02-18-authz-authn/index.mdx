...
Put simply: authentication answers _"who are you?"_ while authorization answers _"what are you allowed to do?"_. You need both—authentication without authorization means everyone who logs in gets full access, and authorization without authentication means you can't reliably enforce any rules. The rest of this guide covers how Weaviate implements each.
...
Weaviate supports [anonymous access](https://docs.weaviate.io/deploy/configuration/authentication#anonymous-access), where no authentication is required to access the database.
...
API keys work well for many production scenarios, especially when combined with [role-based access control](#authorization-in-weaviate-rbac).
...
OIDC also works with [RBAC](#authorization-in-weaviate-rbac), and supports **OIDC groups** — allowing you to manage role assignments at the group level in your identity provider rather than per-user in Weaviate. We'll explore OIDC groups in detail in an upcoming blog post.
...
If you need a managed solution, Weaviate Cloud's **[Premium plan](https://weaviate.io/pricing)** offers dedicated infrastructure with SSO/SAML support, PrivateLink, and compliance certifications (SOC II, HIPAA).
...
- Multiple teams share a single Weaviate instance and should not interfere with each other (e.g., `core_products`, `marketing_campaigns`)
...
- **Roles** are containers for one or more permissions (e.g., `admin`, `dataScientist`, `searchApplication`)
...
When a request comes in, Weaviate checks the user's role(s), evaluates their permissions for the requested operation, and either allows or denies access accordingly. All access decisions are recorded in [audit logs](#audit-logging), giving you full visibility into who accessed what and when.

![RBAC diagram](./img/weaviate-rbac.png)

_RBAC structure showing how users inherit permissions through roles_
...
Weaviate records all authorization decisions in audit logs, giving you a detailed trail of who accessed what, when, and whether the request was allowed or denied.

This is valuable for:

- **Security monitoring**: Detect unauthorized access attempts or unusual patterns, such as a user repeatedly trying to access collections they don't have permissions for
- **Incident response**: When a security event occurs (like a compromised API key), audit logs let you determine exactly what was accessed and what was blocked
- **Compliance**: Many regulations (GDPR, HIPAA, SOC II) require demonstrable access controls and audit trails — Weaviate's logs provide the evidence

For configuration details, see the [Weaviate logging documentation](https://docs.weaviate.io/deploy/configuration/logging).
...
For organizations with enterprise security requirements, Weaviate Cloud's **Premium plan** offers dedicated infrastructure with SSO/SAML support for Console access, PrivateLink, and compliance certifications (SOC II, HIPAA). Check out the [Pricing Plan](https://weaviate.io/pricing) for details or [contact us directly](https://weaviate.io/contact) for more info.
...
- **Enterprise requirements?** Use Weaviate Cloud's [Premium plan](https://weaviate.io/pricing) for dedicated infrastructure with SSO/SAML, or deploy open source with [OIDC integration](https://docs.weaviate.io/deploy/configuration/authentication#oidc-authentication) — and use [Assurance](https://weaviate.io/contact) for paid support with your local installation, including hands-on help configuring OIDC, SSO, and RBAC in self-hosted environments
