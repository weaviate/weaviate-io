...
- [Default Compression for New Collections](#default-compression-for-new-collections)
...
The Java v6 client is available now at [Maven Central](https://central.sonatype.com/artifact/io.weaviate/client).
...
Weaviate `v1.35` continues to optimize default settings for new collections. While RQ compression has been the default for HNSW indexes since `v1.33`, the documentation now clarifies that:

- **Default quantization** can be set for new collections using the `DEFAULT_QUANTIZATION` environment variable
- **Flat and dynamic indexes** also respect default quantization settings
- **Index type compatibility** is automatically checked - quantization won't be applied if unsupported
...
- [Configuration: Vector Index](https://docs.weaviate.io/weaviate/config-refs/indexing/vector-index)
- [Configuration: Environment Variables](https://docs.weaviate.io/deploy/configuration/env-vars)
...
- **Improved LSM tree operations** - Better performance for disk-based operations
- **Enhanced cursor management** - More efficient query result pagination
- **Optimized WAL recovery** - Faster startup times after crashes
- **Better async replication** - Improved replication performance
- **Memory optimizations** - Reduced memory footprint for various operations
