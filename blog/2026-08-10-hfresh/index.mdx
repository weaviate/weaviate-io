...
image: ./img/hero.png
description: "HFresh is Weaviate's disk-based vector index for memory-efficient vector search, combining low heap usage with incremental background maintenance."
faq:
  - question: "What is HFresh?"
    answer: "HFresh is Weaviate's disk-based vector index for memory-efficient vector search. It keeps a compact centroid index and supporting metadata in memory and stores compressed vector postings on disk."
  ...
  - question: "When should I use HFresh instead of HNSW?"
    answer: "Use HFresh when lower memory use matters more than peak query throughput, whether you are building a small application with limited resources or working with a large, frequently updated dataset. Use HNSW when the lowest latency and highest throughput are the primary requirements."
---
...
HFresh is Weaviate's disk-based vector index for applications that prioritize lower memory use over peak query throughput. That tradeoff is useful for small applications with limited resources as well as large datasets. Weaviate Cloud's [Free Tier](https://docs.weaviate.io/cloud/manage-clusters/create#optimization-profile), for example, uses HFresh by default through its Cost Optimized profile.
...
Using HNSW here also means that improvements to HNSW automatically benefit HFresh. A good example is [ACORN](/blog/speed-up-filtered-vector-search), which improves filtered search by making graph traversal more efficient when only part of the dataset matches a filter. Since HFresh relies on HNSW as the centroid layer, those improvements are not isolated: they strengthen HFresh's query path.
...
HFresh uses RQ8 here because routing mistakes are expensive. If the centroid search misses the right region, the later posting scan may never see the true nearest neighbors. RQ8 gives a strong compromise: it makes centroid-vector payloads approximately **4x smaller** before overhead while keeping enough precision for accurate routing.
...
2. Navigate the centroid HNSW with [ACORN](/blog/speed-up-filtered-vector-search), selecting postings that can contribute at least one matching vector.
...
We benchmarked uncompressed HNSW, HNSW with RQ1, HNSW with RQ8, and HFresh on the DBpedia OpenAI 1M dataset. HNSW indexes were built with `efConstruction=256` and `maxConnections=16`.
...
The clearest difference is heap usage at rest. HFresh uses 239 MB of heap, compared with 6.67 GB for uncompressed HNSW.

HFresh also uses less heap than quantized HNSW. HNSW with RQ1 uses 715 MB, approximately 3x as much as HFresh, while HNSW with RQ8 uses 2.38 GB, approximately 10x as much.
...
HFresh gives Weaviate a disk-based index for memory-efficient vector search, from small applications to large, mutable datasets. By keeping a compact centroid index and supporting metadata in memory and maintaining disk-resident postings incrementally, it reduces the heap required for index data without relying on full index rebuilds. HNSW remains the better choice when the lowest latency and highest throughput matter most; HFresh is designed for workloads where a smaller heap footprint justifies lower query throughput.
