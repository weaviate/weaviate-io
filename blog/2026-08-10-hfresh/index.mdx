...
faq:
  - question: "What is HFresh?"
    answer: "HFresh is Weaviate's disk-based vector index for large-scale vector search. It keeps a compact centroid index and supporting metadata in memory and stores compressed vector postings on disk."
...
HFresh gives Weaviate a disk-based index for large, mutable vector datasets. By keeping a compact centroid index and supporting metadata in memory and maintaining disk-resident postings incrementally, it reduces the heap required for index data without relying on full index rebuilds. HNSW remains the better choice when the lowest latency and highest throughput matter most; HFresh is designed for workloads where a smaller heap footprint and scale justify lower query throughput.
