...
faq:
  ...
  - question: "How does HFresh differ from HNSW?"
    answer: "HNSW keeps its graph and vector cache in memory for the lowest latency and highest throughput. HFresh uses an in-memory HNSW index only for routing, then searches selected postings stored on disk to reduce memory requirements."
...
HFresh is Weaviate's disk-based vector index for large-scale vector search when memory efficiency matters more than peak throughput.

## Understanding HNSW's memory bottleneck in similarity search

When it comes to finding similar vectors quickly, HNSW (Hierarchical Navigable Small World) has become the gold standard. It's fast and accurate, but as datasets grow from millions to billions of vectors, HNSW reveals a fundamental constraint: it needs to keep everything in memory.
...
When fewer than 5,000 vectors satisfy the filter, HFresh uses flat search and computes distances directly over those vectors.

For small allow lists, this brute-force approach can be faster than the regular search path, which must navigate the centroid graph, read postings from disk, and rescore candidates.

### Regular search

When 5,000 or more vectors pass the filter, HFresh uses the full search pipeline with one key modification.

Each posting's metadata records the IDs of the vectors it contains.

The process is:

1. Navigate the centroid HNSW using the [ACORN](https://weaviate.io/blog/speed-up-filtered-vector-search) search strategy.
2. Collect postings containing at least one vector that passes the filter.
3. Search the selected postings by brute force, skipping vectors that do not match the filter.
4. Rescore the top candidates using uncompressed vectors.
...
To compare HFresh with HNSW in practice, we benchmarked four index configurations on the DBpedia OpenAI 1M dataset: uncompressed HNSW, HNSW with RQ1, HNSW with RQ8, and HFresh. We used default index parameters, varied the search-time parameters, and measured heap usage at rest and query throughput.
...
This difference follows directly from HFresh's architecture. By keeping only the RQ8-compressed centroid index in memory and storing postings on disk, HFresh reduces the need for high-memory instances as datasets grow.

### Query throughput

![Query throughput versus recall for HFresh, uncompressed HNSW, HNSW with RQ1, and HNSW with RQ8.](./img/qps-recall.png)

The QPS-recall curve shows the query-performance side of HFresh's memory advantage.
...
These results clarify when to choose each index. HNSW remains the better option when the lowest latency and highest throughput matter most. HFresh is designed for workloads where keeping the full index in memory is too expensive and lower throughput is an acceptable tradeoff for a much smaller memory footprint.

## Conclusion

HFresh gives Weaviate a disk-based index for large, mutable vector datasets. By keeping only the centroid index in memory and maintaining disk-resident postings incrementally, it reduces memory requirements without relying on full index rebuilds. HNSW remains the better choice when the lowest latency and highest throughput matter most; HFresh is designed for workloads where memory efficiency and scale justify lower query throughput.

HFresh has been available since Weaviate 1.36. See the [documentation](https://docs.weaviate.io/weaviate/concepts/vector-index#hfresh-index) to get started.
