...
Weaviate `v1.36` introduces **HFresh**, a new cluster-based vector index inspired by the [SPFresh algorithm](https://arxiv.org/abs/2410.06981) ("SPFresh: Incremental In-Place Update for Billion-Scale Vector Search"). HFresh is now available as a **technical preview**.

Unlike HNSW, which builds an in-memory graph that can be expensive to maintain at scale, HFresh takes a fundamentally different approach. It organizes vectors into **posting lists** (clusters), each represented by a centroid vector. Search works in three steps:

1. **Partitioning** — Vectors are grouped into clusters, each with a centroid
2. **Centroid indexing** — An HNSW index over the centroids enables fast cluster lookup
3. **Cluster search** — Only the most relevant posting lists are scanned for a given query

This architecture means HFresh only needs the centroid index and metadata in memory — the full vectors live on disk. The result is **significantly lower memory usage** compared to HNSW, while maintaining competitive search performance.

### Key Capabilities

- **Low memory footprint** — Only centroids are held in memory, making HFresh particularly effective for large-scale datasets and high-dimensional embeddings where HNSW's memory requirements become prohibitive
- **Balanced K-Means splitting** — Clusters are organized using balanced K-Means, improving search quality by keeping cluster sizes uniform
- **RQ compression** — Built-in Rotational Quantization compresses vectors aggressively while maintaining high recall through rescoring
- **Filtered search** — Native support for filtered vector search, combining metadata filters with approximate nearest neighbor queries
- **Cost-efficient scaling** — Reduced memory requirements enable cost-sensitive deployments with lower infrastructure overhead
...
| ---------------------- | ------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **`searchProbe`**      | `64`    | Yes     | Number of posting lists to search per query. Higher values improve recall at the cost of latency.                                                                                                                                                                                |
| **`replicas`**         | `4`     | No      | Number of posting lists each vector is added to (min: `1`, max: `10`). Higher values improve recall by increasing the chances of finding relevant vectors during search.                                                                                                         |
| **`maxPostingSizeKB`** | `48`    | Yes     | Maximum size in KB for a posting list (min: `8`, max: `1024`). Weaviate uses this value along with the vector dimensions to calculate the maximum number of vectors per posting. Larger values allow bigger clusters, which can improve recall but increase per-query scan time. |

Since `replicas` is immutable after collection creation, choose this value carefully based on your recall requirements. `searchProbe` and `maxPostingSizeKB` can be adjusted at runtime to fine-tune the recall/latency trade-off as your dataset grows.

### Limitations

- Only supports `cosine` and `l2-squared` distance metrics (`dot` is not supported)
...
- **HFresh (Preview)** — A new segment-based vector index designed for high write throughput and memory efficiency
- **Server-side Batching GA** — Production-ready server-managed batching with flow control and automatic retries
- **Object TTL GA** — Automatic object expiration with batch deletions, throttling, and observability
- **Async Replication GA** — Improved reliability with automatic shard recovery and better error handling
