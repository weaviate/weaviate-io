---
title: '32x Reduced Memory Requirements Using Binary Quantization!'
slug: binary-quantization
authors: [abdel, zain]
date: 2024-04-02
tags: ['research','engineering']
image: ./img/hero.png
description: "Binary quantization in Weaviate"
---
![Binary Quanitzation in Weaviate - How to Reduce Memory Requirements 32x](./img/hero.png)


:::info Highlights
- [What is Binary Quantization?](#product-quantizationpq) - how BQ reduces the memory requirements of running Weaviate
- [Details of Working with Binarized Vectors](#the-problem-with-pq) - What to consider when using binary quantization
- [Weaviate Performance Improvements with BQ](#experiment-results) - QPS vs. recall, indexing times, and memory savings
- [Comparing between Product and Binary Quantization](#experiment-results)
- [Benchmarking BQ](#benchmarking-pq-with-your-own-data) - scripts for benchmarking BQ with your own data
:::
...
![bq vectors](./img/bq_vecs.png)

:::info Enabling Binary Quantization in Weaviate
To learn how to enable and configure Weaviate to use BQ refer to the docs [here](/developers/weaviate/configuration/bq-compression). You can read more about HNSW+BQ in the documentation [here](/developers/weaviate/concepts/vector-index#binary-quantization).
:::
...
In this scenario, counterintuitively, the quantizer would indicate that yellow points are distant from red points and closer to both blue and green points. While this holds in angular-based metrics like `cosine`, it contradicts the interpretation under the `L2` metric where we can see that the yellow and red points are closer in proximity.
...
However, the upside lies in the exponential growth of regions with additional dimensions. With each dimension, the number of regions doubles (2d), offering greater potential for differentiation between vector representations. For instance, with 756 dimensions, you already have astonishing 2756 regions at your disposal - this makes collisions between vectors highly improbable even when you have billions or trillions of vectors. With 1.5K dimensions, the number of regions can easily accommodate any practical ammount of vectors without a single collision.
...
**In essence, the higher the dimensionality of your data, the more effectively you can expect binary quantization to perform.**
...
**Tab. 1**: *Vector search time improvements between uncompressed and BQ.*
...
**Tab. 2**: *Indexing time improvements between flat, HNSW uncompressed and BQ indices.*
...
**Tab. 3**: *Memory footprint improvements for the various indices.*
...
**Tab. 4**: *Comparing the various indices and compression techniques.*
