...
:::note Model update
Cohere has released newer multilingual models since this post was published. The current recommended default is `embed-multilingual-v3.0`, and `embed-v4.0` is the flagship — see the [Cohere embeddings docs](https://docs.weaviate.io/weaviate/model-providers/cohere/embeddings).
:::
...
If you have interesting data project ideas, give me a shout on [Twitter @sebawita](https://twitter.com/sebawita).
...
* `return_properties` – a list of properties we want back from the database
* `collection.query.near_text(...)` – runs a semantic search. Note, how it doesn’t even mention the Cohere model, as all of that is done through configuration.
...
def semantic_search(query):
    ...
    response = documents.query.near_text(
        ...
        distance=0.7,
        limit=5,
        return_properties=["name", "content", "enName", "link", "lang"],
