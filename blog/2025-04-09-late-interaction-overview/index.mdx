...
The advantage of full interaction retrieval models is that they are contextually rich, meaning they effectively capture nuanced contextual relationships between the search query and the document. However, the disadvantage is that they are computationally expensive because all documents have to be processed at query time. That’s why full interaction models are not scalable for large amounts of documents. 
...
The advantage of late interaction retrieval models is that they are both scalable and contextually rich. On the downside, the disadvantage of late interaction retrieval models is that they require an embedding for each token, which requires a lot more storage for a complete set of vectors. However, token embeddings can be stored at a much lower dimension and quantization level to decrease computational costs and increase storage efficiency - mitigating some of these issues. For example, in ColBERTv2, aggressive quantization is performed to reduce the size of each vector from 256 bytes to 36 bytes (2-bit compression) or even 20 bytes (1-bit compression), whilst maintaining a good level of accuracy (see ColBERTv2).
...
The same fine-tuned BERT model is used as both the query encoder and the document encoder separately,  but an additional special token of [Q] and [D] is prepended to the query and document respectively. This marker is included in training, and the model learns to distinguish the query and document via these special tokens.

The full architecture of ColBERT is given in the following: tokenize the query/document (and include special token [Q] or [D]), forward pass the tokens through a 110M fine-tuned BERT-base transformer model, project the final BERT representations with a special projection layer to reduce the embedding dimension to 128 instead of 768, then compare all tokens between query and document using late interaction and MaxSim.
...
One advantage of ColBERT is its impact on retrieval performance. Because it’s scalable and contextually rich, it can improve retrieval accuracy and, therefore, overall performance in RAG applications and ranking pipelines. 
...
However, the multi-vector approach of the original ColBERT model has one big disadvantage of increased storage footprint. Because with ColBERT you now have one vector embedding for **each token** in a document instead of **one** vector embedding for one document, the storage requirements and memory usage balloon up.

Let’s compare the storage requirements of dense vector embeddings and multi-vector embeddings. Assume that each document consists of 100 words on average, and we are storing a variable number of documents. The table below gives the storage cost, in megabytes, to store all of the documents as their respective embeddings.

|  |  | Single Vector Embeddings |  |  | Multi Vector Embeddings | (ColBERTv2 quantization) |  |
| --- | --- | --- | --- | --- | --- | --- | --- |
|  |  | **d=768** | **d=1024** | **d=1536** | **d=64** | **d=96** | **d=128** |
| Number of Documents (n)  | **n=100** | 0.15 MB | 0.20 MB | 0.31 MB | 0.36 MB | 0.55 MB | 0.73 MB |
| (100 tokens per document) | **n=500** | 0.77 MB | 1.02 MB | 1.54 MB | 1.26 MB | 1.89 MB | 2.52 MB |
...
[ColPali](https://arxiv.org/abs/2407.01449)’s model architecture uses the PaliGemma Vision LLM: It uses Gemma as the query encoder and PaliGemma as the vision encoder. Additionally, ColPali has:
...
[ColQwen](https://huggingface.co/vidore/colqwen2-v0.1)’s model architecture uses the Qwen2 Vision LLM as vision encoder and has: 
...
The advantages of multimodal late interaction retrieval models like ColPali and ColQwen are that they allow for RAG applications without a complex preprocessing or chunking strategy for the PDF documents. 
