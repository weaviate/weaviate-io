...
- [API-based RBAC user management](#api-based-rbac-user-management)
...
## API-based RBAC user management

Role-based access control (RBAC) in Weaviate `v1.30` now supports API-based user management. This means that you can now manage roles and permissions from its client libraries, or using the REST API, as long as you have the necessary permissions.

This should make administration of Weaviate instances easier. It means managing RBAC users no longer requires direct access to the Weaviate instance, and can be done programmatically without restarting the instance.

You can now create or delete users, as well as to rotate their API keys, all from the client library or REST API.
