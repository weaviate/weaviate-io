...
Following our introduction of the [**`QueryAgent`**](/blog/query-agent) last week, we’re back with yet another one: The **`TransformationAgent`** is now ready in public preview for all [Weaviate Serverless Cloud](/deployment/shared) users (including free Sandbox users)!
