...
:::note Model versions and defaults have changed since 2023
This walkthrough was recorded in January 2023, when `text2vec-cohere` used Cohere's `embed-multilingual-v2.0` model. The `768` dimensions and the exact distances below come from that model.
:::
...
questions = client.collections.use("Question")
wv_resp = questions.query.near_text(
    query="fast food chains",
    limit=2,
...
co_resp = co.embed(texts=[str_in], model="embed-multilingual-v2.0")
```
...
questions = client.collections.use("Question")
questions.query.near_vector(
    near_vector=co_resp.embeddings[0],
    limit=2,
    return_metadata=MetadataQuery(distance=True),
)
```
...
* The property `dataType` determines whether Weaviate will ignore the property, as it will ignore everything but `text` (and `text[]`) values.
...
client.collections.create(
    ...
    vector_config=Configure.Vectors.text2vec_cohere(
        vectorize_collection_name=False,  # Ignore collection name
    ),
    properties=[
        Property(
            name="answer",
            ...
            skip_vectorization=False,        # Do not skip this property
            vectorize_property_name=False,   # Ignore property name
        ),
        Property(
            name="question",
            ...
            data_type=DataType.TEXT,
            skip_vectorization=False,        # Do not skip this property
            vectorize_property_name=True,    # Do not ignore property name
        ),
    ],
)
```
...
- Vectorize properties that use the `text` (or `text[]`) data type
