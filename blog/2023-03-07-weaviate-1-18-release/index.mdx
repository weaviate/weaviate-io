...
tags: []
image: ./img/hero.png
description: "Weaviate 1.18 introduces Roaring Bitmaps,   Learn all about it."
---
...
1. [Vector Indexing](#vector-indexing) – enables great performance at a fraction of the previous RAM requirements
...
1. [Replication](#replication) – two new replication features: tunable consistency for all read and write operations, and repair on read
...
![roaring bitmaps](./img/roaring-bitmaps.png)
...
Weaviate version `1.18` and onwards will include this feature, and our team will be maintaining our underlying Roaring Bitmap library to address any issues and make improvements as needed.
...
![vector indexing](./img/vector-indexing.png)
...
The `where` filter allows you to narrow down your data based on specified conditions. [Vector search parameters](/developers/weaviate/api/graphql/vector-search-parameters) are applied to GraphQL queries on the class level. We have added `where` filters to BM25 and hybrid search!
...
Weaviate version 1.18 introduces two new [replication](/developers/weaviate/concepts/replication-architecture) features: [tunable consistency](#tunable-consistency) for all read and write operations, and [repair-on-read](#repair-on-read).

### Tunable consistency
As a distributed database subject to the [CAP theorem](/developers/weaviate/concepts/replication-architecture/index.md#cap-theorem), Weaviate favors availability over consistency for data, and is strictly consistent when it comes to schema operations. **Tunable consistency** allows you to decide between three levels of consistency vs. availability:
...
Version `1.17` supported tunable consistency for [fetching by id](/developers/weaviate/api/rest/objects.md#get-a-data-object). Weaviate `1.18` adds tunable consistency support for all read and write REST operations, including [batching](/developers/weaviate/api/rest/batch.md) and [cross-references](/developers/weaviate/api/rest/objects.md#cross-references). GraphQL queries do not support tunable consistency yet.
...
We have added an object ID-based `after` cursor to both the REST and GraphQL APIs in this release. This enhancement makes it easy for you to systemically retrieve all objects in a class from Weaviate. 

### How to use
To use this with the `objects` REST endpoint, provide an object uuid with the `after` parameter in the GET request. The syntax looks like this:

`<WEAVIATE_INSTANCE>/v1/objects?class=<CLASS_NAME>&limit=<N_OBJECTS>&after=<UUID>`

Where:

- `<WEAVIATE_INSTANCE>` is Weaviate URL (e.g. `https://yoursandbox.weaviate.io`),
- `<CLASS_NAME>` is the class (e.g. `Article`) being retrieved (this MUST be set), 
- `<N_OBJECTS>` are the number of objects to be retrieved in that request, and
...
Or, you can use this with the `GET` function with a GraphQL query.
...
This releases extends our cloud-native backup capabilities ([added in the 1.15 release](https://weaviate.io/blog/weaviate-1-15-release#cloud-native-backups)) further by introducing the `backup-azure` module. 
