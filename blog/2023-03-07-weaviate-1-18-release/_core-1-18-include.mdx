...
Weaviate version `1.18` and onwards will include this feature, and our team will be maintaining our underlying Roaring Bitmap library to address any issues and make improvements as needed. To read more about pre-filtering read the documentation [here](https://weaviate.io/developers/weaviate/concepts/filtering)
...
The [`where` filter](https://weaviate.io/developers/weaviate/api/graphql/filters#where-filter) allows you to narrow down your data based on specified conditions. [Search operators](https://weaviate.io/developers/weaviate/api/graphql/search-operators) are applied to GraphQL queries on the class level. We have added `where` filters to BM25 and hybrid search!
...
Check out the documentation for more information on how to add filters to [BM25](https://weaviate.io/developers/weaviate/api/graphql/search-operators#bm25-with-where-filter) and [hybrid](https://weaviate.io/developers/weaviate/api/graphql/search-operators#hybrid-with-where-filter).
...
For more information on how to update a class using the RESTful API, check out the [documentation](https://weaviate.io/developers/weaviate/api/rest#tag/schema).

### Ongoing Fixes and Improvements
Although [BM25](https://weaviate.io/developers/weaviate/api/graphql/search-operators#bm25) and [Hybrid Search](https://weaviate.io/developers/weaviate/api/graphql/search-operators#hybrid) were released in Weaviate `1.17`, we continued to make fixes and improvements. Here are a list of the patch releases related to BM25/Hybrid:
...
Weaviate version 1.18 introduces two new [replication](https://weaviate.io/developers/weaviate/concepts/replication-architecture) features: [tunable consistency](#tunable-consistency) for all read and write REST endpoints, and [repair-on-read](#repair-on-read).

### Tunable consistency
As a distributed database subject to the [CAP theorem](https://weaviate.io/developers/weaviate/concepts/replication-architecture/consistency), Weaviate favors availability over consistency for data, and is strictly consistent when it comes to schema operations. **Tunable consistency** allows you to decide between three levels of consistency vs. availability:
...
Version `1.17` supported tunable consistency for [fetching by id](https://weaviate.io/developers/weaviate/api/rest#tag/objects/get/objects/%7BclassName%7D/%7Bid%7D). Weaviate `1.18` adds tunable consistency support for all read and write REST operations, including [batching](https://weaviate.io/developers/weaviate/api/rest#tag/batch) and [cross-references](https://weaviate.io/developers/weaviate/api/rest#tag/objects/post/objects/%7BclassName%7D/%7Bid%7D/references/%7BpropertyName%7D). GraphQL queries do not support tunable consistency yet.
...
Learn about the Cursor API in the [documentation](https://weaviate.io/developers/weaviate/api/graphql/additional-operators#cursor-with-after).
...
The module supports Azure’s Storage Connections strings, as well as name-and-key based authentication into the storage box. Please refer to [our documentation here](https://weaviate.io/developers/weaviate/configuration/backups#azure-storage) for information on how to configure it.
