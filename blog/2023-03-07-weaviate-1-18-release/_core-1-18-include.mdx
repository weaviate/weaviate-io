...
For more information on how to update a class using the RESTful API, check out the [documentation](/developers/weaviate/api/rest#tag/schema).
...
Version `1.17` supported tunable consistency for [fetching by id](developers/weaviate/api/rest#tag/objects/get/objects/%7BclassName%7D/%7Bid%7D). Weaviate `1.18` adds tunable consistency support for all read and write REST operations, including [batching](/developers/weaviate/api/rest#tag/batch) and [cross-references](developers/weaviate/api/rest#tag/objects/post/objects/%7BclassName%7D/%7Bid%7D/references/%7BpropertyName%7D). GraphQL queries do not support tunable consistency yet.
