...
This implementation is [Weaviate's own version](https://weaviate.io/developers/weaviate/concepts/filtering) of that described in the original paper, with modifications to make it even better for Weaviate users.

### How to use ACORN

ACORN can be enabled as the filter strategy by setting `filterStrategy` for the relevant HNSW vector index [in the collection configuration](https://weaviate.io/developers/weaviate/manage-data/collections#set-vector-index-parameters).
...
In Weaviate `1.27`, multi-target vector search has been improved further to allow searching the [same vector field multiple times](https://weaviate.io/developers/weaviate/search/multi-vector#specify-query-vectors) in one query.

These new options provide even more flexibility for comprehensive searches through your data with Weaviate.

Take a look at the [multi-target vector search page](https://weaviate.io/developers/weaviate/search/multi-vector) for more examples.

:::info What is a multi-target vector search?

Weaviate `1.26` introduced [multi-target vector searches](https://weaviate.io/developers/weaviate/search/multi-vector), where one search query can search across multiple target vectors.
<br/>
...
Weaviate's [Jina AI embedding integration](https://weaviate.io/developers/weaviate/model-providers/jinaai/embeddings) now supports Jina AI's new V3 embeddings.

### How to use the v3 embedding model

From your perspective, all you have to set is the model name, to `jina-embeddings-v3` [here](https://weaviate.io/developers/weaviate/model-providers/jinaai/embeddings#configure-the-vectorizer).
...
Note you can combine Weaviate's [quantization techniques](https://weaviate.io/developers/weaviate/starter-guides/managing-resources/compression) with this to further manage resource usage.

See the [Jina AI model integration page](https://weaviate.io/developers/weaviate/model-providers/jinaai/embeddings) for information on how to use this model.
...
[Creating backups](https://weaviate.io/developers/weaviate/configuration/backups) is a big part of database administration, especially in a production setting.
...
Backups can be cancelled [through a supported Weaviate client](https://weaviate.io/developers/weaviate/configuration/backups#cancel-backup), or through the [REST endpoint](https://weaviate.io/developers/weaviate/api/rest).
...
Weaviate [integrates with embedding model providers](https://weaviate.io/developers/weaviate/model-providers) in order to conveniently vectorize data during import.
:::
...
If you prefer the previous behavior where the inputs are lowercased before import, set the `LOWERCASE_VECTORIZATION_INPUT` [environment variable](https://weaviate.io/developers/weaviate/config-refs/env-vars#general) to `true`.

### Google modules renamed

The Weaviate modules that underpin its [Google model integrations](https://weaviate.io/developers/weaviate/model-providers/google) have been renamed. Historically, they were named after the `PaLM` generation of models, thus having names `text2vec-palm`, `generative-palm`, `multi2vec-palm`.
...
[Product quantization (PQ)](https://weaviate.io/developers/weaviate/concepts/vector-quantization#product-quantization) is a technique used to compress vectors for efficient storage and retrieval.
