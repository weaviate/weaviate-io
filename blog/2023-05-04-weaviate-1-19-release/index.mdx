...
### Fixes for BM25
As an ongoing effort to improve BM25, we realized there was an error when calculating the average length of a property. Two issues were found:

- The component would encounter errors if too many properties were present
- BM25 could produce incorrect mean values if properties had a zero-length.

To fix this, the property length tracker has been completely replaced. This is a non-breaking change and will seamlessly migrate at startup. This will result in a more accurate BM25 search.

This change was part of the `1.19.0` release.
