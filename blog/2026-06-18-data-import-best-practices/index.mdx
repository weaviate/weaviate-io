...
faq:
  ...
  - question: 'Can Weaviate vectorize PDFs?'
    answer: "Yes. Render each page to an image and embed it with a multimodal model — no OCR, text extraction, or layout detection required. The lowest-friction path is a Weaviate Cloud instance with Weaviate Embeddings, whose multimodal model is built for image-based document retrieval. Note that the multimodal modules embed page images, not raw PDF files, so the page-to-image step is on you."
...
Another ready-made option is Google's `multi2vec-google` (with `gemini-embedding-2` at 3072 dimensions), enabled by default on Weaviate Cloud. It follows the same workflow: you render pages to images and embed those. The module takes image input, not raw PDF files, so the rasterization step is the same as with Weaviate Embeddings.
...
| Provider                | Module               | Text | Image | Audio | Video |
| ----------------------- | -------------------- | ---- | ----- | ----- | ----- |
| Weaviate Embeddings     | native (WCD)         | ✓    | ✓     |       |       |
