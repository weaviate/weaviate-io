...
The `volumes` parameter [mounts a volume](/developers/weaviate/current/configuration/persistence.html) from outside the container such that Weaviate can save data to it. Setting it to `./backups:/tmp/backups` maps `./backups` on the local device to `/tmp/backups` within the container, so the generated backup data will end up in the `./backups` directory as you will see later on.
...
* [Local backup](/developers/weaviate/current/configuration/backups.html#filesystem)
* [GCS backup](/developers/weaviate/current/configuration/backups.html#gcs-google-cloud-storage)
* [S3 backup](/developers/weaviate/current/configuration/backups.html#s3-aws-or-s3-compatible)
