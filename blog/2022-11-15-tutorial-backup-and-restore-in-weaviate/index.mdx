...
The `volumes` parameter [mounts a volume](/developers/docs/configuration/persistence) from outside the container such that Weaviate can save data to it. Setting it to `./backups:/tmp/backups` maps `./backups` on the local device to `/tmp/backups` within the container, so the generated backup data will end up in the `./backups` directory as you will see later on.
...
* [Local backup](/developers/docs/configuration/backups#filesystem)
* [GCS backup](/developers/docs/configuration/backups#gcs-google-cloud-storage)
* [S3 backup](/developers/docs/configuration/backups#s3-aws-or-s3-compatible)
