...
:::caution Outdated preview content
Published 2023-09-26 as a *preview* of the v4 Python client. Several API names in this post changed before GA (e.g. `client.collection.get` became `client.collections.get`, and was later renamed again to `client.collections.use`), and `wvc.ConfigFactory` was renamed to `wvc.config.Configure` before release, so the snippets below will not run as written. Collection creation has since moved on again: `vectorizer_config=` is now `vector_config=`, and the `Vectorizer` factory is now `Vectors`. For the current API, see the [Python client docs](https://docs.weaviate.io/weaviate/client-libraries/python). The official v4 release announcement is [here](/blog/py-client-v4-release/).
:::
