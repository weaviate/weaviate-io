...
> **Get started with [Engram](/product/engram) in [Weaviate Cloud](https://console.weaviate.cloud/) today.**
...
All of your Engram configuration is then collected into groups, which combine the topics (which control _what_ memories are extracted) and associated pipeline (which controls _how_ memories are extracted). Memories belonging to one group are isolated from other groups using multi-tenancy, and should correspond to distinct use cases in your application that you want to keep separate. Each group can contain multiple topics, and each topic can have different scopes.
...
[
  {
    "topic": "task_goal",
    "content": "User asked assistant to query their collection for comedy movies."
  },
  {
    "topic": "actions_taken",
    "content": "Assistant called the search tool to do a near-text query on 'comedy'."
  }
]
```
...
[
  {
    "topic": "feedback",
    "content": "Comedy is a genre, and so you should filter on the 'genres' property."
  }
]
```
