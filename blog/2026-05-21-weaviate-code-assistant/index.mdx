...
services:
  weaviate:
    image: cr.weaviate.io/semitechnologies/weaviate:1.38.8
    ports:
      - '8080:8080'
      - '50051:50051'
    environment:
...
:::info Auth for production
This example uses `AUTHENTICATION_ANONYMOUS_ACCESS_ENABLED: 'true'` so the post stays focused on the MCP wiring. For any networked deployment, enable an API key and add `Authorization: Bearer <key>` to your client config — Weaviate's MCP server respects standard auth and RBAC. See [§"Going further"](#going-further) for the RBAC permissions involved.
:::
...
Ingest with [server-side batching](https://docs.weaviate.io/weaviate/manage-objects/import#server-side-batching). `data.ingest()` takes any iterable, so a generator expression over the repo streams chunks to the server as they are produced, at whatever pace the server asks for:

```python
code_chunks = client.collections.use("CodeChunks")

code_chunks.data.ingest(
    chunk
...
:::note Requires Python client 4.20+
:::
...
doc_chunks = client.collections.use("DocChunks")

doc_chunks.data.ingest(
    {
        "content": content,
        "title": md_file.stem,
    ...
    for content in chunk_markdown(md_file.read_text())
)
```
...
try:
    ...
    code = client.collections.use("CodeChunks")
    code_count = 0
    with code.batch.stream() as batch:
    ...
    docs = client.collections.use("DocChunks")
    doc_count = 0
