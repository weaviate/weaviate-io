...
Asynch replication address this risk and minimizes it. The aysnch replication mechanism uses an efficient algorithm to monitor and compare nodes within a cluster. If the algorithm identifies an inconsistency, it resyncs the data on the inconsistent node.
...
Weaviate gathers metrics that are compatible with the open-source [Prometheus](https://prometheus.io/docs/introduction/overview/) monitoring and alerting system. The v1.26 release adds [new metrics](/developers/weaviate/configuration/monitoring) that monitor aysnc indexing queues.
