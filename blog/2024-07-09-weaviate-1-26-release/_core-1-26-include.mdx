...
Asynchronous replication is much more effective than repair-on-read when there are many inconsistencies. If, for example, an offline node misses a series of updates, asynchronous replication quickly restores consistency when the node returns to service.
