...
Isn’t it annoying when you just want to find a remedy for your headache and instead find out that you have a horrible disease? With Healthsearch you will find exactly and only that - and no unsolicited health diagnosis.
...
Imagine just being able to type, 'I need better sleep,' and instantly get a list of related supplements tailored to enhance your sleep quality. Or you're dealing with persistent joint pain, and you're looking for a natural supplement to alleviate discomfort.

We are super excited to release our latest open-source demo, Healthsearch. This demo decodes user-written reviews of supplements and performs semantic- and generative search on them, retrieving the most related products for specific health effects based on the reviews, and leveraging Large Language Models to generate product and review summaries. The demo can understand natural language queries and derive all search filters directly from the context of your query.

In this blog post, I’ll give you a quick tour of the Healthsearch project and its exciting features. We will also go through the implementations and the challenges I faced while working on it.
...
<figure>
<img
    src={require('./img/query.gif').default}
    alt="alt"
    style={{ width: "100%" }}
/>
...
<figure>
<img
    src={require('./img/reviews.gif').default}
    alt="alt"
    style={{ width: "100%" }}
/>
...
<figure>
<img
    src={require('./img/generative.gif').default}
    alt="alt"
    style={{ width: "100%" }}
/>
...
Generating a query and its summary, mainly when using the GPT4 model, can be a time-intensive process. Consequently, caching previous results becomes crucial to improving the system's efficiency. But what if we could take it a step further? What if we employed Weaviate not only as a cache but also took advantage of its semantic search capabilities?
...
The repository for this project is entirely open-source, granting you access to every line of code, both front-end and back-end. We also offer an extract of our supplement dataset, enabling you to run the entire demo on your personal device. The demo can serve as a starting point and a template for displaying your own data.
