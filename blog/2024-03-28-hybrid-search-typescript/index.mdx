...
Think of alpha as a measure of how much your hybrid search should lean towards either vector search or keyword search where an alpha of 1 is a pure vector search, and an alpha of 0 is a pure keyword search. It is 0.5 by default.
...
const quotesCollection = await client.collections.create({
  ...
  vectorizers: weaviate.configure.vectors.text2VecOpenAI('default'),
});
```
...
export async function findQuotesBySearchTerm(
  ...
  const collection = await client.collections.use<Omit<QuoteType, 'distance'>>(
    'Quotes'
  );
...
If you'd like to dive into the code, you can take a look at the projects [GitHub repository](https://github.com/weaviate/quote-finder).
