...
export async function findQuotesBySearchTerm(
  ...
  const collection = client.collections.use<'Quotes', Omit<QuoteType, 'score'>>(
    'Quotes'
  );
...
import { findQuotesBySearchTerm } from '@/actions';
...
export default function Home() {
  ...
  const handleSubmit = async () => {
    const quotes = await findQuotesBySearchTerm(searchTerm, alpha);
    setQuotes(quotes);
  };
