...
2. Use built-in [liveness and readiness](/developers/weaviate/api/rest/well-known) checks
...
::::note
If you are using Weaviate `1.17` or lower, you may want to upgrade to `1.18` before enabling Prometheus metrics. The reason being Weaviate previously published many histograms which has since been [replaced](https://github.com/weaviate/weaviate/pull/2605) by summaries for performance reasons. Additionally, be careful enabling Prometheus metrics if you have thousands of classes you may end up with high cardinality labels due to metrics being produced per class.
::::
...
<details>
<summary>See the yaml file</summary>
...
Dependening on the dataset and whether you are using a module for vectorizing data, monitoring relative batch
latency can give an indication if there is a problem with indexing data. This metric has a label `operation` which
allows you to see how long objects, vectors, and inverted index sub operations take.
...
Here are other products that have integrations for Prometheus that can also be used:
