...
In Weaviate `1.32`, we introduce **optimized HNSW connections**, which can significantly reduce the memory footprint of the HNSW graph structure. This is achieved by carefully selecting the precision of the connections between the nodes in the HNSW graph.

Two amazing things about this feature:
- It works out-of-the-box, without any configuration needed.
- You don't need to re-index your data, either. Once you upgrade to Weaviate `1.32`, the HNSW index will automatically start using the optimized connections.

The amount of memory reduction depends a number of factors. Generally, you will get the most benefit if you are using ColBERT, or ColPali-type multi-vector embeddings, which are made up of many, smaller vectors. In this case, you could get up to 20% memory reduction for the HNSW index, as the HNSW graph structure takes up a larger relative share of the total memory usage.
