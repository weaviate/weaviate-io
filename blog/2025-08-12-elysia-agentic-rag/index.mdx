...
Elysia is architectured as a modern web application with a full-featured frontend for a responsive, real-time interface and a FastAPI backend serving both the web interface and API. The core logic is written in pure Python – what we call "blood, sweat, and tears" custom logic – with [DSPy](/integrations/llm-agent-frameworks/dspy) handling LLM interactions for flexible, future-proof implementations. 
...
import elysiatree from './img/elysiatree.mp4';

<video width="100%" autoplay loop controls>
  <source src={elysiatree} type="video/mp4" />
Your browser does not support the video tag.
</video>
...
For the Glowe-Elysia tree, we created three custom tools specifically to the app’s needs: 
...
**The Elysian Council in Session |** Oil on canvas, contemporary   
*Danny, Edward, and Victoria review architectural plans during a council meeting.*
