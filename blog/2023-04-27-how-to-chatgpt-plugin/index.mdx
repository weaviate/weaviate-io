...
:::caution Historical post: the ChatGPT plugins program is retired
Published 2023-04-27, when ChatGPT plugins were in a closed alpha. OpenAI retired the program in 2024, so the `ai-plugin.json` manifest, the `.well-known` mount, and the plugin install flow below no longer apply. The hackathon this post promotes was held in May 2023, and the Weaviate code uses the removed `v3` Python client.

What still transfers is the service design. The model decides *when* and *how* to call your endpoints purely from the descriptions you write for them, which is the same thing that makes or breaks a tool definition today. The modern equivalent of this build is a [Model Context Protocol (MCP)](https://docs.weaviate.io/weaviate/configuration/mcp-server) server, and Weaviate ships one inside the database. For a current walkthrough, read [Build a Coding Assistant with Weaviate MCP](/blog/coding-assistant-weaviate-mcp).
:::
...
1. Create a remote Weaviate Instance: This was done using the [Weaviate Cloud](https://console.weaviate.cloud/)
...
3. Update the plugin manifest config files `ai-plugin.json` and `openapi.yaml` to now [use authentication](https://platform.openai.com/docs/plugins/getting-started/plugin-manifest) in the form of a bearer token and your newly created WCD instance instead of localhost.
