...
Transfer data from our Iceberg table to Weaviate with [server-side batching](https://docs.weaviate.io/weaviate/manage-objects/import#server-side-batching). `data.ingest()` accepts any iterable, so we hand it a generator over the DataFrame rows and the server paces the import for us. This requires Weaviate `1.36`+ and Python client `4.20`+.
```python
def save_reviews_to_weaviate(client, df):
    reviews_collection = client.collections.use('Airline_Reviews')
    ...
    return reviews_collection.data.ingest(review_objects())

reviews_saved = save_reviews_to_weaviate(weaviate_client, df)
print(f"Imported {len(reviews_saved.uuids)} reviews, {len(reviews_saved.errors)} errors")
```
### Hybrid Search
...
Retrieval augmented generation is a powerful technique that retrieves relevant data to provide to large language models (LLMs) as context, along with the task prompt. It is also called RAG, generative search, or in-context learning in some cases. In Weaviate, you can use RAG to either transform a [whole result set](https://docs.weaviate.io/weaviate/search/generative#grouped-task-search) into new text or generate new content for [individual objects](https://docs.weaviate.io/weaviate/search/generative#single-prompt-search). Please check docs to understand which suits your use case better. In our demo, we are using RAG `grouped_task` to retrieve reviews(correct context) and generate informed analysis.
