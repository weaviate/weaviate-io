...
2. **Effect Embedding (Positive and Negative):**
   - Captures the _skincare benefits_ of a product, primarily extracted and weighted from actual user reviews.
...
This is where it gets interesting. When building a user's curated skincare stack, we need to understand _how strongly_ a product is linked to certain effects. To reflect that, we use a **TF-IDF-weighted embedding** based on review mentions.
...
Count how often each effect appears in _this_ product's reviews:
...
$$
w_{\text{hydration}} \approx 0.14,\quad
w_{\text{soothing}} \approx 0.23,\quad
w_{\text{redness}} \approx 0.63
$$

$$
w_{\text{hydration}} + w_{\text{soothing}} + w_{\text{redness}} = 1
$$
...
$$
v_{\text{effect}} =
v_{\text{hydration}} \cdot w_{\text{hydration}} +
v_{\text{soothing}} \cdot w_{\text{soothing}} +
v_{\text{redness}} \cdot w_{\text{redness}}
$$
...
$$
\text{vector} = v_{\text{effect}} + \gamma D
$$
