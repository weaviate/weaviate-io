...
<details>
  <summary>macOS support</summary>

  macOS support for Embedded DB is in the works (you can track progress in [this issue](https://github.com/weaviate/weaviate-python-client/issues/239)). For now, you can use a Linux Docker image to follow along, for example if you want to dry-run a CI/CD pipeline. [Install Docker](https://docs.docker.com/desktop/install/mac-install/), then run this command to land in a bash Linux prompt:
...
The script will exit with an exit code of `0` if the test passed.
...
* Get started very quickly with Weaviate on your local machine, without having to explicitly download, install or instantiate a server.
* Use Weaviate from Jupyter notebooks.
* Play with Weaviate on Google Colaboratory. Here is the example above as [Colab notebook](https://colab.research.google.com/drive/1XAJc9OvkKhsJRmheqWZmjYU707dqEIl8?usp=sharing).
  ![Colab screenshot](./img/colab.png)
* Use Weaviate in CI/CD pipelines, where you can run integration tests without having to manage a separate server instance.
