...
A compressed vector has less information than the full, uncompressed vector. A vector that would normally match a search query might be missed if key information is missing from the compressed vector. That lowers recall.
