...
![Weaviate 1.22](./core-1-22-img/hero.png)

<!-- truncate -->
1. **Nested JSON object storage** - store nested JSON objects directly in Weaviate as property values
...
![Illustration of nested objects being added into Weaviate](./core-1-22-img/Weaviate-release-1-22-full-object-storage.png)

`1.22` welcomes new members to the Weaviate data type family, with `object` and `object[]` joining the available list.

This allows you to store nested JSON objects directly in Weaviate as property values. In other words, you can store them without having to flatten the object or use cross-references to other Weaviate classes.
...
![Indexing progress](./core-1-22-img/Weaviate-release-1-22-indexing-progress.png)
...
![gRPC & Weaviate](./core-1-22-img/Weaviate-release-1-22-grpc.png)
...
With these changes, we have seen improvements in import speeds of around ~60-80% for batch imports, `Get` queries are many times faster, too. A big part of this speedup is because gRPC is a binary, and HTTP/2 protocol is more efficient than the text-based GraphQL protocol.
...
![Python client](./core-1-22-img/Weaviate-release-1-22-python.png)
...
You may already have seen [a blog post](https://www.weaviate.io/blog/collections-python-client) about this new client, but we are excited to announce that it is now available in beta!
